...
// API server port from centralized config
const BABYLON_API_PORT = process.env.BABYLON_API_PORT ?? '5009'
const DEFAULT_BASE_URL = `http://localhost:${BABYLON_API_PORT}`

type TestScenarioKey = keyof typeof TEST_SCENARIOS
type A2ATestScenarioKey = keyof typeof A2A_TEST_SCENARIOS

function isTestScenarioKey(key: string): key is TestScenarioKey {
  return key in TEST_SCENARIOS
}

function isA2ATestScenarioKey(key: string): key is A2ATestScenarioKey {
  return key in A2A_TEST_SCENARIOS
}
...
async function runLoadTest(args: ReturnType<typeof parseArgs>): Promise<void> {
  ...
  const baseUrl = getOption(args, 'url') || DEFAULT_BASE_URL
  ...
  if (!isTestScenarioKey(scenarioKey)) {
    logger.fail(`Invalid scenario key: ${scenarioKey}`)
    process.exit(1)
  }
...
async function runA2AStressTest(
  ...
  const scenarioKey = scenario.toUpperCase().replace('-', '_')
  if (!isA2ATestScenarioKey(scenarioKey)) {
    logger.fail(`Invalid A2A scenario key: ${scenarioKey}`)
    process.exit(1)
  }
