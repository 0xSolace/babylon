...
export async function runParallelGeneration(
  ...
  let managerId: string | undefined = providedManagerId
  if (!managerId) {
    const adminUsersRaw = await db
      .select()
      .from(users)
      .where(eq(users.isAdmin, true))
      .limit(1)
    const adminUsers = adminUsersRaw as unknown as UserRow[]

    if (adminUsers[0]) {
      managerId = String(adminUsers[0].id)
      logger.info(
        `Using admin user as manager: ${adminUsers[0].username ?? 'unknown'}`,
      )
    } else {
      // Try any user
      const anyUserRaw = await db.select().from(users).limit(1)
      const anyUser = anyUserRaw as unknown as UserRow[]
      ...
      managerId = String(anyUser[0].id)
      logger.warn(
        `Using regular user as manager: ${anyUser[0].username ?? 'unknown'}`,
      )
    }
  }

  // managerId must be defined at this point
  if (!managerId) {
    logger.fail('Could not determine manager ID')
    await closeDatabase()
    process.exit(1)
  }
  ...
  if (cleanup) {
    ...
    result.agentsCreated.slice(0, 5).forEach((id) => {
      console.log(`  - ${id}`)
    })
