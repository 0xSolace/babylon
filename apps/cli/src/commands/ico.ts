...
function getAddresses(): { token: string; presale: string } {
...
async function getStatus(network: NetworkName): Promise<void> {
  ...
  const [softCap, hardCap, currentPrice] = (await Promise.all([
    client.readContract({
      address: presale as `0x${string}`,
      abi: PRESALE_ABI,
...
async function getStats(network: NetworkName): Promise<void> {
  ...
  // Get all data
  const [status, _contributors, softCap, hardCap, presaleStart, presaleEnd] =
    (await Promise.all([
      client.readContract({
        address: presale as `0x${string}`,
        abi: PRESALE_ABI,
  ...
  const [raised, participants, _progress] = status;
...
async function listContributors(network: NetworkName): Promise<void> {
  ...
  const contributors = (await client.readContract({
    address: presale as `0x${string}`,
    abi: PRESALE_ABI,
    functionName: 'getContributors',
  ...
  for (const addr of contributors.slice(0, 50)) {
    ...
    const [ethAmount, tokenAllocation] = contribution;
    console.log(
...
`);
  ...
  const deployerKey = `0x${randomBytes(32).toString('hex')}`;
