...
import { ElizaHolderAirdropService, getAirdropBonusService } from '@babylon/api'
import {
...
tokenCommand
  ...
  .action(
    async (options: {
      ...
      console.log(`Total ELIZA supply: ${formatUnits(totalSupply, 18)} ELIZA\n`)

      if (options.single) {
        // Snapshot single wallet
        const evmAddress = toAddress(options.single)
      ...
      if (options.wallets) {
        ...
        const rawData = JSON.parse(readFileSync(options.wallets, 'utf-8'))
        ...
        const result = await service.runFullSnapshot(
          walletData.map((w) => ({
            evmAddress: toAddress(w.evmAddress),
