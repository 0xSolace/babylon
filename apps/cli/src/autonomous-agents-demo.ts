...
const agentTemplates: Omit<AgentConfig, 'a2aEndpoint'>[] = [
  {
    ...
    personality:
      'Aggressive momentum trader who looks for rapidly changing markets and high volume',
  ...
  },
  {
    ...
    personality:
      'Conservative fundamental analyst who thoroughly researches questions',
  ...
  },
  {
    ...
    personality:
      'Contrarian who looks for mispriced markets and crowd psychology errors',
  ...
  },
  {
    ...
    personality:
      'News-driven trader who reacts quickly to events and information',
  ...
  },
  {
    name: 'Eve (Quant)',
    personality:
      'Quantitative analyst using statistical models and data analysis',
...
async function main() {
  ...
  logger.info(
    `Connecting ${options.count} agents to ${options.endpoint}`,
    undefined,
    'CLI'
  );
  ...
  for (let i = 0; i < options.count!; i++) {
    ...
    agent.on(
      ...
      (data: {
        questions: Question[];
        priceUpdates?: Array<Record<string, JsonValue>>;
        timestamp?: number;
      }) => {
        logger.info(
          `${config.name} received market update`,
          { activeQuestions: data.questions.length },
          'CLI'
        );
      }
    );
    ...
    agent.on('analysisComplete', (analysis: AgentAnalysisResult) => {
      logger.info(
        `${config.name} completed analysis`,
        {
    ...
    agent.on('coalitionJoined', (invite: CoalitionProposal) => {
      logger.info(
        `${config.name} joined coalition: ${invite.name || invite.coalitionId}`,
        undefined,
        'CLI'
      );
    });
  ...
  setTimeout(async () => {
    ...
    if (agents.length >= 2) {
      ...
      logger.info(
        `${leaderStatus.name} searching for momentum traders...`,
        undefined,
        'CLI'
      );
      const partners = registry.findByStrategy('momentum');
      logger.info(
        `Found ${partners.length} momentum traders:`,
        partners.map((p) => ({
  ...
  // Keep process alive
  logger.info(
    'Agents are now running. Press Ctrl+C to stop.',
    undefined,
    'CLI'
  );
