...
function generateAbiMarkdown(contract: ContractABI): string {
  ...
  const contractConstructor = contract.abi.find(
  ...
  if (contractConstructor) {
    md += '## Constructor\n\n'
    md += '```solidity\n'
    md += `constructor(${contractConstructor.inputs?.map((i) => `${i.type} ${i.name}`).join(', ') || ''})\n`
    md += '```\n\n'
  }
...
async function main() {
  ...
  await fs.writeFile(
    jsonPath,
    JSON.stringify(
      contracts.reduce<Record<string, ABIItem[]>>((acc, c) => {
        acc[c.name] = c.abi
        return acc
      }, {}),
      null,
