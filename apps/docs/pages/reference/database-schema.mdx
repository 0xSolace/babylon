...
Babylon uses **CovenantSQL (CQL)** for type-safe database access.
...
export const users = pgTable('User', {
  ...
  oauth3Id: varchar('oauth3Id', { length: 191 }).unique(),
  privyId: varchar('privyId', { length: 191 }).unique(), // @deprecated - kept for migration
...
**Key Fields**:
- `oauth3Id`: OAuth3 (Jeju) authentication identifier - the primary auth method
- `privyId`: **@deprecated** - Legacy Privy auth identifier, retained for backward compatibility during migration. New users authenticate via OAuth3.
...
> **Migration Note**: Authentication has migrated from Privy to OAuth3 (Jeju). The `privyId` field is deprecated and will be removed in a future release. All new user authentication uses `oauth3Id`.
...
# Migration files created in cql/migrations/
```
...
```typescript
// CQL manages connections via connection pool
...
# Or via CQL schema export
bun run db:schema:export
