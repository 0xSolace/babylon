...
export function AirdropCard() {
  ...
  useEffect(() => {
    ...
    const nextDripTime = drip?.nextDripTime
    if (!nextDripTime || drip.canDripToday) {
      setCountdown('')
      return
    }

    const updateCountdown = () => {
      const nextTime = new Date(nextDripTime).getTime()
      const now = Date.now()
  ...
  if (!status.drip || !status.claim || !status.allocation) {
    return null
  }
