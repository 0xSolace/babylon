import { logger, PerpHistoryApiResponseSchema } from '@babylon/shared';
 ...
 * ```
 */
/** Raw API history point structure */
...
export function usePerpHistory(
  ...
  const formatHistory = useCallback(
    (points: RawHistoryPoint[]): PerpHistoryPoint[] => {
      const parsed = points
        .map((point) => {
  ...
  const fallbackFromSeed = useCallback((): PerpHistoryPoint[] => {
  ...
  const {
    ...
    queryKey: ['perpHistory', ticker, limit],
    queryFn: async (): Promise<{
      history: PerpHistoryPoint[];
      error?: string;
    }> => {
      const response = await fetch(
        `/api/markets/perps/${encodeURIComponent(ticker as string)}/history?limit=${limit}`
      );

      const json: unknown = await response.json();
      const parsed = PerpHistoryApiResponseSchema.safeParse(json);

      if (!parsed.success) {
        return {
          history: fallbackFromSeed(),
          error: 'Invalid response format from perp history API',
        };
      }

      const historyArray = parsed.data.history;
      ...
      if (
        ...
        return { history: formatHistory(historyArray as RawHistoryPoint[]) };
      }

      if (!response.ok) {
        const message =
          parsed.data.error ?? `Failed to fetch history: ${response.status}`;
        return { history: fallbackFromSeed(), error: message };
      }

      return { history: fallbackFromSeed() };
    },
    enabled: !!ticker,
    staleTime: 30_000,
  });
  ...
  useEffect(() => {
    ...
    if (queryData?.history) {
      ...
      if (queryData.history.length > 0) {
        lastAppendedPriceRef.current =
          queryData.history[queryData.history.length - 1]!.price;
      }
    }
  }, [ticker, queryData]);
  ...
  // Log errors
  useEffect(() => {
    if (queryError && ticker) {
      logger.error(
        'Failed to fetch perp history',
        { ticker, error: queryError },
        'usePerpHistory'
      );
    }
  }, [queryError, ticker]);
