  ...
  return runWithOptionalUserRls(authUser, async (db) => {
    ...
    if (following && userId && authUser && authUser.userId === userId) {
      ...
      const allFollowedIds = await getCacheOrFetch(
        followsCacheKey,
        async () => {
          const [followedUserIds, followedActorIds] = await Promise.all([
            selectFollowingIdsByFollowerId(db, userId),
            selectFollowedActorIdsByUserId(db, userId),
          ]);
      ...
      const usersList =
        authorIds.length > 0
          ? await selectUsersCommentAuthorSlicesByIds(db, authorIds)
          : [];
      ...
      const [reactionCounts, commentCounts] = await Promise.all([
        countPostLikesGroupedByPostIds(db, postIds),
        countCommentsGroupedByPostIds(db, postIds),
      ]);

      // Create maps for quick lookup
      const reactionMap = new Map(
        reactionCounts.flatMap((r) =>
          r.postId ? ([[r.postId, r.count]] as const) : []
        )
      );
      const commentMap = new Map(
        commentCounts.flatMap((c) =>
      ...
      if (followingRepostIds.length > 0) {
        const originals = await listPostsByIdsWhereNotDeleted(
          db,
          followingRepostIds
        );
      ...
      const formattedFollowingPosts = filteredPosts.map((post: Post) => {
        ...
        if (
          postsWithOriginal.originalPostId &&
          postsWithOriginal.originalPost
        ) {
    ...
    if (type) {
      ...
      postsResult = await selectPostsByTypePaginated(db, {
        postType: type,
        limit,
        now,
        cursorTimestamp: cursor ? new Date(cursor) : undefined,
      });
    ...
    if (originalPostIds.length > 0) {
      const originalPostsList = await listPostsByIdsWhereNotDeleted(
        db,
        originalPostIds
      );
    ...
    const authorIds = [
      ...new Set([...postAuthorIds, ...originalPostAuthorIds]),
    ];
    ...
    } = await fetchPostFeedMetadataConsolidated(db, allPostIds);
    ...
    for (const [postId, rawPreviews] of rawCommentPreviewMap) {
      ...
      if (postCommentCount >= ENGAGEMENT_THRESHOLDS.COMMENTS_HIGH) {
        ...
        previewLimit = 1;
        showPreview =
          engagementBucket < ENGAGEMENT_THRESHOLDS.VISIBILITY_MEDIUM;
      } else if (
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const { post, mentionedUsers } = await asUser(canonicalUserId, async (tx) => {
    const created = await insertUserAuthoredPostReturning(tx, {
      id: postId,
    ...
    if (!created) {
      return {
        ...
        mentionedUsers: [] as { id: string; username: string | null }[],
      };
    }

    const mentioned =
      usernames.length > 0
        ? await selectUsersIdUsernameByUsernames(tx, usernames as string[])
        : [];

    return { post: created, mentionedUsers: mentioned };
  });
