...
export const GET = withErrorHandling(
  async (
    ...
    return runWithOptionalUserRls(user, async (db) => {
      ...
      const post = await selectPostByIdWhereTimestampLte(db, postId, now);
      ...
      if (!post) {
        ...
        if (gamePost) {
          ...
          const userRecord = actor
            ? undefined
            : await selectUserDisplayNameUsernameById(db, gamePost.authorId);
          ...
          if (userId) {
            const [likedRow, sharedRow] = await Promise.all([
              ...
              selectShareIdForUserAndPost(db, { userId, postId }),
            ]);
            isLiked = !!likedRow;
            isShared = !!sharedRow;
          }
          ...
          if (originalPostIdFromGame) {
            const originalPost = await selectPostAuthorIdContentById(
              db,
              originalPostIdFromGame
            );
            ...
            if (originalPost) {
              ...
              const originalAuthor =
                originalUser || originalActor || originalOrg;
          ...
          if (parts.length >= 3 && parts[1]) {
            ...
            if (!isNaN(timestampNum) && timestampNum > 1000000000000) {
              ...
              if (parts.length >= 4 && parts[2] && !parts[2].includes('.')) {
                ...
                const actorRecord =
                  StaticDataRegistry.getActor(potentialActorId);
                if (actorRecord) {
        ...
        const existingPost = await selectPostById(db, postId);

        let createdPost;
        if (!existingPost) {
          // Create the post
          createdPost = await insertGamePostStubReturning(db, {
            id: postId,
        ...
        if (userId) {
          const [likedRow, sharedRow] = await Promise.all([
            selectPostLikeReactionIdForUser(db, { postId, userId }),
            selectShareIdForUserAndPost(db, { userId, postId }),
          ]);
          isLiked = !!likedRow;
          isShared = !!sharedRow;
        ...
        if (actorRecord) {
          ...
          authorProfileImageUrl =
            ...
            `/images/actors/${actorRecord.id}.jpg`;
        } else {
          const userRecord = await selectUserUsernameDisplayProfileById(
            db,
      ...
      if (actor) {
        ...
        if (org) {
          ...
          const usr = await selectUserUsernameDisplayProfileById(
            db,
            post.authorId
          );
      ...
      if (post.originalPostId) {
        const originalPost = await selectPostByIdNotDeleted(
          db,
          post.originalPostId
        );
...
export const DELETE = withErrorHandling(
  async (
    ...
    return asUser(user, async (db) => {
      ...
      const post = await selectPostSoftDeleteSliceById(db, postId);
      ...
      await updatePostDeletedAtById(db, postId, deletedAt);
      ...
      return successResponse({
        ...
        data: { id: postId, deletedAt: deletedAt.toISOString() },
      });
    });
  }
);
