...
/**
 * Transfer trading balance from user to agent (deposit)
 */
async function depositTradingBalance(
  agentId: string,
  userId: string,
  amount: number
): Promise<void> {
  ...
  await db
    .update(users)
    .set({ virtualBalance: String(userBalance - amount) })
    .where(eq(users.id, userId));
  await db
    .update(users)
    .set({
      ...
      totalDeposited: String(
        Number(agentResult[0]?.virtualBalance ?? 0) + amount
      ),
    })
    .where(eq(users.id, agentId));
  ...
  await db.insert(balanceTransactions).values({
    ...
    description: `Deposit from owner ${userId}`,
    relatedId: userId,
    createdAt: new Date(),
  });
}
...
/**
 * Transfer trading balance from agent to user (withdraw)
 */
async function withdrawTradingBalance(
  agentId: string,
  userId: string,
  amount: number
): Promise<void> {
  ...
  await db
    ...
    .set({
      ...
      totalWithdrawn: String(
        Number(agentResult[0]?.totalWithdrawn ?? 0) + amount
      ),
    })
    .where(eq(users.id, agentId));
  ...
  await db.insert(balanceTransactions).values({
    ...
    balanceBefore: String(agentBalance),
    balanceAfter: String(agentBalance - amount),
    description: `Withdrawal to owner ${userId}`,
    relatedId: userId,
    createdAt: new Date(),
  });
}
...
export async function POST(
  ...
  try {
    if (action === 'deposit') {
      await depositTradingBalance(agentId, user.id, amount);
      logger.info(
      ...
      await withdrawTradingBalance(agentId, user.id, amount);
      logger.info(
