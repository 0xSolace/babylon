 ...
 * Stories Feed API
 *
 * @route GET /api/feed/stories — daily-topic-focused narrative feed
 *
 * Returns narrative stories sorted by score, with stories matching today's
 * daily topic boosted to the top. Replaces the "Hot" tab with a coherent,
 * topic-driven experience per BAB-134 (one topic per day) and BAB-199
 * (story-by-story narrative sorting).
 *
 * Story Scoring:
 *   storyScore = calculateStoryScore(...) * arcMultiplier * resolutionBoost * topicBoost
 *   topicBoost = 2.0 when story.topicKey === today's topicKey, else 1.0
 *
 * @openapi
 * /api/feed/stories:
 *   get:
 ...
 *     description: Returns narrative stories for today's daily topic, sorted by narrative score.
 *     responses:
 *       200:
 *         description: Stories feed with today's topic metadata
 */
...
// Stories matching today's topic are surfaced first
...
interface CachedResult {
  ...
  postIds: string[];
  topic: { topicKey: string; topicLabel: string; summary: string } | null;
}

export const GET = withErrorHandling(async (request: NextRequest) => {
  const user = await optionalAuth(request).catch((err) => {
    logger.debug('optionalAuth failed', { error: err }, 'StoriesFeedAPI');
    return null;
  });

  // Resolve today's topic before cache so the cache key is topic-aware
  const todaysTopic = await dailyTopicService.getCurrentTopic().catch((err) => {
    logger.warn('Failed to get daily topic', { error: err }, 'StoriesFeedAPI');
    return null;
  });

  const cacheKey = `feed:stories:v1:${todaysTopic?.topicKey ?? 'none'}`;
  ...
  const result = await getCacheOrFetch<CachedResult>(
    ...
    async () => {
      ...
      if (recentPosts.length === 0) {
        return { stories: [], postIds: [], topic: topicMeta };
      }
      ...
      for (const [storyKey, storyPosts] of storyPostMap) {
        ...
        stories.push({
          ...
          topicLabel:
            storyTopicKey === todaysTopicKey ? todaysTopicLabel : null,
        });
      }
      ...
      return { stories, postIds: allPostIds, topic: topicMeta };
    },
    { namespace: 'feed', ttl: 60 }
  );

  // Per-user like/share enrichment (bypasses cache, same pattern as hot route)
