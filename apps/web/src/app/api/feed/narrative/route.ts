  ...
  const result = await getCacheOrFetch<CachedResult>(
    ...
    async () =>
      runWithOptionalUserRls(null, async (db) => {
        ...
        const recentPosts = await selectNarrativeFeedRecentPostSlices(db, {
          cutoff,
          now,
        ...
        const { reactionMap, commentMap, shareMap } =
          await executeNarrativeFeedEngagementCounts(db, postIds);
        ...
        if (repostOriginalIds.length > 0) {
          ...
          const originalAuthorUsers =
            originalAuthorIds.length > 0
              ? await selectNarrativeFeedAuthorUsersByIds(db, originalAuthorIds)
              : [];
        ...
        for (const [storyKey, storyPosts] of storyPostMap) {
          ...
          const meta =
            questionNumber !== null
              ? questionMetaMap.get(questionNumber)
              : null;
          const storyTitle =
            meta?.title ??
        ...
        if (storyQuestionNumbers.length > 0) {
          ...
          for (const story of stories) {
            if (!story.isNewMarket && story.questionNumber !== null) {
              story.marketId =
                questionToMarket.get(story.questionNumber) ?? null;
            }
          }
        }
        ...
        const newMarketQuestions = await selectNarrativeNewMarketCardRows(db, {
          newMarketCutoff,
        ...
        for (const q of newMarketQuestions) {
          ...
          stories.push({
            ...
            arcState: (q.arcState as ArcStateType | null) ?? null,
            storyScore:
              Math.round(recencyScore * arcMultiplier * 10000) / 10000,
            postCount: 0,
            posts: [],
            hasUserPosition: false,
            isNewMarket: true,
  ...
  if (user?.userId) {
    ...
    try {
      ...
      if (cachedEnrichment) {
        ...
        const [userLikes, userShares, userPositions] =
          await runWithOptionalUserRls(user, async (db) =>
            Promise.all([
              selectNarrativeUserLikedPostIds(db, {
                userId,
                postIds: result.postIds,
              }),
              selectNarrativeUserSharedPostIds(db, {
                userId,
                postIds: result.postIds,
              }),
              selectNarrativeUserActivePositionQuestionIds(db, {
                userId,
                questionNumbers: questionNumbersInResult,
              }),
            ])
          );
