...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  const newsItems: BreakingNewsItem[] = await runWithOptionalUserRls(
    authUser,
    async (db) => {
      ...
      const allEventTypesRaw = await selectWorldEventTypesSample(db, 1000);
      ...
      const recentEvents = await selectRecentPublicWorldEvents(db, {
      ...
      for (const event of significantEvents) {
        ...
        const firstActorId =
          event.actors && event.actors.length > 0 ? event.actors[0] : undefined;
        if (firstActorId) {
      ...
      const priceUpdatesRaw = await selectRecentStockPricesOrdered(
        db,
        FEED_WIDGET_CONFIG.MAX_PRICE_UPDATES_QUERY
      );

      const orgStates = await selectOrganizationStateAll(db);
      ...
      const recentPosts = await selectRecentNonDeletedPosts(db, {
        notAfter: currentTime,
      ...
      if (items.length < FEED_WIDGET_CONFIG.MAX_BREAKING_NEWS_ITEMS) {
        ...
        const fallbackPosts = await selectFallbackActorPostsExcludingIds(db, {
          notAfter: currentTime,
          authorIds: Array.from(actorIds),
      ...
      // Only show events up to current time (prevent future access)
      if (items.length < FEED_WIDGET_CONFIG.MAX_BREAKING_NEWS_ITEMS) {
        const allRecentEvents = await selectRecentWorldEventsUpToTime(db, {
          notAfter: currentTime,
          limit: 10,
        });
