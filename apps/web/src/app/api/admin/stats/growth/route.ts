...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  return await asSystem(async (tx) => {
    ...
    `,
        ...
        tx.$queryRaw<TraderCommanderResult>`
    ...
    `,
        ...
        tx.$queryRaw<TradesPerTraderResult>`
      SELECT 
    ...
    `,
        ...
        tx.$queryRaw<ActionsPerCommanderResult>`
      SELECT 
    ...
    const tradesRow: TradesPerTraderResult | undefined =
      tradesPerTraderResult[0];
    ...
    const tradesPerTrader =
      uniqueTraders > 0
        ? Math.round((totalTrades / uniqueTraders) * 10) / 10
        : 0;
    ...
    if (includeTimeSeries) {
      ...
      const dailyWauRows = await tx.$queryRaw<WAUTimeSeriesRow>`
      WITH date_series AS (
    ...
    const sessionResult = await tx.$queryRaw<SessionMetricsResult>`
    SELECT 
  ...
  `.catch(() => [] as SessionMetricsResult[]);
    ...
    const cohortResult = await tx.$queryRaw<RetentionCohortResult>`
    WITH cohorts AS (
      SELECT 
