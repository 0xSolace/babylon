...
export const PATCH = withErrorHandling(
  async (
    ...
    // Get counts
    const [likeCountResults, replyCountResults] = await Promise.all([
      db
        .select({ count: count() })
        .from(reactions)
        .where(
    ...
    const replyCountResult = (
      replyCountResults as unknown as Array<{ count: number }>
    )[0];
...
export const DELETE = withErrorHandling(
  async (
    ...
    const replyCountResults = await db
      .select({ count: count() })
      .from(comments)
      .where(eq(comments.parentCommentId, commentId));

    // Type assertion for aggregate query result
    const replyCountResultDel = (
      replyCountResults as unknown as Array<{ count: number }>
    )[0];

    if (!replyCountResultDel) {
      throw new Error('Failed to get reply count');
    }

    const repliesCount = Number(replyCountResultDel.count);
