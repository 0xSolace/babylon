...
const prismaAvailable = !!prisma?.user;
if (!prismaAvailable) {
...
describeWaitlist('WaitlistService', () => {
  ...
  beforeEach(() => {
    ...
    prisma.user.findUnique = mock(
      async ({
        ...
        if (where.id) {
          ...
          user =
            Object.values(createdUsers).find((u) => u.referralCode === where.referralCode) || null;
        }
    ...
    prisma.user.count = mock(async ({ where }: { where?: Record<string, unknown> }) => {
      ...
      if (where?.OR) {
        ...
        users = users.filter((u) => {
          ...
          if (
            where.isWaitlistActive !== undefined &&
            u.isWaitlistActive !== where.isWaitlistActive
          ) {
            return false;
          }
          ...
          for (const condition of orConditions) {
            const invitePointsCondition = condition.invitePoints as
              | { gt?: number }
              | number
              | undefined;
            const waitlistJoinedAtCondition = condition.waitlistJoinedAt as
              | { lt?: Date }
              | undefined;

            if (
              invitePointsCondition &&
              typeof invitePointsCondition === 'object' &&
              'gt' in invitePointsCondition &&
              invitePointsCondition.gt !== undefined
            ) {
            ...
            if (typeof invitePointsCondition === 'number' && waitlistJoinedAtCondition?.lt) {
              if (
                u.invitePoints === invitePointsCondition &&
                u.waitlistJoinedAt &&
                u.waitlistJoinedAt < waitlistJoinedAtCondition.lt
              ) {
                return true;
              }
            }
    ...
    if (prisma.pointsTransaction) {
      // @ts-expect-error - Mocking Prisma methods for testing
      prisma.pointsTransaction.create = mock(
  ...
  describe('getWaitlistPosition', () => {
    it('should calculate dynamic leaderboard rank based on invite points', async () => {
      ...
      if (positionA?.leaderboardRank === undefined || positionB?.leaderboardRank === undefined) {
        throw new Error('Expected leaderboard rankings for both users.');
      }
    ...
    it('should handle tie-breaking by signup date', async () => {
      ...
      if (position1?.leaderboardRank === undefined || position2?.leaderboardRank === undefined) {
        throw new Error('Expected leaderboard rankings for both tied users.');
      }
