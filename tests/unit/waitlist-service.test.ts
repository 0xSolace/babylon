...
describe('WaitlistService', () => {
  ...
  describe('getWaitlistPosition', () => {
    it('should calculate dynamic leaderboard rank based on invite points', async () => {
      ...
      console.log(
        '✅ Dynamic ranking works: User with more invites ranks higher!'
      );
    });
  ...
  describe('bonuses', () => {
    ...
    it('should award wallet bonus only once', async () => {
      ...
      // Award first time
      const awarded1 = await WaitlistService.awardWalletBonus(
        user.id,
        '0x1234'
      );
      expect(awarded1).toBe(true);
      ...
      // Try to award again
      const awarded2 = await WaitlistService.awardWalletBonus(
        user.id,
        '0x5678'
      );
      expect(awarded2).toBe(false); // Should not award twice
