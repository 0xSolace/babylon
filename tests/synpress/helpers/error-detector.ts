...
export class ButtonValidator {
   ...
   */
  async findAllButtons(): Promise<
    { selector: string; text: string; visible: boolean }[]
  > {
    ...
    const linkElements = await this.page
      .locator('a[role="button"], a.button, a[class*="button"]')
      .all();
  ...
  async testAllButtons(
    options: { skipPatterns?: RegExp[]; screenshotPrefix?: string } = {}
...
export class PageValidator {
  ...
  async validatePageLoad(expectedUrl: string): Promise<void> {
    ...
    const isLoading = await this.page
      .locator('[data-testid="loading"], [data-loading="true"]')
    ...
    if (this.errorDetector.hasErrors()) {
      throw new Error(
        `Page loaded with errors: ${this.errorDetector.formatErrorReport()}`
      );
    }
