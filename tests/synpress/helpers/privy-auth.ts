...
export async function loginWithPrivyEmail(page: Page, account: PrivyTestAccount): Promise<void> {
  ...
  const loginButton = page
    .locator('button:has-text("Log in"), button:has-text("Sign in"), [data-testid="privy-login"]')
    .first();
  ...
  if (isVisible) {
    ...
    if (emailInputVisible) {
      ...
      const continueButton = page
        .locator('button:has-text("Continue"), button:has-text("Log in"), button[type="submit"]')
        .first();
      ...
      if (account.password) {
        ...
        if (passwordVisible) {
          ...
          const submitButton = page
            .locator('button:has-text("Log in"), button[type="submit"]')
            .first();
      ...
      if (account.otp) {
        const otpInput = page
          .locator('input[type="text"][maxlength="6"], input[name="otp"]')
          .first();
