...
const _waitForPostHogInit = async (page: Page) => {
  // Wait for PostHog to initialize
  await page.waitForFunction(
    () => {
      return (
        typeof window !== 'undefined' &&
        (window as Window & { posthog?: { __loaded?: boolean } }).posthog
          ?.__loaded === true
      );
    },
    { timeout: 10000 }
...
const _getPostHogEvents = async (page: Page): Promise<string[]> => {
  // Get captured events from PostHog instance
  return await page.evaluate(() => {
    const ph = (
      window as Window & { posthog?: { _events?: Array<{ event: string }> } }
    ).posthog;
...
const mockPostHogCapture = async (page: Page) => {
  ...
  await page.addInitScript(() => {
    ...
    const originalPostHog = (
      window as Window & { posthog?: typeof import('posthog-js').default }
    ).posthog;
    if (originalPostHog) {
      const originalCapture = originalPostHog.capture;
      originalPostHog.capture = function (
        event: string,
        properties?: Record<string, unknown>
      ) {
        (
        ...
        ).__postHogEvents?.push({ event, properties });
...
test.describe('PostHog Analytics Integration', () => {
  ...
  test('should track page views on navigation', async () => {
    ...
    const _pageviewEvent = events.find((e) => e.event === '$pageview');
  ...
  test('should identify user when authenticated', async () => {
    ...
    const hasPostHog = await page.evaluate(() => {
      return (
        typeof (window as Window & { posthog?: unknown }).posthog !==
        'undefined'
      );
    });
  ...
  test('should have PostHog configured with proper privacy settings', async () => {
    ...
    const privacySettings = await page.evaluate(() => {
      const ph = (
        window as Window & {
          posthog?: {
            config?: { respect_dnt?: boolean; capture_exceptions?: boolean };
          };
        }
      ).posthog;
