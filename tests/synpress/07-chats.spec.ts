...
test.describe('Chats Page - Updated Design', () => {
  ...
  test('should load chats page with new design', async ({ page }) => {
    ...
    const hasMessagesHeader = await page
      .getByText('Messages')
  ...
  test('should display All/DMs/Groups filter tabs', async ({ page }) => {
    ...
    const hasAllTab = await page
      ...
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const hasDMsTab = await page
      .getByText('DMs')
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const hasGroupsTab = await page
      .getByText('Groups')
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    ...
    console.log(
      `✅ Filter tabs visible: All=${hasAllTab}, DMs=${hasDMsTab}, Groups=${hasGroupsTab}`
    );
  });
...
test.describe('Chat Messaging - New Implementation', () => {
  ...
  test('should display SSE connection status', async ({ page }) => {
    ...
    const hasSSEIndicator = await page
      .getByText(/Live|Connecting/i)
      .isVisible({ timeout: 5000 })
      .catch(() => false);
...
test.describe('Profile Message Button', () => {
  ...
  test('should navigate to DM when clicking message button', async ({
    ...
    const messageButton = page
      .locator('button[title*="message" i], button:has-text("Message")')
      .first();
