...
test.describe('Chat and Social Features E2E', () => {
  test.beforeAll(async () => {
    ...
    if (existingUsers.length >= 2) {
      testUser1 = existingUsers[0] as typeof testUser1;
      testUser2 = existingUsers[1] as typeof testUser2;
      ...
      console.log(
        `   User 1: ${testUser1.displayName} (@${testUser1.username})`
      );
      console.log(
        `   User 2: ${testUser2.displayName} (@${testUser2.username})`
      );
  ...
  test('1. User can follow another user and see counts update', async ({
    page,
  }) => {
    if (!testUser2) {
      test.skip();
...
test.describe('API Integration Tests', () => {
  test('5. Follow API endpoints work correctly', async () => {
    ...
    console.log(
      `✅ API Test: User 1 has ${followerCount} followers, ${followingCount} following`
    );
  });
  ...
  test('6. DM chat exists and has messages', async () => {
    ...
    if (chat) {
      ...
      console.log(
        `✅ API Test: DM chat has ${chat.messages.length} messages, ${chat.participants.length} participants`
      );
    } else {
      console.log(
        'ℹ️  DM chat not created yet (will be created on first message)'
      );
    }
  });
  ...
  test('7. Group chats have correct participant structure', async () => {
    ...
    if (groupChats.length > 0) {
      ...
      console.log(
        `✅ API Test: Test group has ${testGroup.participants.length} participants, ${testGroup.messages.length} messages`
      );
    }
  });
});
