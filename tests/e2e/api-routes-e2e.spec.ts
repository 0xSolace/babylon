...
test.describe('API Validation - E2E', () => {
  test('POST /api/onboarding/check-username - validates username', async ({
    request,
  }) => {
    const response = await request.post(
      `${BASE_URL}/api/onboarding/check-username`,
      {
  ...
  test('GET /api/onboarding/random-assets - returns random assets', async ({
    request,
  }) => {
    const response = await request.get(
      `${BASE_URL}/api/onboarding/random-assets`
    );
