  ...
  await page.addInitScript(() => {
    ...
    console.log = function (...args: unknown[]) {
      if (typeof args[0] === 'string' && args[0].includes('PostHog')) {
        (window as Window & { __postHogLogs?: string[] }).__postHogLogs =
          (window as Window & { __postHogLogs?: string[] }).__postHogLogs || [];
        (window as Window & { __postHogLogs?: string[] }).__postHogLogs?.push(
          args[0] as string
        );
      }
...
const _getTrackedEvents = async (page: Page): Promise<string[]> => {
  return await page.evaluate(() => {
    return (
      (
        window as Window & { __mockPostHogEvents?: Array<{ event: string }> }
      ).__mockPostHogEvents?.map((e) => e.event) || []
    );
  });
};
...
test.describe('PostHog Client-Side Tracking', () => {
  ...
  test('should handle PostHog initialization failure gracefully', async ({
    page,
  }) => {
...
test.describe('PostHog API Tracking', () => {
  ...
  test('should handle tracking failures without affecting API responses', async ({
    request,
  }) => {
