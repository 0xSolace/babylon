...
const LOCAL_DATABASE_URL =
  'postgresql://babylon:babylon_dev_password@localhost:5432/babylon';
...
describe('Followers/Following and Relationships Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    if (!user) {
      ...
      testUser = user;
      console.log(
        `✅ Found test user: ${testUser.displayName || testUser.username}`
      );
    }
    ...
    if (actors.length < 2) {
      throw new Error(
        `Not enough actors found in database for testing. Found ${actors.length}, need at least 2. Total in DB: ${totalActors}`
      );
    }
    ...
    console.log(
      `✅ Using test actors: ${testActor.name} and ${secondTestActor.name}`
    );
  });
  ...
  describe('NPC Relationship Seeding', () => {
    ...
    it('should have ActorRelationship metadata in database (optional)', async () => {
      ...
      if (relationshipCount > 0) {
        console.log(
          `✅ Found ${relationshipCount} NPC relationships with metadata`
        );
      } else {
        console.log(
          `ℹ️  No ActorRelationship metadata found (this is optional)`
        );
      }
    ...
    it('should have actors with followers', async () => {
      ...
      actorsWithFollowers.forEach((actor) => {
        console.log(
          `   - ${actor.name}: ${actor._count.followedBy} NPC followers`
        );
      });
    });
    ...
    it('should have actors with following', async () => {
      ...
      actorsWithFollowing.forEach((actor) => {
        console.log(
          `   - ${actor.name}: following ${actor._count.following} NPCs`
        );
      });
    });
    ...
    it('should not have actors with zero followers AND zero following', async () => {
      ...
      if (isolatedActors.length > 0) {
        console.warn(
          `⚠️  Found ${isolatedActors.length} isolated actors (no followers or following):`
        );
  ...
  describe('Actor Stats Accuracy', () => {
    it('should correctly count actor followers (NPC + User)', async () => {
      ...
      const totalFollowers =
        actorFollowerCount + userFollowerCount + legacyFollowerCount;
    ...
    it('should have consistent ActorFollow bidirectional relationships', async () => {
      ...
      if (sampleFollow) {
        ...
        console.log(
          `✅ Bidirectional relationship verified: ${sampleFollow.follower.name} → ${sampleFollow.following.name}`
        );
      }
    });
  });
  ...
  describe('User Following Stats Accuracy', () => {
    it('should correctly count user following (Users + Actors)', async () => {
      ...
      console.log(
        `✅ ${testUser.displayName || testUser.username} is following:`
      );
    ...
    it('should correctly count user followers', async () => {
      ...
      console.log(
        `✅ ${testUser.displayName || testUser.username} has ${followerCount} user followers`
      );
  ...
  describe('Follow/Unfollow Operations', () => {
    ...
    it('should allow user to follow an actor', async () => {
      ...
      if (!existingFollow) {
        ...
        console.log(
          `✅ User ${testUser.displayName || testUser.username} followed ${testActor.name}`
        );
      } else {
    ...
    it('should reflect follow in actor stats', async () => {
      ...
      console.log(
        `✅ ${testActor.name} now has ${userFollowerCount} user followers`
      );
    });
  ...
  describe('Stats API Consistency', () => {
    it('should return consistent stats for actors', async () => {
      ...
      console.log(
        `   - Followers: ${dbActorFollowers + dbUserFollowers} (${dbActorFollowers} NPCs + ${dbUserFollowers} users)`
      );
    ...
    it('should return consistent stats for users via _count', async () => {
      ...
      if (user) {
        const totalFollowing =
          user._count.following + user._count.userActorFollows;

        console.log(
          `✅ ${testUser.displayName || testUser.username} stats from _count:`
        );
        console.log(`   - Followers: ${user._count.followedBy}`);
        console.log(
          `   - Following: ${totalFollowing} (${user._count.following} users + ${user._count.userActorFollows} NPCs)`
        );
  ...
  describe('Relationship Integrity', () => {
    ...
    it('should not have self-follows in ActorFollow', async () => {
      const _selfFollows = await prisma.actorFollow.count({
        where: {
          followerId: {
  ...
  describe('Post Count Consistency', () => {
    ...
    it('should count posts correctly for users', async () => {
      ...
      console.log(
        `✅ ${testUser.displayName || testUser.username} has ${postCount} posts`
      );
      expect(postCount).toBeGreaterThanOrEqual(0);
    });
  });
