...
async function testSolanaToEvm() {
  ...
  if (!ataInfo) {
    throw new Error(
      `No BBLN token account found for ${solanaKeypair.publicKey.toBase58()}`
    );
  }

  const tokenAccount = await getAccount(connection, ata);
  console.log(
    `   BBLN Balance: ${(Number(tokenAccount.amount) / 1e9).toFixed(2)} BBLN`
  );

  if (tokenAccount.amount < TEST_AMOUNT) {
    throw new Error(
      `Insufficient BBLN balance. Have ${tokenAccount.amount}, need ${TEST_AMOUNT}`
    );
  }
  ...
  if (!warpRouteAddress) {
    ...
    console.log(
      '   To complete this test, deploy a Hyperlane warp route on Solana'
    );
...
async function testEvmToSolana() {
  ...
  if (!warpRouteAddress) {
    ...
    console.log(
      '   To complete this test, deploy a Hyperlane warp route on EVM'
    );
...
async function verifyConfiguration() {
  ...
  const mailboxCode = await publicClient.getCode({
    address: EVM_CONFIG.mailbox,
  });
  ...
  console.log(
    `   SOLANA_MINT: ${process.env.SOLANA_MINT ?? '❌ Not set (deploy first)'}`
  );
  console.log(
    `   EVM_TOKEN: ${process.env.EVM_TOKEN ?? '❌ Not set (using default testnet)'}`
  );
  console.log(
    `   WARP_ROUTE_SOLANA: ${process.env.WARP_ROUTE_SOLANA ?? '❌ Not set (deploy Hyperlane)'}`
  );
  console.log(
    `   WARP_ROUTE_EVM: ${process.env.WARP_ROUTE_EVM ?? '❌ Not set (deploy Hyperlane)'}`
  );
}
