...
async function main() {
  ...
  try {
    const result = await deployContract(jejuPublic, jejuWallet, 'Presale', [
      tokenAddress,
  ...
  console.log(
    `  cast call ${tokenAddress} "totalSupply()" --rpc-url ${JEJU_RPC}`
  );
  console.log(
    `  cast call ${tokenAddress} "balanceOf(address)" ${account.address} --rpc-url ${JEJU_RPC}`
  );
}
...
function printResults(results: TestResult[], startTime: number) {
  ...
  for (const result of results) {
    ...
    console.log(
      `${color}${status}\x1b[0m ${result.step} (${result.duration}ms)`
    );
    if (result.details) {
