...
function hashLeaf(address: Address, allocation: bigint, bonus: number): Hex {
  return keccak256(
    encodePacked(['address', 'uint256', 'uint8'], [address, allocation, bonus])
  );
}
...
async function calculateAllocations(): Promise<UserAllocation[]> {
  ...
  for (const user of eligibleUsers) {
    ...
    const tradingAllocation = BigInt(
      Math.floor(
        Number(airdropPoolWei) * (tradingVolume / 1_000_000_000) * 0.25
      )
    );
    ...
    // Total before bonus
    let totalAllocation =
      baseAllocation + tradingAllocation + referralAllocation;
