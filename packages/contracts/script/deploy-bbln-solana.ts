...
async function deploy(
  ...
  if (privateKeyEnv) {
    ...
    console.log(
      `   Private Key (save this!): ${bs58.encode(deployer.secretKey)}`
    );
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          if (attempt < 3) {
            ...
            console.log(
              `   Please fund ${deployer.publicKey.toBase58()} manually`
            );
            console.log(`   Use: https://faucet.solana.com/`);
            throw new Error(
              'Devnet airdrop unavailable. Please fund wallet manually.'
            );
          }
        }
      }
  ...
  if (dryRun) {
    ...
    console.log(
      `Initial Liquidity: ${(
        Number(TOKEN_CONFIG.initialLiquidity) / 10 ** TOKEN_CONFIG.decimals
      ).toLocaleString()} BBLN`
    );
