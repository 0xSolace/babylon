...
contract LvrMarket is ReentrancyGuard {
    ...
    IBabylonPredictionOracleAdapter public immutable duelOracle;
    bytes32 public immutable duelKey;
    uint256 public immutable bondValue;
    uint256 public immutable mathScale;
    ...
    constructor(
        ...
        duelKey = duelKey_;
        duelOracle = IBabylonPredictionOracleAdapter(oracle_);
        ...
        require(collateralDecimals <= 18, "invalid decimals");
        mathScale = 10 ** (18 - collateralDecimals);
        bondValue = 50 * (10 ** collateralDecimals);
    ...
    function settleFromOracle() external isRouter nonReentrant {
        ...
        IBabylonPredictionOracleAdapter.DuelState memory duel = duelOracle.getDuel(duelKey);
        require(
            duel.status == IBabylonPredictionOracleAdapter.DuelStatus.RESOLVED
                || duel.status == IBabylonPredictionOracleAdapter.DuelStatus.CANCELLED,
            "Oracle duel not resolved"
        );

        if (duel.status == IBabylonPredictionOracleAdapter.DuelStatus.CANCELLED) {
            outcome = 2; // sentinel: no winner, both tokens redeemable 1:1
        } else {
            require(
                duel.winner == IBabylonPredictionOracleAdapter.Side.A
                    || duel.winner == IBabylonPredictionOracleAdapter.Side.B,
                "Invalid winner"
            );
            outcome = duel.winner == IBabylonPredictionOracleAdapter.Side.A ? 0 : 1;
        }
    ...
    function initializeLiquidity(uint256 collateralIn) external isRouter nonReentrant returns(uint256){
        ...
        yesToken = new YesToken(address(this), routerAddress, collateralIn);
        noToken = new NoToken(address(this), routerAddress, collateralIn);
        liquidity = Math.calcInitialLiquidity(collateralIn * mathScale);
        liquidityInitialized = true;

        emit MarketInitialized(liquidity / mathScale, collateralIn, block.timestamp);
        return liquidity / mathScale;
    }
    ...
    function _swap(bool yesToNo, int256 amountIn) internal view returns(uint256){
        ...
        int256 currentReserveYes = (IERC20(address(yesToken)).balanceOf(address(this)) * mathScale).toInt256();
        int256 currentReserveNo = (IERC20(address(noToken)).balanceOf(address(this)) * mathScale).toInt256();
        uint256 amountOutScaled = SwapMath.getSwapAmount(
            yesToNo,
            currentReserveYes,
            currentReserveNo,
            liq,
            amountIn * int256(mathScale)
        );
        return amountOutScaled / mathScale;
    }
    ...
    function previewBuy(bool isBuyYes, uint256 amountIn) external view returns (uint256) {
        return _previewBuy(isBuyYes, amountIn);
    }

    function previewSwitch(bool isSellYes, uint256 amountIn) external view returns (uint256) {
        return _previewSwitch(isSellYes, amountIn);
    }
    ...
    function getPriceYes() public view returns(uint256) {
        ...
        uint256 liq = isDynamic ? Math.calcLiquidity(liquidity, deadline, block.timestamp) : liquidity;
        return Math.calcPrice(
            yesToken.balanceOf(address(this)) * mathScale,
            noToken.balanceOf(address(this)) * mathScale,
            liq
        );
    }
    ...
    function _previewBuy(bool isBuyYes, uint256 amountIn) internal view returns (uint256) {
        uint256 feeAmount = (amountIn * feeBps) / 10000;
        uint256 amountInAfterFee = amountIn - feeAmount;
        uint256 amountOut = _swap(!isBuyYes, amountInAfterFee.toInt256());
    ...
    function _previewSwitch(bool isSellYes, uint256 amountIn) internal view returns (uint256) {
        uint256 feeAmount = (amountIn * feeBps) / 10000;
        uint256 amountInAfterFee = amountIn - feeAmount;
        return _swap(isSellYes, amountInAfterFee.toInt256());
    }
}
