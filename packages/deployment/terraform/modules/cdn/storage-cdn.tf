# Storage CDN Module - CloudFront for Decentralized Storage Gateway
# Provides fast, cached access to IPFS/storage content with JNS resolution

variable "enable_storage_cdn" {
  description = "Enable dedicated storage CDN gateway"
  type        = bool
  default     = true
}

variable "ipfs_gateway_origin" {
  description = "IPFS gateway origin URL"
  type        = string
...
resource "aws_s3_bucket" "storage_cache" {
  ...
  tags = merge(
    var.tags,
    {
      Name        = "babylon-${var.environment}-storage-cache"
...
resource "aws_s3_bucket_lifecycle_configuration" "storage_cache" {
  ...
  rule {
    ...
    # No expiration - pinned content stays forever
  }
}

# ============================================================================
# Storage CDN CloudFront Distribution
# ============================================================================
...
resource "aws_cloudfront_distribution" "storage_cdn" {
  ...
  comment         = "Babylon Storage CDN Gateway - ${var.environment}"
  ...
  # Primary origin: IPFS Gateway
  origin {
    domain_name = var.ipfs_gateway_origin
    origin_id   = "ipfs-gateway"
    ...
    custom_header {
      name  = "X-CDN-Origin"
      value = "babylon-storage-cloudfront"
    }
  }
  ...
  # Default: Serve from IPFS gateway
  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "ipfs-gateway"
  ...
  # IPFS content by CID - cache immutably (content-addressed = immutable)
  ordered_cache_behavior {
    ...
    response_headers_policy_id = aws_cloudfront_response_headers_policy.storage_immutable[0].id
  }

  # IPNS names - shorter cache (can be updated)
  ordered_cache_behavior {
    path_pattern     = "/ipns/*"
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
  ...
  tags = merge(
    var.tags,
    {
      Name        = "babylon-${var.environment}-storage-cdn"
      Environment = var.environment
      Service     = "storage-cdn"
      Project     = "babylon"
    }
...
# ============================================================================
# Response Headers Policy for Storage
# ============================================================================

resource "aws_cloudfront_response_headers_policy" "storage_immutable" {
  count = var.enable_storage_cdn ? 1 : 0

  name    = "babylon-${var.environment}-storage-immutable"
  comment = "Headers for immutable storage content (IPFS CIDs)"
