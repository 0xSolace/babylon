...
export class MarketOutcomesTracker {
  ...
  async syncWindowOutcomes(windowId: string): Promise<void> {
    ...
    // Use perp snapshots for stock outcomes
    const stockOutcomes: WindowOutcomes['stocks'] = perpSnapshots.map(
      (snap) => ({
        ticker: snap.ticker,
    ...
    for (const market of resolvedMarkets) {
      const outcomeStr = market.resolution === true ? 'YES' : 'NO';
      ...
      if (!existingOutcome) {
        await db.marketOutcome.create({
          data: {
            ...
            changePercent: String(stock.changePercent.toFixed(2)),
