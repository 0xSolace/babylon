...
import { CHAIN, keccak256, logger, stringToHex } from '@babylon/shared'
import {
  type Address,
  createPublicClient,
  createWalletClient,
...
export class BabylonTreasuryClient {
  ...
  async isActiveOperator(): Promise<boolean> {
    const operator = await this.publicClient.readContract({
      address: this.treasuryAddress,
      abi: BABYLON_TREASURY_ABI,
      functionName: 'operator',
    })
    return operator.toLowerCase() === this.signerAddress.toLowerCase()
  }
  ...
  async recordTraining(
    ...
    if (!(await this.isActiveOperator())) {
      logger.warn('[Treasury] Not active operator, skipping on-chain recording')
      return {
        epoch: 0,
        datasetCID,
        modelHash: '',
    ...
    const txHash = await this.walletClient.writeContract({
      ...
      args: [datasetCID, modelHash as Hex],
      account: this.account,
      chain: null,
    })
  ...
  async updateState(stateCID: string): Promise<string> {
    ...
    const txHash = await this.walletClient.writeContract({
      ...
      args: [stateCID, stateHash as Hex],
      account: this.account,
      chain: null,
    })
    await this.publicClient.waitForTransactionReceipt({ hash: txHash })
  ...
  async getGameState(): Promise<{
    ...
    const [cid, hash, version, keyVer, lastBeat, active] = result
    return {
      stateCID: cid,
      stateHash: hash,
