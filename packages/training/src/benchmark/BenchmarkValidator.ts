...
export class BenchmarkValidator {
  ...
  static validate(snapshot: unknown): BenchmarkValidationResult {
    ...
    if (isObject(snap.groundTruth)) {
      const gt = snap.groundTruth

      if (!isObject(gt.marketOutcomes)) {
        errors.push('groundTruth.marketOutcomes must be an object')
      }

      if (!isObject(gt.priceHistory)) {
    ...
    if (isObject(snap.initialState) && isObject(snap.groundTruth)) {
      ...
      const markets = Array.isArray(initialState.predictionMarkets)
        ...
        : []
      const outcomes = isObject(groundTruth.marketOutcomes)
  ...
  static validateOrThrow(
    ...
    const result = BenchmarkValidator.validate(snapshot)
