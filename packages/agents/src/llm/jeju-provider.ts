...
export interface JejuInferenceParams {
  ...
  requireTEE?: boolean;
  /** TEE type preference */
  teeType?: 'intel-tdx' | 'intel-sgx' | 'amd-sev' | 'arm-trustzone';
...
function getJejuConfig(): JejuComputeConfig | null {
  ...
  const rpcUrl =
    process.env.JEJU_RPC_URL || rpcUrls[jejuNetwork] || 'http://127.0.0.1:9545';
  const computeApiUrl =
  ...
  return {
    ...
    preferredPaymentToken:
      (process.env.JEJU_PAYMENT_TOKEN as 'JEJU' | 'ETH' | 'USDC') || 'JEJU',
  };
}
...
export async function callJejuInference(
  ...
  const requestBody = {
    model: params.model || 'llama3-70b', // Default to llama3-70b
    messages,
    temperature: params.temperature ?? 0.7,
    max_tokens: params.maxTokens ?? 2048,
    stream: false,
    // Jeju-specific options
    options: {
...
export async function getJejuComputeStatus(): Promise<{
  ...
  const network =
    process.env.JEJU_NETWORK || process.env.NEXT_PUBLIC_JEJU_NETWORK || null;
