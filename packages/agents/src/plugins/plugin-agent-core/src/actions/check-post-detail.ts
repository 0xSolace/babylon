...
export const checkPostDetailAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      const post = postResult[0] as PostWithAuthor | undefined;
      ...
      const postCommentsRaw = await db
        .select({
          id: comments.id,
      ...
      const postComments =
        postCommentsRaw as unknown as CommentWithAuthorFields[];

      // Transform to our format
      const commentsWithAuthor: CommentWithAuthor[] = postComments.map((c) => ({
        id: c.id,
        content: c.content,
        authorId: c.authorId,
