...
// Track all SQLit operations for verification
const sqlitOperations: {
  type: 'query' | 'execute'
  sql: string
  params: unknown[]
  response: unknown
...
// Mock messages storage (simulates SQLit)
...
describe('End-to-End Messaging Flow', () => {
  ...
  it('should send encrypted message and store in SQLit', async () => {
    ...
    console.log('✅ Message stored in SQLit')
  ...
  it('should query and retrieve messages from SQLit', async () => {
    ...
    console.log('✅ Messages retrieved from SQLit')
  ...
  it('should track all SQLit operations', () => {
    // Verify we have tracked operations
    expect(sqlitOperations.length).toBeGreaterThan(0)

    console.log('\n📊 SQLit Operations Summary:')
    console.log(`   Total operations: ${sqlitOperations.length}`)
    console.log(
      `   Queries: ${sqlitOperations.filter((o) => o.type === 'query').length}`,
    )
    console.log(
      `   Executes: ${sqlitOperations.filter((o) => o.type === 'execute').length}`,
    )

    // Log each operation
    console.log('\n📝 Operation Details:')
    sqlitOperations.forEach((op, i) => {
      const sqlPreview = op.sql.slice(0, 60).replace(/\s+/g, ' ')
      console.log(`   ${i + 1}. [${op.type.toUpperCase()}] ${sqlPreview}...`)
    })
  })

  it('should store messages on-chain (visible in SQLit)', () => {
    // This test verifies that messages are persisted and can be retrieved
    console.log('\n🔗 On-Chain Message Verification:')
    console.log(
      '   Messages are stored in SQLit, which replicates across nodes.',
    )
    console.log(
    ...
    console.log('\n✅ All messages verified and visible on-chain (SQLit)')
  })
})
