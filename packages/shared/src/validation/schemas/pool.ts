...
export const CreatePoolDepositSchema = z.object({
  ...
  amount: z
    ...
    .max(1000000, 'Maximum deposit is $1M'),
});
...
export const PoolDepositBodySchema = z.object({
  ...
  amount: z
    ...
    .max(1000000, { message: 'Maximum deposit is $1M' }),
});
 ...
 */
export const CreatePoolWithdrawalSchema = z
  .object({
    poolId: UUIDSchema,
  ...
  .refine(
    (data) => {
      ...
      const methods = [data.shares, data.percentage, data.amount].filter(
        Boolean
      );
 ...
 */
export const CreateNPCTradeSchema = z
  .object({
