 ...
 */
export function initializeMarket(initialLiquidity = 10_000) {
...
export function calculateBuyWithFees(
  currentYesShares: number,
  currentNoShares: number,
  side: 'yes' | 'no',
  totalAmount: number,
  ...
  const base = calculateBuy(currentYesShares, currentNoShares, side, netAmount)
  return {
    ...base,
    fee,
    netAmount,
...
export function calculateSellWithFees(
  currentYesShares: number,
  currentNoShares: number,
  side: 'yes' | 'no',
  sharesToSell: number,
