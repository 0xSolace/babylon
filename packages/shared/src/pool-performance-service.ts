...
export class PoolPerformanceService {
  ...
  static async updateAllPools(): Promise<void> {
    ...
    logger.info(
      `Updated ${activePools.length} pools`,
      {},
  ...
  private static async updatePositionPnL(
    ...
    if (position.marketType === 'perp' && position.ticker) {
      ...
      if (market) {
        const totalShares =
          parseFloat(market.yesShares.toString()) +
          parseFloat(market.noShares.toString());
        if (totalShares > 0) {
          const yesPrice =
            (parseFloat(market.yesShares.toString()) / totalShares) * 100;
          const noPrice =
            (parseFloat(market.noShares.toString()) / totalShares) * 100;
  ...
  static async closePosition(
    ...
    await prisma.$transaction(async (tx) => {
      ...
      logger.info(
        `Position closed: ${positionId} with P&L: ${realizedPnL}`,
        {},
        'PoolPerformanceService'
      );
    });
