 ...
 * Base Type Guards
 *
 * Core type guards for primitive and common types.
 * These are foundational guards used throughout the codebase.
 *
 ...
 *   Object.entries(value).forEach(([key, val]) => {
 *     // val is guaranteed to be string
 *   })
 * }
 * ```
 */
 ...
 *   const data = response.data // JsonValue
 * }
 * ```
 */
 ...
 * @example
 * ```ts
 * if (hasStringProperty(value, 'name')) {
...
export function hasStringProperty<K extends string>(
  ...
  // After hasProperty returns true, value is narrowed to Record<K, unknown>
 ...
 * @example
 * ```ts
 * if (hasNumberProperty(value, 'count')) {
 *   // value is Record<'count', number>
 *   console.log(value.count.toFixed(2))
 * }
 * ```
 */
 ...
 * @example
 * ```ts
 * if (hasBooleanProperty(value, 'active')) {
 *   // value is Record<'active', boolean>
 *   console.log(value.active ? 'yes' : 'no')
 * }
 * ```
 */
