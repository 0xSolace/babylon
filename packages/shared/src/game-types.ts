...
export interface Organization {
  ...
  description: string
  profileDescription?: string | null // What the organization says about itself on its profile
  type: 'company' | 'media' | 'government' | 'vc' | 'organization' | 'financial'
  canBeInvolved: boolean
  postStyle?: string
  postExample?: string[]
  // Stock price fields (only for companies)
  /** Starting price for tradeable orgs, null if not configured */
  initialPrice?: number | null
  /** Current price, null if not configured */
...
export interface Question {
  ...
  resolvedOutcome?: boolean | null // Final outcome when resolved
  resolutionProofUrl?: string | null // URL to post/article proving outcome
  resolutionDescription?: string | null // Description of how it resolved
