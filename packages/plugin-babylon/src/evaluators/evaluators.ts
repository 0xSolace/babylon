...
import type {
  BabylonMarket,
  ContentWithMarketMetadata,
  MarketAnalysis,
  MarketAnalysisState,
  PortfolioManagementState,
  SocialInteractionState,
  SocialPost,
  TradingCharacterSettings,
} from '../types'
import { assertDefined, hasMarketMetadata } from '../types'
...
export const marketAnalysisEvaluator: Evaluator = {
  ...
  handler: async (
    ...
    // State is mutable - we're extending it with our properties
    const analysisState = state as MarketAnalysisState

    // Extract marketId from state or message metadata
    let marketId = analysisState?.marketId
    if (!marketId && hasMarketMetadata(message.content)) {
      const contentWithMeta = message.content as ContentWithMarketMetadata
      marketId = contentWithMeta.metadata?.marketId
    }
...
export const socialInteractionEvaluator: Evaluator = {
  ...
  handler: async (
    ...
    const messageText = message.content.text?.toLowerCase() || ''
    const prefersLikes =
      messageText.includes('like') || messageText.includes('favorite')
    const prefersComments =
      messageText.includes('comment') || messageText.includes('reply')
    const prefersFollows =
      messageText.includes('follow') || messageText.includes('subscribe')
    const prefersPosts =
      messageText.includes('post') || messageText.includes('create')
    ...
    const posts: SocialPost[] | undefined = socialState.posts
    ...
    const shouldComment = prefersComments
      ? Math.random() < 0.5
      : Math.random() < 0.15 // Higher if user mentioned comments
    const shouldFollow = prefersFollows
      ? Math.random() < 0.6
      : Math.random() < 0.2 // Higher if user mentioned follows
    ...
    if (shouldFollow) {
      ...
      if (sortedAuthors.length > 0) {
        const selectedAuthor =
          sortedAuthors[Math.floor(Math.random() * sortedAuthors.length)]
        if (selectedAuthor) {
