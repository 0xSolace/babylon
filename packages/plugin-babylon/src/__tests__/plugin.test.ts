...
describe('Plugin Configuration', () => {
  ...
  it('should throw error for invalid configuration', async () => {
    ...
    if (predictionMarketsPlugin.init) {
      await expect(
        predictionMarketsPlugin.init(invalidConfig, runtime),
      ).rejects.toThrow()
    }
  })
...
describe('BUY_SHARES Action', () => {
  ...
  beforeEach(() => {
    ...
    buyAction = predictionMarketsPlugin.actions?.find(
      (a) => a.name === 'BUY_SHARES',
    )
  })
  ...
  it('should validate buy intent messages', async () => {
    ...
    const mockBabylonService = createPartialMock<BabylonClientService>({
      getClient: () => ({
        buyShares: async () => ({ success: true }),
      }),
    })
    runtime.getService = mock(() => mockBabylonService)
  ...
  it('should handle buy action with callback', async () => {
    ...
    const result = await buyAction.handler(
      runtime,
      message,
      state,
      undefined,
  ...
  it('should throw when service not available', async () => {
    ...
    await expect(
      buyAction.handler(runtime, message, undefined, undefined, undefined),
    ).rejects.toThrow()
  })
})
...
describe('SELL_SHARES Action', () => {
  ...
  beforeEach(() => {
    runtime = createMockRuntime()
    sellAction = predictionMarketsPlugin.actions?.find(
      (a) => a.name === 'SELL_SHARES',
    )
  })
...
describe('CHECK_WALLET Action', () => {
  ...
  beforeEach(() => {
    runtime = createMockRuntime()
    checkWalletAction = predictionMarketsPlugin.actions?.find(
      (a) => a.name === 'CHECK_WALLET',
    )
  })
...
describe('BabylonClientService', () => {
  ...
  it('should stop service correctly', async () => {
    ...
    // service is already BabylonClientService from start()
...
describe('A2A Market Data Provider', () => {
  let runtime: IAgentRuntime
  const provider = predictionMarketsPlugin.providers?.find(
    (p) => p.name === 'a2aMarketDataProvider',
  )
