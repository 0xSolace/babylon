...
export function createMockRuntime(
  ...
  const mockRuntime: IAgentRuntime = {
    ...
    getService: mock().mockImplementation(function (
      this: IAgentRuntime,
      serviceType: string,
    ) {
    ...
    getServicesByType: mock().mockImplementation(function (
      this: IAgentRuntime,
      serviceType: string,
    ) {
    ...
    registerService: mock().mockImplementation(async function (
      this: IAgentRuntime,
      serviceType: string,
      service: Service,
    ) {
    ...
    getRegisteredServiceTypes: mock().mockImplementation(function (
      this: IAgentRuntime,
    ) {
      return Array.from(servicesMap.keys())
    }),
    hasService: mock().mockImplementation(function (
      this: IAgentRuntime,
      serviceType: string,
    ) {
      const services = servicesMap.get(serviceType as ServiceTypeName)
 ...
 */
export function isMockFunction(
  value: unknown,
): value is ReturnType<typeof mock> {
  return (
    value !== null &&
    typeof value === 'object' &&
    'mock' in value &&
...
/**
 * Type-safe partial mock creator for services.
 *
 * Note: This function creates a partial implementation of a service type.
 * Tests only use specific methods so full implementations are not needed.
 * The generic constraint ensures type safety at call sites.
 *
 * @example
 * ```typescript
 * const mockService = createPartialMock<BabylonClientService>({
 *   getClient: () => ({ buyShares: async () => ({ success: true }) }),
 * })
 * runtime.getService = mock(() => mockService)
 * ```
 */
export function createPartialMock<T>(partial: Partial<T>): T {
  // Test mock pattern: create partial implementation that TypeScript treats as full type.
  // This is intentional - tests only use specific methods and don't need full implementations.
  return partial as T
}
