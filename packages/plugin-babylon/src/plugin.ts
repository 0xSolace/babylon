...
import {
  assertDefined,
  BABYLON_ROOM_ID,
  type BabylonTradingState,
  isDiscoveryService,
  SYSTEM_ENTITY_ID,
  type TradingCharacterSettings,
} from './types'
...
export class BabylonClientService extends Service {
  ...
  constructor(runtime: IAgentRuntime) {
    ...
    const settings = (runtime.character.settings ||
      {}) as TradingCharacterSettings

    // Try to get from discovery service first (if agent already discovered)
    const discoveredApiUrl = runtime.getSetting?.('babylon.apiEndpoint')

    this.clientConfig = {
      characterId: runtime.character.name || 'agent',
      apiBaseUrl:
        (typeof discoveredApiUrl === 'string' ? discoveredApiUrl : undefined) ||
      ...
      tradingLimits: {
        maxTradeSize:
          (typeof settings.babylonMaxTradeSize === 'number'
            ...
            : undefined) ||
          parseInt(process.env.BABYLON_MAX_TRADE_SIZE || '100', 10),
        maxPositionSize:
  ...
  static override async stop(runtime: IAgentRuntime): Promise<void> {
    ...
    assertDefined(service, 'BabylonClientService')
    await service.stop()
  }
...
export class BabylonTradingService extends Service {
  ...
  async start(): Promise<void> {
    ...
    // Verify BabylonClientService is available
    const clientService = this.runtime.getService<BabylonClientService>(
      BabylonClientService.serviceType,
    )
    assertDefined(clientService, 'BabylonClientService')

    const settings = this.runtime.character.settings as
  ...
  static override async stop(runtime: IAgentRuntime): Promise<void> {
    ...
    assertDefined(service, 'BabylonTradingService')
    await service.stop()
  }
    ...
    if (analyses && analyses.length > 0) {
      ...
      for (const analysis of analyses) {
        ...
        if (
          ...
          const tradeState = (await this.runtime.composeState(
            tradeMessage,
          )) as BabylonTradingState
...
export const predictionMarketsPlugin: Plugin = {
  ...
  async init(
    ...
    logger.info(`Babylon plugin initialized for agent: ${runtime.agentId}`)
