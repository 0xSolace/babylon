/**
 * EQLite Database Client
 *
 * This is the PRIMARY database interface for Babylon.
 * ALL database operations go through EQLite.
 *
 * API:
...
type Repo<TSelect, TInsert = Partial<TSelect>> = EQLiteTableRepository<
  TSelect,
  TInsert
>
...
// ============================================================================
// EQLite Client Interface
// ============================================================================

// Import schema types from eqlite-schema-types for repository typing
import type * as SchemaTypes from './eqlite-schema-types'

export interface EQLiteClient {
  ...
  // Transaction support
  transaction: <T>(fn: (tx: EQLiteClient) => Promise<T>) => Promise<T>
...
function repo<TSelect, TInsert = Partial<TSelect>>(
  ...
  return new EQLiteTableRepository<TSelect, TInsert>(tableName, getDB)
}

export function createEQLiteClient(): EQLiteClient {
  ...
  const $queryRaw = async <T = Record<string, SQLValue>>(
    ...
    strings.forEach((str, i) => {
      ...
      if (i < values.length) {
        ...
        if (value === undefined) {
          throw new Error('[EQLite] Missing parameter value for $queryRaw')
        }
  ...
  const $executeRaw = async (
    ...
    strings.forEach((str, i) => {
      ...
      if (i < values.length) {
        const value = values[i]
        if (value === undefined) {
          throw new Error('[EQLite] Missing parameter value for $executeRaw')
        }
        params.push(toQueryParam(value))
        sql += `$${i + 1}`
      }
  ...
  const client: EQLiteClient = {
    ...
    select: queryTx.select.bind(queryTx) as EQLiteClient['select'],
    selectDistinct: queryTx.select.bind(
      queryTx,
    ) as EQLiteClient['selectDistinct'],
    selectDistinctOn: (<
      T extends Record<string, unknown> = Record<string, unknown>,
    >(
      _columns: unknown[],
      _fields?: T,
    ) => queryTx.select(_fields)) as EQLiteClient['selectDistinctOn'],
    insert: queryTx.insert.bind(queryTx) as EQLiteClient['insert'],
    update: queryTx.update.bind(queryTx) as EQLiteClient['update'],
    delete: queryTx.delete.bind(queryTx) as EQLiteClient['delete'],
...
let clientInstance: EQLiteClient | null = null

export function getEQLiteClient(): EQLiteClient {
  if (!clientInstance) {
    clientInstance = createEQLiteClient()
  }
  return clientInstance
}

export function resetEQLiteClient(): void {
  clientInstance = null
  resetDB()
}
...
function createLazyEQLiteProxy(): EQLiteClient {
  const handler: ProxyHandler<EQLiteClient> = {
    get(_target, prop: string | symbol) {
      const client = getEQLiteClient()
      const value = client[prop as keyof EQLiteClient]
  ...
  return new Proxy({} as EQLiteClient, handler)
}

export const db: EQLiteClient = createLazyEQLiteProxy()
