/**
 * EQLite Integration Test
 *
 * Verifies that Babylon can:
 * 1. Initialize a database through DWS/EQLite
 * 2. Create tables properly
 * 3. Persist data across restarts
 */

import { afterAll, beforeAll, describe, expect, test } from 'bun:test'
import { BABYLON_SCHEMAS } from '../decentralized/eqlite-schema'
import { db, getDB, initializeDatabase, initializeDB, resetDB } from '../index'

// Skip tests if EQLite is not available
...
describe.skipIf(!EQLITE_AVAILABLE)('EQLite Integration', () => {
  beforeAll(async () => {
    // Set environment for localnet
    process.env.JEJU_NETWORK = 'localnet'
    process.env.EQLITE_BLOCK_PRODUCER_ENDPOINT = 'http://localhost:4661'
    process.env.EQLITE_DATABASE_ID = 'babylon-test'
  ...
  test('should create tables from schema', async () => {
    ...
    // Table names are PascalCase in EQLite
  ...
  test('db client should work with repository pattern', async () => {
    ...
    // This should use the EQLite-backed repository
...
describe.skipIf(!EQLITE_AVAILABLE)('EQLite Persistence', () => {
  beforeAll(async () => {
    process.env.JEJU_NETWORK = 'localnet'
    process.env.EQLITE_BLOCK_PRODUCER_ENDPOINT = 'http://localhost:4661'
    process.env.EQLITE_DATABASE_ID = 'babylon-persistence-test'
    resetDB()
    await initializeDatabase()
  })
  ...
  test('should persist data in EQLite data directory', async () => {
