/**
 * EQLite Table Repository
 *
 * Provides an ORM-style API (findUnique, findMany, create, update, delete)
 * backed by EQLite (EQLite) instead of PostgreSQL.
 *
 * EQLite-based TableRepository for decentralized operation.
 */
...
import { type DB, getDB, initializeDB, resetDB } from './decentralized/db'
...
/**
 * EQLite Table Repository with proper type preservation.
 *
 * Note: We use minimal constraints on TSelect and TInsert to preserve
 * the exact types from schema inference. The constraints are
 * only applied where needed for internal operations.
 */
export class EQLiteTableRepository<
  TSelect = Record<string, DatabaseValue>,
  TInsert = Partial<TSelect>,
> {
  constructor(
    private readonly tableName: string,
    private readonly getDbInstance: () => DB,
  ) {}
  ...
  async create(options: CreateOptions<TInsert>): Promise<TSelect> {
    // TInsert extends Partial<TSelect> which extends Record<string, DatabaseValue>
    ...
    const result = await this.db.queryOne<TSelect>(query, values)
    if (!result) {
  ...
  async createMany(options: {
    ...
    // Cast needed: TInsert is generic, we need to iterate over keys
    const firstAsRecord = firstRecord as Record<string, DatabaseValue>
    const columns = Object.keys(firstAsRecord)
  ...
  async update(options: UpdateOptions<TSelect, TInsert>): Promise<TSelect> {
    ...
    for (const [key, value] of Object.entries(options.data)) {
      ...
      if (isIncrementOp(value)) {
        setClauses.push(`"${key}" = "${key}" + ${value.increment}`)
      } else if (isDecrementOp(value)) {
        setClauses.push(`"${key}" = "${key}" - ${value.decrement}`)
  ...
  async deleteMany(options: Partial<DeleteOptions<TSelect>> = {}): Promise<{
    count: number
  }> {
    const params: QueryParam[] = []
    const { sql: whereSQL } = buildWhereClause(options.where, params)
