...
export class PerpDbAdapter {
  ...
  async getOpenPositionsByUser(userId) {
    const positions = await this.dbClient
      .select()
      .from(perpPositions)
      .where(
  ...
  async getOpenPositionByUserAndTicker(userId, ticker) {
    const [pos] = await this.dbClient
      .select()
      .from(perpPositions)
      .where(
  ...
  async updateOpenPosition(positionId, updates) {
  ...
  async closePosition(positionId, updates) {
  ...
  async updateMarketStats(ticker, updates) {
    const now = new Date();
...
//# sourceMappingURL=PerpDbAdapter.js.map
