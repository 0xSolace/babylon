...
export class PerpMarketService {
   ...
   */
  async applyPriceUpdates(priceUpdates) {
    const summary = {
      marketsUpdated: 0,
      positionsUpdated: 0,
      liquidations: 0,
   ...
   */
  async processFundingAndLiquidations(priceUpdates) {
    let summary;
    if (priceUpdates) {
   ...
   */
  calculateMarkPrice(spotPrice, annualFundingRate) {
    const fundingPremium = annualFundingRate / periodsPerYear();
    return spotPrice * (1 + fundingPremium);
  }
}
function calculateLiquidationPrice(entryPrice, side, leverage) {
...
function calculateUnrealizedPnL(entryPrice, currentPrice, side, size) {
...
function normalizePriceMap(input) {
  if (input instanceof Map) return input;
  if (Array.isArray(input)) return new Map(input);
  return new Map(Object.entries(input).map(([k, v]) => [k, Number(v)]));
}
function createPerpAggregate(ticker, organizationId) {
  return {
    ticker,
    organizationId,
    longOpenInterest: 0,
...
function calculateFundingPayment(size, fundingRate) {
...
//# sourceMappingURL=PerpMarketService.js.map
