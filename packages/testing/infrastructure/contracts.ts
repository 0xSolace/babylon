...
export async function getChainConfig(
  ...
  const configs = {
    localnet: {
      chain: hardhat,
      rpcUrl: process.env.PUBLIC_RPC_URL ?? 'http://localhost:6545',
      deployerKey:
...
export async function checkMessagingContracts(
  ...
  const keyRegistryAddress = toAddressOrNull(process.env[ENV_KEYS.keyRegistry])
  const messageNodeRegistryAddress = toAddressOrNull(
    process.env[ENV_KEYS.messageNodeRegistry],
  )
...
export async function deployMessagingContracts(
  ...
  if (!keyRegistryMatch?.[1] || !messageNodeRegistryMatch?.[1]) {
    throw new Error('Failed to parse deployed contract addresses')
  }

  const addresses = {
    keyRegistry: toAddress(keyRegistryMatch[1]),
    messageNodeRegistry: toAddress(messageNodeRegistryMatch[1]),
  }
...
export async function ensureContractsDeployed(
  ...
  if (status.allDeployed) {
    ...
    const keyRegistry = status.contracts.find(
      (c) => c.name === 'KeyRegistry',
    )?.address
    const messageNodeRegistry = status.contracts.find(
      (c) => c.name === 'MessageNodeRegistry',
    )?.address
    if (!keyRegistry || !messageNodeRegistry) {
      throw new Error('Contracts marked as deployed but addresses not found')
    }
    return {
      keyRegistry,
      messageNodeRegistry,
...
export function printContractStatus(status: ContractsStatus): void {
  console.log(`\n${'═'.repeat(60)}`)
  console.log(`CONTRACT STATUS (${status.network}, chainId: ${status.chainId})`)
...
// Use JejuNetwork type from shared
...
// CLI entry point
if (import.meta.main) {
  const network = toMessagingNetwork(process.argv[2])
