  ...
  test('should identify top engaged users', async () => {
    ...
    if (user1.id) {
      for (let i = 0; i < 10; i++) {
        ...
        await recordUserInteraction(user1.id, npc.id, postId, comment, 0.9)
      }
    }
    ...
    if (user2.id) {
      for (let i = 10; i < 15; i++) {
        ...
        await recordUserInteraction(user2.id, npc.id, postId, comment, 0.75)
      }
    }

    // User 3: Low engagement (1 comment)
    if (user3.id) {
      const post15 = posts[15]
      if (!post15) throw new Error('Post 15 not found')
      const comment3 = await createComment(user3.id, post15, 'User3 comment')
      await recordUserInteraction(user3.id, npc.id, post15, comment3, 0.6)
    }
...
describe('Full Engagement → Invite Flow', () => {
  ...
  test('should invite user with high engagement score', async () => {
    ...
    const post1 = posts[1]
    const post2 = posts[2]
    if (!post0 || !post1 || !post2) throw new Error('Required posts not found')
    await createComment(engagedUser.id, post0, 'Great analysis!')
    await createComment(engagedUser.id, post2, 'Really helpful')
    await createComment(engagedUser.id, post1, 'Thanks for sharing')

    // Likes: 5-8 per NPC (ideal)
    for (let i = 0; i < 8; i++) {
      const postId = posts[i]
      if (!postId) continue
      await createLike(engagedUser.id, postId)
    }

    // Shares: 1-2 (ideal)
    await createShare(engagedUser.id, post0)
    await createShare(engagedUser.id, post1)
  ...
  test('should NOT invite user with spam behavior', async () => {
    ...
    const { score, breakdown } =
      await NPCGroupDynamicsService.calculateReplyGuyScore(spammer.id, [npc.id])
