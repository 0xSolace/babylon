...
describe('Full Production Tick Integration Test', () => {
  ...
  describe('3. Lookahead Content Generation', () => {
    test.skipIf(!hasLLMKey)(
      ...
      async () => {
        // Dynamic import: Only load when LLM key is available (conditional via test.skipIf)
  ...
  describe('5. Database State Validation', () => {
    test('validates posts in database', async () => {
      // Get recent posts - SelectBuilder infers Post[] from typed table reference
      ...
      for (const post of recentPosts) {
        ...
        results.content.postSamples.push({
          id: String(post.id),
          authorId,
          authorName: actor?.name ?? authorId,
    ...
    test('validates events in database', async () => {
      ...
      results.content.eventSamples = recentEvents.map((e) => ({
        id: String(e.id),
        type: String(e.eventType ?? 'unknown'),
    ...
    test('validates questions in database', async () => {
      ...
      results.questions.questionSamples = allQuestions
        ...
        .map((q) => ({
          id: String(q.id),
          text: String(q.text ?? '').substring(0, 100),
          status: String(q.status ?? 'unknown'),
          outcome:
            q.outcome === null || q.outcome === undefined
              ? null
              : typeof q.outcome === 'boolean'
                ? q.outcome
                : String(q.outcome) === 'true',
        }))
    ...
    test('validates markets in database', async () => {
      ...
      results.markets.marketSamples = allMarkets.slice(0, 10).map((m) => ({
        id: String(m.id),
        ticker: String(m.question ?? '').substring(0, 50),
        type: 'prediction',
    ...
    test('validates NPC trades in database', async () => {
      ...
      for (const trade of recentTrades) {
        const npcActorId = String(trade.npcActorId ?? '')
        const actor = StaticDataRegistry.getActor(npcActorId)
        const marketType = String(trade.marketType ?? '')
        ...
        if (results.trading.tradeSamples.length < 10) {
          const ticker = trade.ticker
            ...
            : trade.marketId
              ...
              : 'unknown'
          const side = trade.side ? String(trade.side) : 'unknown'
          results.trading.tradeSamples.push({
            actorId: npcActorId,
            actorName: actor?.name ?? npcActorId,
            marketType:
              marketType === 'perp' || marketType === 'prediction'
                ? (marketType as 'perp' | 'prediction')
                : 'prediction',
            ticker,
            side,
