...
describe('Group Chat Simulation - End to End Flow', () => {
  ...
  describe('Engagement Score Calculation', () => {
    ...
    test('should calculate engagement score from user interactions', async () => {
      ...
      const post0 = posts[0]
      const post1 = posts[1]
      if (!post0 || !post1) throw new Error('Required posts not found')
      ...
      await recordUserInteraction(user.id, npc.id, post0, comment1, 0.85)
      await recordUserInteraction(user.id, npc.id, post1, comment2, 0.9)
    ...
    test('should rank users by engagement level', async () => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        await recordUserInteraction(highUser.id, npc.id, postId, comment, 0.9)
      }
      ...
      for (let i = 8; i < 12; i++) {
        ...
        await recordUserInteraction(medUser.id, npc.id, postId, comment, 0.75)
      }

      // Low engagement: 1 comment
      const post14 = posts[14]
      if (!post14) throw new Error('Post 14 not found')
      const lowComment = await createComment(lowUser.id, post14, 'Interesting')
      await recordUserInteraction(lowUser.id, npc.id, post14, lowComment, 0.6)
      ...
      const medScore = medUserScore?.engagementScore ?? 0
      const lowScore = lowUserScore?.engagementScore ?? 0
      expect(highUserScore?.engagementScore ?? 0).toBeGreaterThan(medScore)
  ...
  describe('Reply Guy Score Calculation', () => {
    ...
    test('should give combined score for ideal engagement pattern', async () => {
      ...
      const post1 = posts[1]
      if (!post0 || !post1) throw new Error('Required posts not found')
      await createComment(user.id, post0, 'Great analysis')
      await createComment(user.id, post1, 'Thanks for sharing')

      for (const postId of posts) {
        await createLike(user.id, postId)
      }

      await createShare(user.id, post0)
  ...
  describe('Group Invite Service', () => {
    ...
    test('should track and respect group membership limits', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const npc = npcs[i]
        if (!npc) throw new Error(`NPC ${i} not found`)
        const chatId = await createGroupChat(`Group ${i}`, npc.id)
  ...
  describe('User vs Agent Parity', () => {
    ...
    test('agents should earn same engagement score as users for same behavior', async () => {
      ...
      const humanPost = await createNPCPost(npc.id, 'Post for human engagement')
      const agentPost = await createNPCPost(npc.id, 'Post for agent engagement')
