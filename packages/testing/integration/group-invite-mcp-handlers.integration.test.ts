...
import { executeAcceptGroupInvite, executeDeclineGroupInvite, executeGetGroupInvites } from '@babylon/mcp';
...
async function createTestGroupChat(options: { name?: string; npcAdminId: string }) {
  ...
  await db.chatParticipant.create({ data: { id: participantId, chatId: id, userId: options.npcAdminId } });
...
async function cleanupTestData() {
  ...
  if (testIds.participantIds.length > 0) await db.chatParticipant.deleteMany({ where: { id: { in: testIds.participantIds } } });
  if (testIds.inviteIds.length > 0) {
    await db.chatInvite.deleteMany({ where: { id: { in: testIds.inviteIds } } });
...
describe('MCP Group Invite Handlers', () => {
  ...
  describe('executeGetGroupInvites', () => {
    ...
    test('should return user-initiated (chatInvite) invites with source=user', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'User Invite Group', npcAdminId: npc.id });
    ...
    test('should return NPC-initiated (userGroupInvite) invites with source=npc', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'NPC Invite Group', npcAdminId: npc.id });
    ...
    test('should return combined invites from both sources', async () => {
      ...
      const chat1 = await createTestGroupChat({ name: 'User Group', npcAdminId: npc.id });
      const chat2 = await createTestGroupChat({ name: 'NPC Group', npcAdminId: npc.id });
    ...
    test('should not return non-pending invites', async () => {
      ...
      const chat1 = await createTestGroupChat({ name: 'Status Group Accepted', npcAdminId: npc.id });
      const chat2 = await createTestGroupChat({ name: 'Status Group Declined', npcAdminId: npc.id });
    ...
    test('should not return invites for other users', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'Access Group', npcAdminId: npc.id });
  ...
  describe('executeAcceptGroupInvite', () => {
    test('should accept user-initiated invite and create participant', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'Accept Group', npcAdminId: npc.id });
      ...
      const invite = await db.chatInvite.findUnique({ where: { id: inviteId } });
    ...
    test('should accept NPC-initiated invite and create participant + membership', async () => {
      const invitee = await createTestUser({ displayName: 'NPC Accept Invitee' });
      const npc = await createTestNPC({ name: 'NPC Accept NPC' });
      const chat = await createTestGroupChat({ name: 'NPC Accept Group', npcAdminId: npc.id });
    ...
    test('should throw error for non-existent invite', async () => {
      ...
      await expect(executeAcceptGroupInvite(agent, { inviteId: fakeInviteId })).rejects.toThrow(
        'Invite not found or access denied'
      );
    });
    ...
    test('should throw error when accepting invite for another user', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'Security Group', npcAdminId: npc.id });
      ...
      await expect(executeAcceptGroupInvite(attackerAgent, { inviteId })).rejects.toThrow(
        'Invite not found or access denied'
      );
  ...
  describe('executeDeclineGroupInvite', () => {
    test('should decline user-initiated invite', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'Decline Group', npcAdminId: npc.id });
    ...
    test('should decline NPC-initiated invite with respondedAt', async () => {
      const invitee = await createTestUser({ displayName: 'NPC Decline Invitee' });
      const npc = await createTestNPC({ name: 'NPC Decline NPC' });
      const chat = await createTestGroupChat({ name: 'NPC Decline Group', npcAdminId: npc.id });
      ...
      expect(invite?.status).toBe('declined');
      expect(invite?.respondedAt).not.toBeNull();
      expect(invite?.respondedAt!.getTime()).toBeGreaterThanOrEqual(beforeDecline.getTime());
      expect(invite?.respondedAt!.getTime()).toBeLessThanOrEqual(afterDecline.getTime());
    });
    ...
    test('should throw error for non-existent invite', async () => {
      ...
      await expect(executeDeclineGroupInvite(agent, { inviteId: fakeInviteId })).rejects.toThrow(
        'Invite not found or access denied'
      );
    });
    ...
    test('should throw error when declining invite for another user', async () => {
      ...
      const attacker = await createTestUser({ displayName: 'Decline Attacker' });
      const npc = await createTestNPC({ name: 'Decline Security NPC' });
      const chat = await createTestGroupChat({ name: 'Decline Security Group', npcAdminId: npc.id });
      ...
      await expect(executeDeclineGroupInvite(attackerAgent, { inviteId })).rejects.toThrow(
        'Invite not found or access denied'
      );
  ...
  describe('Edge Cases', () => {
    test('should handle missing group chat gracefully', async () => {
      const invitee = await createTestUser({ displayName: 'Missing Chat User' });
    ...
    test('should handle agent users same as regular users', async () => {
      const agentUser = await createTestUser({ isAgent: true, displayName: 'Agent User' });
      const npc = await createTestNPC({ name: 'Agent NPC' });
      const chat = await createTestGroupChat({ name: 'Agent Group', npcAdminId: npc.id });
