...
import { TokenService } from '@babylon/api';
...
describe('TokenService', () => {
  ...
  describe('Airdrop Allocation Calculation', () => {
    it('should calculate allocation for a user with points', async () => {
      const allocation =
        await TokenService.calculateAirdropAllocation(testUserId);
    ...
    it('should return null for non-existent user', async () => {
      const allocation =
        await TokenService.calculateAirdropAllocation('nonexistent');
  ...
  describe('Token Formatting', () => {
    it('should format tokens correctly', () => {
      // Test whole numbers
      const whole = TokenService.formatTokens(
        1000n * 10n ** BigInt(TOKEN_DECIMALS)
      );
      expect(whole).toBe(`1,000 ${TOKEN_SYMBOL}`);

      // Test with decimals
      const decimals = TokenService.formatTokens(1500500000000000000000n); // 1500.5 tokens
      expect(decimals).toContain(TOKEN_SYMBOL);
    });

    it('should parse tokens correctly', () => {
      const parsed = TokenService.parseTokens('1000');
      expect(parsed).toBe(1000n * 10n ** BigInt(TOKEN_DECIMALS));

      const parsedDecimals = TokenService.parseTokens('1000.5');
    ...
    it('should round-trip format and parse', () => {
      ...
      const formatted = TokenService.formatTokens(original);

      // Parse just the numeric part
      const numericPart = formatted
        .replace(` ${TOKEN_SYMBOL}`, '')
        .replace(/,/g, '');
      const parsed = TokenService.parseTokens(numericPart);
  ...
  describe('Merkle Data Generation', () => {
    it('should generate merkle data for eligible users', async () => {
      const merkleData = await TokenService.generateAirdropMerkleData();
