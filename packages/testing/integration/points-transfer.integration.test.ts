...
describe('Points Transfer - Basic Functionality', () => {
  ...
  it('should create transaction records for both sender and recipient', async () => {
    ...
    // Verify metadata
    const senderMetadata = JSON.parse(
      typeof senderTx?.metadata === 'string' ? senderTx.metadata : '{}',
    )
    const recipientMetadata = JSON.parse(
      typeof recipientTx?.metadata === 'string' ? recipientTx.metadata : '{}',
    )
  ...
  it('should include message in transaction metadata', async () => {
    ...
    if (
      currentSenderBalance === undefined ||
      currentRecipientBalance === undefined
    ) {
    ...
    const metadata = JSON.parse(
      typeof transaction?.metadata === 'string' ? transaction.metadata : '{}',
    )
...
describe('Points Transfer - Edge Cases', () => {
  ...
  it('should handle zero-balance user receiving points', async () => {
    ...
    if (currentBalance === undefined || senderCurrentBalance === undefined) {
      throw new Error('Failed to get current balances')
    }
  ...
  it('should maintain correct point totals across multiple transfers', async () => {
    ...
    if (
      user1Before === undefined ||
      user2Before === undefined ||
      user3Before === undefined
    ) {
...
describe('Points Transfer - Trades Feed Integration', () => {
  it('should query transfers from pointsTransaction table', async () => {
    ...
    for (const transfer of transfers) {
      ...
      const metadata = JSON.parse(
        typeof transfer.metadata === 'string' ? transfer.metadata : '{}',
      )
  ...
  it('should create notification for recipient', async () => {
    ...
    const notification = await db.notification.create({
      data: {
        ...
        body: `${testUser1.displayName} sent you 100 points`,
  ...
  it('should include message in notification if provided', async () => {
    ...
    const notification = await db.notification.create({
      data: {
        ...
        body: `${testUser1.displayName} sent you 50 points: "${message}"`,
...
describe('Points Transfer - Data Integrity', () => {
  it('should maintain referential integrity', async () => {
    ...
    for (const tx of transactions) {
      ...
      const metadata = JSON.parse(
        typeof tx.metadata === 'string' ? tx.metadata : '{}',
      )
