...
export function isMessagingEnabled(): boolean {
  // Default is enabled - only disable if explicitly set to centralized
  if (process.env.MESSAGING_MODE === 'centralized') {
    return false
  }
  return true
...
/**
 * Send a message through both centralized and messaging channels
 */
export async function sendMessage(options: {
  ...
  // If messaging is disabled, return early
  if (!isMessagingEnabled()) {
    return result
  }

  // If no sender address, we can't use messaging
  if (!options.senderAddress) {
    logger.debug('No sender address provided, skipping messaging')
    return result
  }
  ...
  try {
    ...
    if (sendResult.decentralized) {
      result.messageId = sendResult.decentralized.id
      result.encrypted = Boolean(sendResult.decentralized.encryptedContent)

      logger.info('Message sent to storage', {
    ...
    logger.warn('Failed to send to storage', {
      chatId: options.chatId,
