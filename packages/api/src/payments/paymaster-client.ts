...
export class PaymasterClient {
  ...
  private async rpc(method: string, params: unknown[]): Promise<Hex> {
...
export function getPaymasterClient(): PaymasterClient {
  ...
  if (!network) throw new Error('[Paymaster] JEJU_NETWORK not set');
  ...
  const paymasterAddr =
    (process.env.PAYMASTER_ADDRESS as Hex | undefined) ?? paymasterAddrs[net];
  const tokenAddr =
    (process.env.CREDIT_TOKEN_ADDRESS as Hex | undefined) ?? tokenAddrs[net];
  ...
  paymasterClient = new PaymasterClient({
    ...
    entryPointAddress:
      (process.env.ENTRY_POINT_ADDRESS as Hex) ?? entryPointAddrs[net],
...
export function isPaymasterAvailable(): boolean {
  return !!(process.env.JEJU_NETWORK || process.env.NEXT_PUBLIC_JEJU_NETWORK);
}
