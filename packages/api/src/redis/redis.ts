...
class Redis implements RedisCompatibleClient {
  ...
  async lpop(key: string): Promise<string | null> {
    ...
    // After length check, shift() always returns string, but TS doesn't know
  ...
  async rpop(key: string): Promise<string | null> {
    ...
    const list = await this.cache.get<string[]>(key)
    if (!list || list.length === 0) return null
    // After length check, pop() always returns string, but TS doesn't know
