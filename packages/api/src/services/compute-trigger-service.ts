...
export class ComputeTriggerService {
  ...
  private getPublicClient() {
    return createBabylonPublicClient({
      chain: this.getChain(),
      rpcUrl: this.config.computeRpcUrl,
    })
  }
  ...
  private async registerOnChain(trigger: TriggerDefinition): Promise<void> {
    if (
      ...
      logger.debug('[ComputeTrigger] No registry configured, skipping on-chain')
      return
    }
  ...
  private async performAction(
    ...
    const baseUrl = process.env.API_BASE_URL ?? process.env.PUBLIC_APP_URL ?? ''
