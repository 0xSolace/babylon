...
import { type DailyEngagement, dailyEngagement, db, eq } from '@babylon/db'
...
export async function getOrCreateEngagement(
  ...
  const existing = await db.dailyEngagement.findFirst({
    where: {
      AND: [{ userId }, { dateKey }],
    },
  })
  ...
  const newRecord: DailyEngagement = {
    id,
    userId,
    dateKey,
    hasLiked: false,
  ...
  await db.dailyEngagement.create({ data: newRecord })
...
export async function recordTrade(
  ...
  await db
    ...
    .set({
      ...
      qualifiedForDrip: qualified,
      qualifiedAt: !engagement.qualifiedForDrip ? now : engagement.qualifiedAt,
      updatedAt: now,
    })
    .where(eq(dailyEngagement.id, engagement.id))
...
export async function canClaimDrip(userId: string): Promise<{
  ...
  if (!engagement.qualifiedForDrip) {
    const socialNeeded = SOCIAL_TRACK_REQUIRED - engagement.socialActionsCount
    return {
      canClaim: false,
...
export async function getEngagementHistory(
  ...
  return db.dailyEngagement.findMany({
    where: { userId },
...
export async function getTodayEngagementStats(): Promise<{
  ...
  const validEngagements = engagements.filter(
    (e): e is NonNullable<typeof e> => e != null,
  )
  return {
    dateKey,
    totalUsers: validEngagements.length,
    qualified: validEngagements.filter((e) => e.qualifiedForDrip).length,
    claimed: validEngagements.filter((e) => e.dripClaimed).length,
    socialComplete: validEngagements.filter((e) => e.socialTrackComplete)
      .length,
    tradingComplete: validEngagements.filter((e) => e.tradingTrackComplete)
      .length,
  }
}
