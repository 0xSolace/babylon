 ...
 */
export async function calculateAirdropAllocation(
  userId: string,
  ...
  const totalsResults = await db.$queryRaw<{
    totalPoints: number
    userCount: number
  }>`
      SELECT SUM(reputation_points) as "totalPoints", COUNT(*) as "userCount"
      FROM "User" WHERE is_actor = false
    `
  const totalsResult = totalsResults[0]
  ...
  const volumeResult = await db.$queryRaw<{ total: string }>`
      SELECT COALESCE(SUM(amount), '0') as total FROM "TradingFee" WHERE user_id = ${userId}
    `
  ...
  const baseAllocation = BigInt(Math.floor(Number(airdropPool) * totalWeight))
 ...
 */
export async function generateAirdropMerkleData(): Promise<
...
> {
  const eligibleUsers = (await db
    ...
    .where(
      and(
        ...
        gte(users.reputationPoints, POINTS.INITIAL_SIGNUP),
      ),
    )) as Array<{
    id: string
    walletAddress: string | null
  ...
  for (const user of eligibleUsers) {
    ...
    const allocation = await calculateAirdropAllocation(String(user.id))
...
export async function getAirdropStatus(userId: string): Promise<{
  ...
  const totalUnlocked = (totalAllocation * BigInt(totalUnlockedPercent)) / 100n
  const claimable =
 ...
 */
export function calculateProfitBonus(
  realizedPnL: number,
 ...
 */
export function calculateLeaderboardBonus(position: number): {
  multiplier: number
  tier: string
} {
...
export async function calculateBonusAllocation(userId: string): Promise<{
  ...
  const allocationResult = (await db
    ...
    .from(airdropAllocations)
    .where(eq(airdropAllocations.userId, userId))
    .limit(1)) as Array<{ totalAllocation: string }>
  ...
  // Query builder field selection returns correctly typed results
  const userResult = (await db
    .select({
      lifetimePnL: users.lifetimePnL,
      totalDeposited: users.totalDeposited,
  ...
  const profitBonus = calculateProfitBonus(realizedPnL, initialDeposit)
  ...
  const leaderboardBonus = calculateLeaderboardBonus(0)
 ...
 */
export async function getTokenBalance(userId: string): Promise<bigint> {
  const result = (await db
    .select()
    .from(tokenBalances)
    .where(eq(tokenBalances.userId, userId))
    .limit(1)) as Array<{ balance: string | null }>

  const balance = result[0]
  if (!balance || typeof balance.balance !== 'string') return 0n
 ...
 */
export async function pointsToDisplayTokens(points: number): Promise<string> {
  const ratio = await getPointsToTokenRatio()
 ...
 */
export async function getPointsToTokenRatio(): Promise<number> {
  const result = await db.$queryRaw<{ totalPoints: string }>`
      SELECT COALESCE(SUM(reputation_points), 0) as "totalPoints" FROM "User"
    `
 ...
 */
export function parseTokens(amountStr: string): bigint {
