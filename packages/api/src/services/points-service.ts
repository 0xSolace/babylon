...
export async function awardPoints(
  ...
  // Get current user state using repository with select for proper typing
  const user = await db.user.findUnique({
    ...
    select: {
      ...
      pointsAwardedForFarcasterFollow: true,
      pointsAwardedForTwitter: true,
      pointsAwardedForTwitterFollow: true,
      pointsAwardedForDiscord: true,
      pointsAwardedForDiscordJoin: true,
  ...
  const alreadyAwarded = checkAlreadyAwarded(user, reason)
  if (alreadyAwarded) {
    return {
      success: true,
      pointsAwarded: 0,
...
async function _awardShareAction(
  ...
  return awardPoints(userId, amount, reason, {
    platform,
    contentType,
    ...(contentId ? { contentId } : {}),
  })
...
export async function awardReferralSignup(
  ...
  const unqualifiedCount = await db.referral.count({
    where: {
      AND: [
        { referrerId },
        { status: 'completed' },
 ...
 */
export async function awardPendingReferralSignupPoints(
  referrerId: string,
  ...
  const result = await awardPoints(
    ...
    'referral_signup',
    {
      ...
      deferredAward: true,
      originalCompletedAt: toNull(pendingReferral.completedAt?.toISOString()),
    },
  )
...
export async function checkAndQualifyReferral(
  ...
  const qualificationResult = await awardPoints(
    user.referredBy,
  ...
  if (qualificationResult.success) {
    ...
    await awardPendingReferralSignupPoints(user.referredBy)
  }
...
export async function getLeaderboard(
  ...
  // Helper to map user records to leaderboard entries
  const mapUserToEntry = (user: {
    ...
    allPoints: Number(user.reputationPoints ?? 0),
    invitePoints: Number(user.invitePoints ?? 0),
    ...
    nftTokenId: Number(user.nftTokenId ?? 0),
  })
  ...
  const combined = [...usersResult.map(mapUserToEntry)]
      ...
      ...actorStates
        .map((state) => {
          ...
          return {
            ...
            allPoints: state.reputationPoints ?? 0,
            invitePoints: 0,
            earnedPoints: 0,
            bonusPoints: 0,
            referralCount: 0,
...
export async function getUserRank(userId: string): Promise<number | null> {
  ...
  const higherUsersCount = await db.user.count({
    where: {
      AND: [
  ...
  const higherActorsCount = await db.actorState.count({
    where: {
      reputationPoints: { gt: user.reputationPoints },
    },
  })
