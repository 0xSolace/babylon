...
import { generateSnowflakeId, logger, NotFoundError } from '@babylon/shared'
 ...
 */
export async function markAsWaitlisted(
  userId: string,
  referralCode?: string,
  ...
  const inviteCode = user.referralCode || generateInviteCode()
  ...
  if (referralCode) {
    ...
    if (referrer) {
      ...
      // Valid referral - use referral system
        ...
        const { awardReferralSignup } = await import('./points-service')
        const referralResult = await awardReferralSignup(referrer.id, userId)
 ...
 */
export async function graduateFromWaitlist(userId: string): Promise<boolean> {
  await db
    .update(users)
    .set({
...
export async function getWaitlistPosition(
  ...
  const usersAheadResults: CountRow[] = await db
    .select({ count: count() })
    .from(users)
    .where(
      and(
        ...
        or(
          ...
          gt(users.invitePoints, user.invitePoints as number),
          // Tie-breaker: If same invite points, earlier signup wins
          and(
            eq(users.invitePoints, user.invitePoints as number),
            lt(users.waitlistJoinedAt, userJoinedAt),
          ),
        ),
      ),
    )
  const usersAheadResult = usersAheadResults[0]
 ...
 */
export async function awardWalletBonus(
  userId: string,
...
export async function getTotalWaitlistCount(): Promise<number> {
  const results: CountRow[] = await db
    .select({ count: count() })
    .from(users)
    .where(and(ne(users.waitlistPosition, 0), eq(users.isWaitlistActive, true)))
 ...
 */
export async function getTopWaitlistUsers(
  limit = 10,
  offset = 0,
