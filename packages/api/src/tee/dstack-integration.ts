...
export class DStackIntegration {
  ...
  private async initializeRealTEE(): Promise<void> {
    ...
    this.platform = process.env.TDX_ENABLED === 'true' ? 'intel_tdx' : 'amd_sev'
  ...
  private async createRealWallet(salt: string): Promise<DStackWallet> {
    ...
    const addressHash = keccak256(toBytes(`${this.measurement}:${salt}:wallet`))
...
export async function _initializeDStack(
  config?: DStackConfig,
...
export function _isDStackAvailable(): boolean {
