...
class CacheClient {
  ...
  private requireInitialized(): void {
    if (!this.initialized) {
      throw new Error('[Cache] Cache not initialized. Call initialize() first.')
    }
  }
  ...
  async expire(key: string, ttlSeconds: number): Promise<boolean> {
    ...
    return this.withRetry(async () => {
      ...
      const data: unknown = await response.json()
      if (!isCacheSuccessResponse(data)) {
        throw new Error('[Cache] Invalid EXPIRE response')
      }
      return data.success
    }, `EXPIRE ${key}`)
  }
  ...
  async mget<T = string>(...keys: string[]): Promise<(T | null)[]> {
    ...
    return this.withRetry(async () => {
      ...
      if (!isCacheEntriesResponse<T>(data)) {
        throw new Error('[Cache] Invalid MGET response')
      }
      return keys.map((k) => toNull(data.entries[this.prefixKey(k)]))
  ...
  async getStats(): Promise<CacheStats> {
    ...
    return this.withRetry(async () => {
      ...
      return {
        totalKeys: data.stats.totalKeys,
        memoryUsedBytes: data.stats.usedMemoryMb * 1024 * 1024,
        hitCount: data.stats.hits,
        missCount: data.stats.misses,
