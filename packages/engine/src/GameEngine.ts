    ...
    try {
      ...
      if (priceUpdates.length > 0) {
        ...
        try {
          ...
          logger.debug(
            'Could not broadcast price updates:',
            error,
            'GameEngine'
          );
        }
    ...
    if (tick.priceUpdates.length > 0) {
      await Promise.allSettled(
        tick.priceUpdates.map(async (update) => {
          try {
            await db.updateOrganizationPrice(
                ...
                try {
                  ...
                  const nestedErrorMessage =
                    nestedError instanceof Error
                      ? nestedError.message
                      : String(nestedError);
                  const nestedErrorStack =
                    nestedError instanceof Error
                      ? nestedError.stack
                      : undefined;
                  logger.warn(
                    `Failed to upsert organization ${update.organizationId}: ${nestedErrorMessage}`,
                    {
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.warn(
              `Failed to update price for ${update.organizationId}: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
                organizationId: update.organizationId,
          ...
          try {
                ...
                try {
                  ...
                  const nestedErrorStack =
                    nestedError instanceof Error
                      ...
                      : undefined;
                  logger.warn(
                    `Failed to reconcile price update for ${update.organizationId}: ${nestedErrorMessage}`,
                    {
                      error: nestedErrorMessage,
                      stack: nestedErrorStack,
                      organizationId: update.organizationId,
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.warn(
              `Failed to record price update for ${update.organizationId}: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
                organizationId: update.organizationId,
