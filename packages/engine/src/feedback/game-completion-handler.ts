...
export class GameCompletionHandler {
  ...
  start(gameEngine: GameEngine): void {
    if (this.isActive) {
      logger.warn(
        'Game completion handler already active',
        undefined,
    ...
    try {
      ...
      gameEngine.on('started', () => {
        logger.info(
          'Game engine started - auto-feedback active',
          undefined,
          'GameCompletionHandler'
        );
      });
      ...
      gameEngine.on('stopped', () => {
        logger.info(
          'Game engine stopped - pausing auto-feedback',
          undefined,
          'GameCompletionHandler'
        );
      });
      ...
      this.isActive = true;
      logger.info(
        'Game completion handler started successfully',
        undefined,
        'GameCompletionHandler'
      );
    } catch (error) {
      ...
      logger.error(
        `Failed to start game completion handler: ${errorMessage}`,
        { error },
        'GameCompletionHandler'
      );
      throw error;
  ...
  stop(): void {
    if (!this.isActive) {
      logger.warn(
        'Game completion handler not active',
        undefined,
        'GameCompletionHandler'
      );
      return;
    ...
    try {
      ...
      this.isActive = false;
      logger.info(
        'Game completion handler stopped',
        undefined,
        'GameCompletionHandler'
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error(
        `Failed to stop game completion handler: ${errorMessage}`,
        { error },
        'GameCompletionHandler'
      );
      throw error;
