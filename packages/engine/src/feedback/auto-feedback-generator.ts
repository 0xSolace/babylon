...
export class AutoFeedbackGenerator {
  ...
  initialize(gameEngine: GameEngine): void {
    if (this.isListening) {
      logger.warn(
        'Auto-feedback generator already listening',
        undefined,
        'AutoFeedback'
      );
      return;
    ...
    this.startListening();
    logger.info(
      'Auto-feedback generator initialized and listening',
      undefined,
      'AutoFeedback'
    );
  }
  ...
  private startListening(): void {
    if (!this.gameEngine) {
      logger.error(
        'Cannot start listening: game engine not initialized',
        undefined,
        'AutoFeedback'
      );
      return;
    ...
    this.gameEngine.on('gameCompleted', (event: GameCompletionEvent) => {
      this.handleGameCompletion(event).catch((error) => {
        ...
        logger.error(
          `Failed to handle game completion: ${errorMessage}`,
          { error, event },
          'AutoFeedback'
        );
      });
    ...
    logger.info(
      'Started listening for game completion events',
      undefined,
      'AutoFeedback'
    );
  }
  ...
  stop(): void {
    if (this.gameEngine) {
      ...
      logger.info(
        'Stopped listening for game completion events',
        undefined,
        'AutoFeedback'
      );
    }
  ...
  private async handleGameCompletion(
    ...
    logger.info(
      'Processing game completion for auto-feedback',
      {
    ...
    try {
      ...
      if (!metrics) {
        logger.warn(
          'Could not calculate metrics for game completion',
          { event },
          'AutoFeedback'
        );
        return;
      ...
      logger.info(
        'Successfully generated auto-feedback',
        {
          agentId: event.agentId,
          gameId: event.gameId,
        },
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error(
        `Failed to generate auto-feedback: ${errorMessage}`,
        { error, event },
        'AutoFeedback'
      );
      throw error;
  ...
  private async calculateGameMetrics(
    ...
    try {
      ...
      if (positions.length === 0) {
        logger.warn(
          'No resolved positions found for game completion',
          {
            agentId: event.agentId,
            gameId: event.gameId,
          },
      ...
      const maxPositionSize = Math.max(
        ...positions.map((p) => Number(p.amount || 0))
      );
      const avgPositionSize = totalInvested / positions.length;
      const riskScore =
        maxPositionSize > 0
          ? Math.min(1.0, avgPositionSize / maxPositionSize)
          : 0.5;
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error(
        `Failed to calculate game metrics: ${errorMessage}`,
        { error, event },
        'AutoFeedback'
      );
      return null;
