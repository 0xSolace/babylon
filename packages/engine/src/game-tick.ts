...
export async function executeGameTick(
  ...
  const continuousGame = await fetchContinuousGameStartedAtSliceAsSystem();
  ...
  const activeQuestions = await listActiveQuestionRowsForGameTickAsSystem();
  ...
  if (activeQuestions.length === 0 && Date.now() < deadline) {
    ...
    currentActiveQuestions = await listActiveQuestionRowsForGameTickAsSystem();
  ...
  if (Date.now() < deadline) {
    try {
      ...
      const activePools = await listActivePoolIdAndNpcActorLimitAsSystem(10);
  ...
  await touchContinuousGameLastTickAsSystem({
    tickAt: timestamp,
  ...
  if (result.questionsResolved > 0) {
    ...
    const resolvedQuestions =
      await listRecentlyResolvedQuestionsForValidationAsSystem({
        updatedAfter: new Date(timestamp.getTime() - 60000),
  ...
  const activeMarkets = await listActivePredictionMarketsForValidationAsSystem({
    asOf: timestamp,
    limit: 10,
  });
...
async function bootstrapContentIfNeeded(_timestamp: Date): Promise<void> {
  const { trendingCount, relationshipCount } =
    await countTrendingTagsAndActorRelationshipsAsSystem();
  ...
  const finalCounts = await countTrendingTagsAndActorRelationshipsAsSystem();
  logger.info(
    'Bootstrap complete',
    {
      trendingCount: finalCounts.trendingCount,
      relationshipCount: finalCounts.relationshipCount,
    },
    'GameTick'
  );
}
...
async function bootstrapTrending(): Promise<void> {
  ...
  const { postCount, taggedPostCount } =
    await countPostsAndTaggedPostsJoinAsSystem();
  ...
  for (let i = 0; i < sampleTags.length; i++) {
    ...
    const existingTag = await fetchTagRowByNameAsSystem(tagData.name);
    ...
    if (existingTag) {
      ...
      const newTag = await insertTagRowReturningSliceAsSystem({
    ...
    await insertTrendingTagRowAsSystem({
      id: await generateSnowflakeId(),
      tagId: tag.id,
      score,
...
export async function updateMarketPricesFromTrades(
  ...
  const snapshots = await listPerpMarketSnapshotTickerOrgPriceAsSystem();
  ...
  const orgStates = await listOrganizationBasePriceByIdsAsSystem(orgIds);
  ...
  const positionsOpen =
    await listOpenPerpPositionSliceForTickersAsSystem(tickers);
...
export async function resolveQuestionPayouts(
  ...
  const question =
    await fetchQuestionRowByQuestionNumberAsSystem(questionNumber);

  if (!question) return;

  let market = await fetchMarketRowByIdAsSystem(question.id);

  if (!market) {
    market = await fetchMarketRowByQuestionTextAsSystem(question.text);
  }
  ...
  await withTransaction(async (tx) => {
    ...
    positionsSettled = await countPredictionPositionsForMarketInTx(
      tx,
      marketId
    );
    ...
    await markQuestionResolvedAfterPayoutInTx(tx, {
    ...
    await markTimeframedMarketResolvedForQuestionInTx(tx, {
      questionId: question.id,
      resolutionTimestamp,
    });
  });
  ...
  if (onChainResolutionTxHash) {
    await updateMarketOnChainResolutionFieldsAsSystem({
      marketId,
...
export async function publishOracleCommitments(
  ...
  await applyOracleCommitUpdatesForQuestionsAsSystem(
    result.successful.map((success) => ({
      questionId: success.questionId,
      sessionId: success.sessionId,
      commitment: success.commitment,
      txHash: success.txHash,
      blockNumber: success.blockNumber ?? null,
    }))
  );
  committed = result.successful.length;
...
export async function publishOracleReveals(
  ...
  await applyOracleRevealUpdatesForQuestionsAsSystem(
    result.successful.map((success) => ({
      questionId: success.questionId,
      txHash: success.txHash,
      blockNumber: success.blockNumber ?? null,
  ...
  revealed = result.successful.length;
...
async function updateWidgetCaches(): Promise<number> {
  ...
  const perpMarketsWithStats = await Promise.all(
    companies
      ...
      .map(async (company: (typeof companies)[number]) => {
        ...
        const priceHistory = await fetchStockPriceHistoryAsSystem(
          company.id,
          1440
        );
  ...
  const poolsList = await listActivePoolsForMarketsWidgetAsSystem();
  ...
  const activeMarketsList =
    await listActiveUnresolvedMarketsForMarketsWidgetAsSystem(new Date());
  ...
  await upsertMarketsWidgetCacheAsSystem(cacheData as JsonValue);
...
async function shouldUpdateWorldFacts(): Promise<boolean> {
  ...
  const lastCreatedAt =
    await fetchLatestAutoGeneratedWorldFactCreatedAtAsSystem();
  ...
  const timeSinceLastGeneration = Date.now() - lastCreatedAt.getTime();
  const shouldUpdate =
  ...
  try {
    ...
    if (factsGenerationSucceeded) {
      ...
      try {
        ...
        await insertWorldFactRowAsSystem({
          id: markerId,
...
export async function simulateMarketVolatility(options?: {
  ...
  try {
    ...
    const markets = await listPerpMarketSnapshotTickerOrgPriceAsSystem();
    ...
    if (priceUpdates.length > 0) {
      await batchUpdatePerpSnapshotCurrentPriceByTickerAsSystem(
        priceUpdates.map((u) => ({
          ticker: u.ticker,
          newPrice: u.newPrice,
        }))
...
async function processNarrativeArcs(
  ...
  const existingArcsList =
    questionIds.length > 0
      ? await listArcStateIdByQuestionIdsAsSystem(questionIds)
      : [];
