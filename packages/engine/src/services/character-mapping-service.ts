...
export class CharacterMappingService {
  ...
  /**
   * Set mock mappings for testing. This bypasses loadMappings() and allows
   * tests to inject specific mapping data without reading from StaticDataRegistry.
   */
  protected setTestMappings(
    characters: CharacterMapping[],
    organizations: OrganizationMapping[],
  ): void {
    this.characterMappingsCache = characters.map((m) => ({
      ...m,
      isActive: true,
    }))
    this.organizationMappingsCache = organizations.map((m) => ({
      ...m,
      isActive: true,
    }))
  ...
  private getReplacementForTerm(
    ...
    if (wordMap.has(searchLower)) {
      const mapped = wordMap.get(searchLower)
      if (mapped !== undefined) {
        return mapped
      }
    }
