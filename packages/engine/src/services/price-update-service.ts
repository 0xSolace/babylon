...
export class PriceUpdateService {
  ...
  static async applyUpdates(
    ...
    if (priceMap.size > 0) {
      ...
      // Broadcast price updates - best effort, non-critical
      // Import may fail if @babylon/api is not available in isolated engine context
      const api = await import('@babylon/api').catch(() => null);
      if (api) {
