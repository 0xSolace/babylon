...
export class SignalExtractionService {
  ...
  static async extractMarketSignal(
    ...
    const question = questionResult[0] as
      | { id: string; questionNumber: number; text: string }
      | undefined
    ...
    // Map posts with proper type coercion
    const postsList = postsResult
      ...
      .map((row) => ({
        ...
        dayNumber: row.dayNumber != null ? Number(row.dayNumber) : null,
        sentiment: row.sentiment != null ? Number(row.sentiment) : null,
        biasScore: row.biasScore != null ? Number(row.biasScore) : null,
    ...
    for (const post of postsList) {
      ...
      // Sentiment is a number from -1 to 1 (or null)
      ...
      if (Math.abs(sentimentValue) < 0.3) {
      ...
      const weight = reliability * clueStrength * Math.abs(sentimentValue)
      ...
      const isYesSignal = sentimentValue > 0
      ...
      const isYesPeriod = sentimentValue > 0
      if (day <= 10) {
