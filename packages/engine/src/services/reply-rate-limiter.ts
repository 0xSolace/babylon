...
export class ReplyRateLimiter {
  ...
  static async canReply(
    ...
    const lastTimestamp = lastInteraction.timestamp as Date
    const now = new Date()
    const timeSinceLastReply = now.getTime() - lastTimestamp.getTime()

    // Calculate expected next reply time using IDEAL_REPLY_INTERVAL_MS
    const expectedNextReply =
      ReplyRateLimiter.getExpectedNextReplyTime(lastTimestamp)

    // Too soon - need to wait
    if (timeSinceLastReply < ReplyRateLimiter.MIN_REPLY_INTERVAL_MS) {
      const nextAllowedAt = new Date(
        lastTimestamp.getTime() + ReplyRateLimiter.MIN_REPLY_INTERVAL_MS,
      )
  ...
  private static async calculateReplyStreak(
    ...
    for (let i = 0; i < interactions.length - 1; i++) {
      ...
      // Database returns Date objects, but TypeScript infers as unknown
      const currentTs =
        current.timestamp instanceof Date
          ? current.timestamp
          : new Date(current.timestamp as string | number)
      const previousTs =
        previous.timestamp instanceof Date
          ? previous.timestamp
          : new Date(previous.timestamp as string | number)
      const gap = currentTs.getTime() - previousTs.getTime()
  ...
  static async getReplyStats(userId: string, npcId: string) {
    ...
    const averageQuality =
      interactions.reduce((sum, i) => sum + Number(i.qualityScore), 0) /
      interactions.length

    // Calculate longest streak from all historical interactions
    const typedInteractions = interactions.map((i) => ({
      timestamp: i.timestamp as Date,
    }))
  ...
  static async getAllReplyStats(userId: string) {
    ...
    for (const interaction of interactions) {
      const npcId = String(interaction.npcId)
      if (!npcMap.has(npcId)) {
        npcMap.set(npcId, [])
      }
      npcMap.get(npcId)?.push(interaction)
    }
