...
Generate investments for ALL ${npcs.length} NPCs. Each NPC must have 2-5 investments totaling their target amount.`
  ...
  private static async executeInvestment(
    ...
    const currentPrice = orgState?.currentPrice
    ...
    if (!existingPool) {
      // Create Pool for NPC using CQL schema
      await db.pool
        .create({
          data: {
            id: investment.npcId,
            creatorId: investment.npcId,
    ...
    // Create position using CQL schema
    ...
    await db.poolPosition.create({
      data: {
        ...
        userId: investment.npcId, // NPC is the user for this position
        shares: shares.toString(),
        depositedAmount: investment.amount.toString(),
      },
    })
