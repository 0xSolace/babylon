...
export async function calculateTradeMetrics(
  ...
  // Fetch position with related data using SQLit
  ...
  const positionData = await db.position.findUnique({
    where: { id: positionId },
  })

  if (!positionData || !positionData.questionId) {
    return null
  }

  // Fetch question resolution date separately
  const questionData = await db.question.findUnique({
    where: { id: positionData.questionId },
  })

  const position: PositionWithQuestion = {
    id: positionData.id,
    userId: positionData.userId,
    questionId:
      typeof positionData.questionId === 'string'
        ? parseInt(positionData.questionId, 10)
        : (positionData.questionId ?? 0),
    outcome: Boolean(positionData.outcome),
    amount: positionData.amount ?? '0',
    pnl: positionData.pnl ?? null,
    createdAt: positionData.createdAt,
    resolvedAt: positionData.resolvedAt ?? null,
    questionResolutionDate: questionData?.resolutionDate ?? null,
  }
  ...
  const tradePosition: TradePosition = {
    ...
    questionId: position.questionId ?? 0,
