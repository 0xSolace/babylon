...
export async function buildRichGameContext(
  ...
  const [allEvents, allPosts, allQuestions, worldFacts] = await Promise.all([
    ...
    includeEventHistory
      ? (async () => {
          let query = db.select().from(worldEvents)
          if (gameId) {
...
function extractNarrativeThreads(
  ...
  for (const event of events) {
    ...
    if (!thread) {
      throw new Error(`Thread not found for key: ${key}`)
    }
    thread.events.push(event)
    event.actors.forEach((a) => {
      thread.actors.add(a)
    })
...
export function extractNarrativeThreadsWithLoopDetection(
  ...
  for (const event of events) {
    ...
    for (const actor of event.actors) {
      ...
      if (!actorCounts) {
        throw new Error(`Actor counts not found for actor: ${actor}`)
      }
