...
import type { Actor, FeedPost, Question, WorldEvent } from '@babylon/shared'
...
/** Extended FeedPost type that may include relatedEvent from generation context */
export interface FeedPostWithRelatedEvent extends FeedPost {
  relatedEvent?: string
}
...
export async function buildComprehensiveNPCContext(
  ...
  if (allPreviousEvents && allPreviousEvents.length > 0) {
    ...
    const eventListResult = await db
      .select()
      .from(worldEvents)
      .where(
        and(
    ...
    const eventList = eventListResult as Array<{
  ...
  if (previousPosts) {
    npcPreviousPostsRaw = (previousPosts as FeedPostWithRelatedEvent[])
