...
describe('Autonomous Agent - Complete E2E Test', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      throw new Error(
        `Server check failed: ${error}. Make sure server is running on ${SERVER_URL}`
      );
    }
  ...
  describe('Phase 3: Trading Actions', () => {
    ...
    it('should get positions after buying', async () => {
      ...
      const predictionPositions = positions.perpPositions.filter(
        (p): p is Record<string, unknown> & { marketId?: string } =>
          typeof p === 'object' && p !== null && typeof p.marketId === 'string'
      );
    ...
    it('should sell shares', async () => {
      ...
      // Get positions first
      const positions = await a2aClient.getPositions();
      const position = (positions.perpPositions as Array<Record<string, unknown>>).find(
        (p): p is Record<string, unknown> & { marketId?: string } =>
          typeof p === 'object' && p !== null && p.marketId === testMarketId
      );
