...
export type BabylonA2AClientConfig = {
...
export class BabylonA2AClient {
  ...
  async connect(): Promise<void> {
    try {
      ...
      // Get agent card
      this.agentCard =
        (await (client as unknown as { agentCardPromise?: Promise<AgentCard> }).agentCardPromise) ||
        null;
      ...
      throw new Error(
        `Failed to connect to Babylon A2A: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async sellShares(positionId: string, shares: number): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Sell ${shares} shares from position ${positionId}`, {
  ...
  async getPredictions(params?: {
  ...
  }): Promise<{ predictions: Array<Record<string, unknown>> }> {
    const response = await this.sendMessage('What prediction markets are available?', {
    ...
    if ('status' in response) {
      ...
      return {
        predictions: (result.predictions as Array<Record<string, unknown>>) || [],
      };
    }
   ...
   */
  async getPerpetuals(): Promise<{
    perpetuals: Array<Record<string, unknown>>;
  }> {
    const response = await this.sendMessage('What perpetual futures markets are available?', {
    ...
    if ('status' in response) {
      ...
      const result = this.extractResult(task);
      return {
        perpetuals: (result.perpetuals as Array<Record<string, unknown>>) || [],
      };
    }
  ...
  async getMarkets(): Promise<{
    ...
    perps: Array<Record<string, unknown>>;
  }> {
  ...
  async getPositions(userId?: string): Promise<{
    perpPositions: Array<Record<string, unknown>>;
    totalPnL: number;
  }> {
    const response = await this.sendMessage(
  ...
  async getFeed(params?: {
    ...
    following?: boolean;
    type?: 'post' | 'article';
  }): Promise<{ posts: Array<Record<string, unknown>> }> {
    const response = await this.sendMessage('Show me recent posts from the feed', {
  ...
  async createComment(postId: string, content: string): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Comment on post ${postId}: ${content}`, {
  ...
  async sendMessageToChat(chatId: string, content: string): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Send message to chat ${chatId}: ${content}`, {
  ...
  ): Promise<{ chats: Array<Record<string, unknown>> }> {
  ...
  async getLeaderboard(params?: {
    ...
    if ('status' in response) {
      ...
      const result = this.extractResult(task);
      return {
        leaderboard: (result.leaderboard as Array<Record<string, unknown>>) || [],
      };
    }
  ...
  async getUserProfile(userId: string): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Show me user ${userId}'s profile`, {
  ...
  async getOrganizations(
    ...
    const response = await this.sendMessage('What organizations/perpetual markets are available?', {
    ...
    if ('status' in response) {
      ...
      const result = this.extractResult(task);
      return {
        organizations: (result.organizations as Array<Record<string, unknown>>) || [],
      };
    }
  ...
  async reportUser(params: {
    ...
    const response = await this.sendMessage(`Report user ${params.userId} for ${params.category}`, {
  ...
  async reportPost(params: {
    ...
    const response = await this.sendMessage(`Report post ${params.postId} for ${params.category}`, {
  ...
  async checkBlockStatus(params: { userId: string }): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Check if user ${params.userId} is blocked`, {
  ...
  async checkMuteStatus(params: { userId: string }): Promise<Record<string, JsonValue>> {
    const response = await this.sendMessage(`Check if user ${params.userId} is muted`, {
