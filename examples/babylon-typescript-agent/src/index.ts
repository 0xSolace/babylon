...
import { AgentDecisionMaker, type DecisionContext } from './decision';
...
async function main() {
  ...
  const runTick = async () => {
    ...
    const decision = await decisionMaker.decide({
      portfolio: {
        ...
        positions: portfolio.positions as unknown as Array<{
          id: string;
          ticker: string;
          side: 'long' | 'short';
          size: number;
  ...
  // Then loop
  const interval = setInterval(runTick, parseInt(process.env.TICK_INTERVAL || '30000', 10));
