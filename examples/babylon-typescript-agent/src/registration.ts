...
const requireEnvVar = (key: string): string => {
  ...
  if (!value) {
    throw new Error(`Environment variable ${key} is required for agent registration`);
  }
  return value;
};
...
export async function registerAgent(): Promise<AgentIdentity> {
  ...
  const rpcUrl = requireEnvVar('AGENT0_RPC_URL');
  const privateKey = requireEnvVar('AGENT0_PRIVATE_KEY');
  ...
  const tokenIdPart = parts[1];
  if (!tokenIdPart) {
    throw new Error(`Invalid agentId format: ${agentId}`);
  }
  const tokenId = parseInt(tokenIdPart, 10);
