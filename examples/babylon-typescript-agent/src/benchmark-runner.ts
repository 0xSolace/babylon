...
async function runBenchmark(benchmarkFile: string, outputDir: string): Promise<SimulationResult> {
  ...
  while (engine.getGameState().tick < totalTicks) {
    ...
    await (async () => {
      ...
      const portfolioResponse = (await a2aInterface.sendRequest('a2a.getPortfolio')) as {
        balance: number;
      ...
      const predictionsResponse = (await a2aInterface.sendRequest('a2a.getPredictions')) as {
        predictions: PredictionMarket[];
      };
      const perpetualsResponse = (await a2aInterface.sendRequest('a2a.getPerpetuals')) as {
  ...
  fs.writeFileSync(path.join(outputDir, 'metrics.json'), JSON.stringify(result.metrics, null, 2));
...
async function runMultiple(benchmarkFile: string, outputDir: string, runs: number) {
