...
function getCurrentLevel(points: number): TrustLevelInfo {
  const level = TRUST_LEVELS.find((level) => points >= level.min && points <= level.max);
        ...
        <div className={`relative ${sizeClasses[size]}`}>
          ...
          {currentLevel.level === 'elite' && (
            <div className="absolute inset-0 animate-pulse">
              <Icon className={`${sizeClasses[size]} ${color} opacity-50`} fill="currentColor" />
            </div>
          )}
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className={`font-semibold ${color} ${textSizeClasses[size]}`}>{label}</span>
            <span className="text-gray-500 text-xs">{reputationPoints.toLocaleString()} pts</span>
          </div>
        </div>
      </div>
