...
import { AlertCircle, MessageSquare, Star, TrendingDown, TrendingUp } from 'lucide-react';
    ...
    <div className={`rounded-lg bg-sidebar p-4 ${className}`}>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-foreground text-xl">Feedback History</h2>
        </div>
        {!loading && feedbackItems.length > 0 && (
          <div className="flex items-center gap-1 text-muted-foreground text-sm">
            <Star className="h-4 w-4 text-yellow-500" fill="currentColor" />
            <span className="font-semibold">{getStarRating(averageScore).toFixed(1)}</span>
            <span>/5</span>
          </div>
        )}
      </div>
                ...
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {feedback.isAutoGenerated ? (
                      ...
                      <Link
                        href={getProfileUrl(feedback.fromUserId || '', feedback.fromUsername)}
...
export function FeedbackSummaryCard({ userId, className = '' }: FeedbackSummaryCardProps) {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      const response = await fetch(`/api/feedback/stats/${encodeURIComponent(userId)}`);
      const data = await response.json();
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="mb-1 flex items-center gap-1">
            ...
            <span className="font-bold text-2xl text-foreground">{starRating.toFixed(1)}</span>
