...
import { Activity, AlertCircle, BarChart, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
export function NPCPortfolioCard({
  ...
  if (!data) {
    return (
      <div className={cn('rounded-2xl bg-sidebar px-4 py-3', className)}>
        <div className="text-muted-foreground text-sm">Portfolio data unavailable</div>
      </div>
    );
  }
  ...
  return (
    <div className={cn('space-y-4 rounded-2xl bg-sidebar px-4 py-3', className)}>
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <div className={cn('font-medium text-sm', getRiskColor(portfolio.riskScore))}>
      ...
      <div className="rounded-lg bg-muted/30 px-4 py-3">
        <div className="mb-1 text-muted-foreground text-xs">Total Portfolio Value</div>
            ...
            {positions.map((position) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-muted-foreground uppercase">{position.side}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-muted-foreground">${position.size.toLocaleString()}</span>
