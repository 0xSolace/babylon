...
export function GroupManagementModal({
  ...
  useEffect(() => {
    ...
    const searchUsers = async () => {
      ...
      if (response.ok) {
        ...
        setSearchResults((data.users || []).filter((u: User) => !existingMemberIds.includes(u.id)));
      }
      setSearching(false);
    };
  ...
  return (
    <>
      ...
      >
        <div className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-xl border border-border bg-background shadow-2xl">
          {/* Header */}
          <div className="flex shrink-0 items-center justify-between border-border border-b p-6">
            <div className="flex items-center gap-2">
              <Settings className="h-5 w-5 text-primary" />
              <h2 className="font-bold text-xl">{groupDetails?.name || 'Group Settings'}</h2>
            </div>
            <button
              type="button"
              onClick={handleClose}
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setConfirmAction({
                                      ...
                                      userId: member.id,
                                      userName:
                                        member.displayName || member.username || 'this user',
                                    })
                                  }
                                  disabled={!!actionLoading}
          ...
          <div className="w-full max-w-sm rounded-xl border border-border bg-background shadow-2xl">
            <div className="space-y-4 p-6">
              ...
              <div className="flex gap-3">
                ...
                >
                  {actionLoading ? <Loader2 className="inline h-4 w-4 animate-spin" /> : 'Confirm'}
                </button>
              </div>
            </div>
          </div>
