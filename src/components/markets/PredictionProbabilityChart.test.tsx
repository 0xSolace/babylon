...
describe('PredictionProbabilityChart - Data Processing', () => {
  ...
  describe('Data Validation', () => {
    it('should handle prices at 0%', () => {
      const data = [{ time: Date.now(), yesPrice: 0, noPrice: 1, volume: 1000 }];
      const [point] = data;

      expect(point!.yesPrice * 100).toBe(0);
      expect(point!.noPrice * 100).toBe(100);
    });

    it('should handle prices at 100%', () => {
      const data = [{ time: Date.now(), yesPrice: 1, noPrice: 0, volume: 1000 }];
      const [point] = data;

      expect(point!.yesPrice * 100).toBe(100);
      expect(point!.noPrice * 100).toBe(0);
    });

    it('should handle negative volumes gracefully', () => {
      const data = [{ time: Date.now(), yesPrice: 0.5, noPrice: 0.5, volume: -1000 }];
      const [point] = data;

      expect(point!.volume).toBe(-1000);
      expect(point!.yesPrice).toBeGreaterThanOrEqual(0);
      expect(point!.noPrice).toBeGreaterThanOrEqual(0);
    });

    it('should handle very old timestamps', () => {
      const oldTime = new Date('2020-01-01').getTime();
      const data = [{ time: oldTime, yesPrice: 0.5, noPrice: 0.5, volume: 1000 }];
      const [point] = data;

      expect(point!.time).toBe(oldTime);
      expect(point!.time).toBeLessThan(Date.now());
    });

    it('should handle future timestamps', () => {
      const futureTime = Date.now() + 365 * 24 * 60 * 60 * 1000; // 1 year ahead
      const data = [{ time: futureTime, yesPrice: 0.5, noPrice: 0.5, volume: 1000 }];
      const [point] = data;

      expect(point!.time).toBe(futureTime);
      expect(point!.time).toBeGreaterThan(Date.now());
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle unsorted data', () => {
      ...
      const future = data[2]!;
      expect(previous.time).toBeLessThan(latest.time);
      expect(future.time).toBeGreaterThan(latest.time);
    });
