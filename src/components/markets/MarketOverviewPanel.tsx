...
type PredictionQuestionSummary = {
  id: string | number;
...
export function MarketOverviewPanel() {
  ...
  useEffect(() => {
    const fetchOverview = async () => {
      ...
      if (data.markets && Array.isArray(data.markets)) {
        ...
        const avgChange =
          markets.length > 0
            ? markets.reduce((sum: number, m: PerpMarket) => sum + (m.changePercent24h || 0), 0) /
              markets.length
            : 0;
  ...
  const predictionOverview = useMemo(() => {
    ...
    const totalVolume = active.reduce((sum, entry) => sum + entry.yesShares + entry.noShares, 0);
  ...
  return (
    <div className="flex flex-1 flex-col rounded-2xl bg-sidebar p-4">
      ...
      {loading ? (
        ...
        <div className="flex-1 space-y-3">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-foreground text-sm">{overview.totalMarkets}</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-muted-foreground text-sm">Open Interest</span>
              </div>
              <span className="font-semibold text-foreground text-sm">
          ...
          <div className="border-border border-t pt-2">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-muted-foreground text-sm">Avg Change 24h</span>
              <div
                className={cn(
          ...
          <div className="space-y-2 border-border border-t pt-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-foreground text-sm">Prediction Markets</span>
            </div>
            <div className="flex items-center justify-between">
        ...
        <div className="flex-1 text-muted-foreground text-sm">No data available</div>
      )}
    </div>
  );
}
