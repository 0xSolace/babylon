...
export function PerpPriceChart({
  ...
  const priceChangePercent =
    chartData.length > 1 && chartData[0]?.price ? (priceChange / chartData[0].price) * 100 : 0;
    ...
    <div className="w-full space-y-2">
      ...
      <div className="flex flex-col items-start justify-between gap-3 px-3 sm:flex-row sm:items-center">
        <div className="flex items-center gap-3">
          <div>
            <div className="font-bold text-2xl">{formatValue(currentPrice, true)}</div>
            <div className={`text-sm ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
              {isPositive ? '↑' : '↓'} {formatValue(Math.abs(priceChange), true)} (
