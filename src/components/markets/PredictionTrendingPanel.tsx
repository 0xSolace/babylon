...
type PredictionTrendingPanelProps = {
  ...
  return (
    <div className="flex flex-1 flex-col rounded-2xl bg-sidebar px-4 py-3">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="font-semibold text-foreground text-sm">Hot Predictions</h2>
      </div>
      {loading ? (
        ...
        <div className="text-muted-foreground text-xs">No active prediction markets</div>
      ) : (
        <div className="flex flex-1 flex-col gap-2">
          {sortedMarkets.map((market) => (
            ...
            >
              <div className="line-clamp-2 font-medium text-foreground text-sm">{market.text}</div>
              <div className="flex items-center justify-between pt-1 text-muted-foreground text-xs">
                <span>Volume: {(market.yesShares + market.noShares).toFixed(0)} shares</span>
