...
type FarcasterMiniAppContextType = {
  isMiniApp: boolean;
  isLoading: boolean;
  error?: string;
  fid?: number;
...
export function FarcasterMiniAppProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [authenticated, createWallet, hasEmbeddedWallet, isCreatingWallet, isMiniApp, ready, user]);
  ...
  return (
    <FarcasterMiniAppContext.Provider value={value}>{children}</FarcasterMiniAppContext.Provider>
  );
}
