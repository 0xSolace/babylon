...
export function AgentLogs({ agentId }: AgentLogsProps) {
  ...
  }, [agentId, getAccessToken, levelFilter, typeFilter]);
      ...
      <div className="rounded-lg border border-border bg-card/50 p-4 backdrop-blur">
        {logs.length === 0 ? (
          ...
          <div className="space-y-2">
            {logs.map((log) => (
              <div key={log.id} className={cn('rounded-lg border p-3', getTypeColor(log.type))}>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-muted-foreground text-xs uppercase">{log.type}</span>
                      <span className="text-muted-foreground text-xs">•</span>
                    ...
                    {expanded.has(log.id) && (
                      <div className="mt-3 space-y-2 text-xs">
                        ...
                        {log.thinking && (
                          <div>
                            <div className="mb-1 font-medium text-muted-foreground">Thinking:</div>
                            <pre className="overflow-x-auto whitespace-pre-wrap rounded bg-black/30 p-2">
                              {log.thinking}
                            </pre>
                          </div>
