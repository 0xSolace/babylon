...
export function AgentChat({ agent, onUpdate }: AgentChatProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const error = (await res.json().catch(() => ({ error: 'Failed to send message' }))) as {
          error: string;
        };
    ...
    <div className="flex h-[600px] flex-col rounded-lg border border-border bg-card/50 backdrop-blur">
      ...
      <div className="flex items-center justify-between border-border border-b p-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{agent.pointsBalance} points available</p>
        </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {loading && messages.length === 0 ? (
          <div className="py-12 text-center text-muted-foreground">Loading chat history...</div>
        ) : messages.length === 0 ? (
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  message.role === 'user' ? 'bg-[#0066FF] text-primary-foreground' : 'bg-muted'
                )}
              >
