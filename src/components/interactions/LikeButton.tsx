...
export function LikeButton({
  ...
  const { toggleLike, toggleCommentLike, postInteractions, commentInteractions, loadingStates } =
    useInteractionStore();
  const { trackPostLike } = useSocialTracking();

  // Get state from store instead of local state
  const storeData =
    targetType === 'post' ? postInteractions.get(targetId) : commentInteractions.get(targetId);
      ...
      <div className="relative">
        ...
        >
          {isLoading ? (
            ...
            <FallbackIcon size={iconSizes[size]} className="transition-all duration-200" />
          )}
