    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <Repeat2
            size={iconSizes[size]}
            className={cn('transition-all duration-200', isAnimating && 'rotate-180')}
          />
        )}
        {showCount && count > 0 && <span className="font-medium tabular-nums">{count}</span>}
      </button>
      ...
      {showConfirmation && (
        <>
          ...
          {/* Modal - Mobile */}
          <div className="fixed inset-x-4 top-20 bottom-auto z-50 flex max-h-[85vh] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#050816] shadow-2xl md:hidden">
            {/* Header */}
            <div className="flex items-center justify-between border-white/10 border-b px-6 py-4">
              ...
              >
                {isLoading ? <output aria-live="polite">Posting...</output> : 'Post'}
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              ...
              {postData && (
                <div className={cn('mt-4 rounded-xl border border-white/10 p-4', 'bg-white/5')}>
                  {/* Original Post Author */}
                  <div className="mb-3 flex items-start gap-3">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10 font-semibold text-foreground text-sm">
          ...
          <div className="fixed inset-0 z-50 hidden items-center justify-center p-4 md:flex">
            <div className="flex max-h-[85vh] w-full max-w-[580px] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#050816] shadow-2xl">
              {/* Header */}
              <div className="flex items-center justify-between border-white/10 border-b px-6 py-4">
                <div className="flex items-center gap-3">
                  <button
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6">
                ...
                <textarea
                  ...
                  className={cn(
                    ...
                    'border-0 bg-transparent',
                    'text-base text-primary-foreground placeholder:text-primary-foreground/40',
                    'resize-none focus:outline-none',
                    'transition-colors'
                  )}
                />
                ...
                {/* Original Post Preview */}
                {postData && (
                  <div className={cn('mt-4 rounded-xl border border-white/10 p-5', 'bg-white/5')}>
                    {/* Original Post Author */}
                    <div className="mb-3 flex items-start gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 font-semibold text-primary-foreground">
                        {postData.authorName[0]?.toUpperCase() || '?'}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                    ...
                    {/* Original Post Content */}
                    <p className="whitespace-pre-wrap break-words text-primary-foreground/90 leading-relaxed">
                      {postData.content}
                    </p>
                  </div>
                )}
