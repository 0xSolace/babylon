...
import { AlertCircle, CheckCircle, Database, RefreshCw, TrendingUp } from 'lucide-react';
...
export function TrainingDataTab() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let result: TrainingDataResponse;
      try {
        result = (await response.json()) as TrainingDataResponse;
      } catch (parseError) {
        logger.error(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-bold text-2xl">{data.summary.totalTrajectories}</span>
          </div>
        ...
        <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-bold text-2xl">{data.summary.totalWindows}</span>
          </div>
        ...
        <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-bold text-2xl">{data.summary.readyWindows}</span>
          </div>
          <div className="text-muted-foreground text-sm">Ready for Training</div>
        ...
        <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
          <div className="mb-2 flex items-center justify-between">
            <AlertCircle
              className={cn('h-5 w-5', getQualityColor(data.qualityMetrics.trainingDataQuality))}
            />
            <span
              className={cn(
      ...
      <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div className="mb-1 text-muted-foreground text-sm">Avg Episode Length</div>
        ...
        {data.recentTrajectories.length > 0 ? (
          <div className="max-h-96 space-y-2 overflow-y-auto">
            {data.recentTrajectories.map((traj) => (
              <div key={traj.id} className="rounded-lg border border-border bg-accent/50 p-3">
          ...
          <div className="py-8 text-center text-muted-foreground">No trajectories recorded yet</div>
        )}
      </div>
        ...
        <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-6">
          <div className="flex gap-3">
            ...
            <div className="text-sm">
              <p className="mb-2 font-medium text-yellow-200">No Training Data Collected</p>
      ...
      {data.summary.readyWindows === 0 && data.summary.totalTrajectories > 0 && (
        <div className="rounded-lg border border-blue-500/20 bg-blue-500/10 p-6">
          <div className="flex gap-3">
            ...
            <div className="text-sm">
              <p className="mb-2 font-medium text-blue-200">Not Ready for Training</p>
              <p className="text-blue-200/80">
                Need at least {data.summary.minAgentsRequired} trajectories per window for GRPO
                training. Current windows have fewer agents. Run more benchmarks or wait for more
                game ticks.
              </p>
            </div>
          </div>
      ...
      {data.summary.readyWindows > 0 && (
        <div className="rounded-lg border border-green-500/20 bg-green-500/10 p-6">
          <div className="flex gap-3">
            ...
            <div className="text-sm">
              <p className="mb-2 font-medium text-green-200">Ready for Training!</p>
              <p className="mb-3 text-green-200/80">
                {data.summary.readyWindows} window
                {data.summary.readyWindows > 1 ? 's' : ''} ready with sufficient data. You can now
                run RL training:
              </p>
