...
import { AlertCircle, Bot, Check, RefreshCw, Sparkles, Zap } from 'lucide-react';
...
export function AIModelsTab() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let result: AIModelsResponse;
      try {
        result = (await response.json()) as AIModelsResponse;
      } catch (parseError) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      let result: AIModelTestSuccess & AIModelTestError;
      try {
        result = (await response.json()) as AIModelTestSuccess & AIModelTestError;
      } catch (parseError) {
      ...
      if (response.ok && result.data) {
        ...
        toast.success(`Test successful! Using ${result.data.provider ?? 'selected provider'}`);
      } else {
        const errorMessage = result.error || 'Test failed';
        toast.error(errorMessage);
        setTestResult({ error: errorMessage, details: result.details });
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-3">
          ...
          {data.activeProvider === 'wandb' && data.currentSettings.wandbModel && (
            <div className="text-muted-foreground text-sm">
              Model:{' '}
              <span className="font-mono text-foreground">{data.currentSettings.wandbModel}</span>
            </div>
          )}
        </div>
      </div>
          ...
          {Object.entries(data.providers).map(([provider, available]) => (
            <div
              ...
              className={cn(
                ...
                available ? 'border-green-500/30 bg-green-500/5' : 'border-border bg-muted/20'
              )}
            >
              <div className="flex items-center justify-between">
        ...
        <div className="rounded-lg border border-border bg-card/50 p-6 backdrop-blur">
          <div className="mb-4 flex items-center justify-between">
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <span className="text-muted-foreground text-sm">Enable Wandb</span>
              <input
                type="checkbox"
                    ...
                    {data.wandbModels.map((model) => (
                      <button
                        ...
                        className={cn(
                          ...
                          selectedModel === model.id ? 'bg-primary/10' : 'hover:bg-accent'
                        )}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex-1 truncate font-mono text-sm">{model.id}</div>
                          {selectedModel === model.id && (
            ...
            <div className="flex gap-3">
              ...
              <div className="text-blue-200 text-sm">
                ...
                <p className="text-blue-200/80">
                  Weights & Biases provides access to open-source models with fast inference. When
                  enabled, all serverless agents will use the selected model for decision making.
        ...
        <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-6">
          <div className="flex gap-3">
            ...
            <div className="text-sm">
              <p className="mb-2 font-medium text-yellow-200">Weights & Biases Not Configured</p>
          ...
          {!testResult.error ? (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium font-mono">{String(testResult.latency || 0)}ms</span>
              </div>
