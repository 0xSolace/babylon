...
import { Calendar, MessageCircle, RefreshCw, Search, User as UserIcon, Users } from 'lucide-react';
...
export function GroupsTab() {
  ...
  const fetchGroups = useCallback(async () => {
    startRefresh(async () => {
      ...
      const response = await fetch(`/api/admin/groups?sortBy=${sortBy}&sortOrder=${sortOrder}`, {
        headers: {
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {filteredGroups.map((group) => (
            <button
              ...
              className={cn(
                'w-full space-y-3 rounded-lg border border-border bg-card p-4 text-left transition-colors',
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <h3 className="truncate font-semibold">{group.name || 'Unnamed Group'}</h3>
              ...
              <div className="border-border border-t pt-2">
                <p className="mb-2 text-muted-foreground text-xs">Participants:</p>
      ...
      {selectedGroup && (
        <div className="space-y-4 rounded-lg border border-primary bg-card p-6">
          ...
          <div>
            <h4 className="mb-2 font-medium">All Participants ({selectedGroup.memberCount})</h4>
            ...
            <div>
              ...
              <div className="max-h-64 space-y-2 overflow-y-auto">
                {selectedGroup.recentMessages.map((message) => (
                  <div key={message.id} className="space-y-1 rounded bg-muted/50 p-3">
                    <div className="flex items-center justify-between">
