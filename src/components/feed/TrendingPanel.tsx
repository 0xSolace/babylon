...
export function TrendingPanel() {
  ...
  return (
    <div className="flex flex-1 flex-col rounded-2xl bg-sidebar p-4">
      <h2 className="mb-3 text-left font-bold text-foreground text-lg">Trending</h2>
      {loading ? (
        ...
        <div className="flex-1 space-y-2 pl-3">
          {trending.map((item) => (
            <button
              ...
              aria-label={`View trending topic ${item.tag}`}
            >
              <div className="min-w-0 flex-1">
                ...
                {/* Tag name */}
                <p className="font-semibold text-foreground text-sm leading-snug">{item.tag}</p>
                {/* Summary */}
                {item.summary && (
                  <p className="line-clamp-1 text-muted-foreground text-xs">{item.summary}</p>
                )}
              </div>
            </button>
          ))}
