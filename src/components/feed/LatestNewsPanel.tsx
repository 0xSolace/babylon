...
export function LatestNewsPanel() {
  ...
  const fetchArticles = useCallback(
    async (skipCache = false) => {
      ...
      if (data.posts && Array.isArray(data.posts) && data.posts.length > 0) {
        ...
        logger.info(
          'Articles processed:',
          { count: articlesData.length, articles: articlesData },
          'LatestNewsPanel'
        );
  ...
  return (
    <div className="flex flex-1 flex-col rounded-2xl bg-sidebar p-4">
      <h2 className="mb-3 text-left font-bold text-foreground text-lg">Latest News</h2>
      {loading ? (
        ...
        <div className="flex-1 pl-3 text-muted-foreground text-sm">No articles available yet.</div>
      ) : (
        <div className="flex-1 space-y-2 pl-3">
          {articles.map((article) => (
            <button
              ...
              className="-ml-1.5 flex w-full items-start gap-3 rounded-lg p-1.5 text-left transition-colors duration-200 hover:bg-muted/50"
              aria-label={`View article ${article.title}`}
            >
              <div className="mt-0.5 shrink-0">{getSentimentIcon(article.sentiment)}</div>
