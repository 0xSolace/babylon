...
function MobileHeaderContent() {
  ...
  const isProduction =
    typeof window !== 'undefined' && window.location.hostname === 'babylon.market';
  ...
  }, [authenticated, setUser, user?.id, user?.profileImageUrl, user?.coverImageUrl, user]);
  ...
  return (
    <>
      <header className={cn('md:hidden', 'fixed top-0 right-0 left-0 z-40', 'bg-sidebar/95')}>
        <div className="flex h-14 items-center justify-between px-4">
          ...
          <div className="-translate-x-1/2 absolute left-1/2 transform">
            <Link href="/feed" className="transition-transform duration-300 hover:scale-105">
              <Image
