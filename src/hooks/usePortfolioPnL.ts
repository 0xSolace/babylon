...
export function usePortfolioPnL(): UsePortfolioPnLResult {
  ...
  const refresh = useCallback(async () => {
    ...
    type PositionsResponse = {
      perpetuals?: {
        positions?: Array<{ unrealizedPnL?: number | string }>;
      };
      predictions?: {
        positions?: Array<{ unrealizedPnL?: number | string }>;
      };
    };
    let balanceJson: BalanceResponse | null = null;
    let positionsJson: PositionsResponse | null = null;
    try {
      balanceJson = await balanceRes.json();
      positionsJson = await positionsRes.json();
    } catch (error) {
      logger.error(
        'Failed to parse portfolio PnL response',
    ...
    const perpPositions: Array<{ unrealizedPnL?: number | string }> =
    ...
    const predictionPositions: Array<{ unrealizedPnL?: number | string }> =
      positionsJson?.predictions?.positions ?? [];
    const predictionUnrealized = predictionPositions.reduce(
      (sum, position) => sum + toNumber(position?.unrealizedPnL),
      0
    );
