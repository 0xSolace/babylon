...
const getGlobal = (): GlobalSSEType =>
  globalThis as typeof globalThis & GlobalSSEType;
...
export function useSSE(options: SSEHookOptions = {}): SSEHookReturn {
    ...
    eventSource.onerror = () => {
      ...
      if (eventSource.readyState === EventSource.CLOSED) {
        ...
        if (
          ...
          logger.debug(
            `Reconnecting SSE in ${Math.round(delay)}ms... (attempt ${reconnectAttemptsRef.current + 1}/${maxReconnectAttempts})`,
            undefined,
            'useSSE'
          );
  ...
  const disconnect = useCallback(() => {
    ...
    const g = getGlobal();
    g.__babylon_sse_subscribers__ = Math.max(
      0,
      (g.__babylon_sse_subscribers__ || 0) - 1
    );
    ...
    if (g.__babylon_sse_subscribers__ === 0 && g.__babylon_sse_source__) {
      ...
      logger.debug(
        'SSE connection closed (no more subscribers)',
        undefined,
        'useSSE'
      );
    }
