...
export async function GET(_request: NextRequest) {
  ...
  const result =
    await prisma.$queryRawUnsafe<Array<{ version: string | null }>>('SELECT version()');
  const rawVersion = result[0]?.version ?? 'unknown';
  checks.prismaVersion = rawVersion.split(' ')[0] ?? 'unknown';
