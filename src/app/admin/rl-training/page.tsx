  ...
  return (
    <div className="container mx-auto space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-3xl tracking-tight">RL Training Dashboard</h1>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">{benchmarkSummary?.totalBenchmarked || 0}</div>
            <p className="text-muted-foreground text-xs">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="font-medium text-sm">Training Bundles</CardTitle>
            <CheckCircle className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="font-bold text-2xl">{modelSelection?.summary.bundleCount || 0}</div>
            <p className="text-muted-foreground text-xs">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="font-medium text-sm">Training Status</CardTitle>
        ...
        <TabsContent value="models" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>All models trained through the RL system</CardDescription>
            </CardHeader>
            <CardContent>
              {models.length === 0 ? (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="mt-1 text-muted-foreground text-sm">
                          ...
                          {model.benchmarkScore && <> • Score: {model.benchmarkScore.toFixed(3)}</>}
                          {model.avgReward && <> • Avg Reward: {model.avgReward.toFixed(3)}</>}
                        </div>
            ...
            <Card>
              ...
              <CardContent>
                {benchmarkSummary?.recentModels.length === 0 ? (
                  <div className="py-8 text-center text-muted-foreground">No recent benchmarks</div>
                ) : (
                  <div className="space-y-3">
                    {benchmarkSummary?.recentModels.slice(0, 5).map((model) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="font-semibold">{model.score?.toFixed(3) || 'N/A'}</div>
                          <div className="text-muted-foreground text-sm">
        ...
        <TabsContent value="selection" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Model selection strategy for the next training run</CardDescription>
            </CardHeader>
                ...
                <>
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="rounded-lg border p-4">
                      <div className="text-muted-foreground text-sm">Training Bundles</div>
                      <div className="font-bold text-2xl">{modelSelection.summary.bundleCount}</div>
                    </div>
                    <div className="rounded-lg border p-4">
                      <div className="text-muted-foreground text-sm">Trained Models</div>
                      <div className="font-bold text-2xl">
                        {modelSelection.summary.trainedModelCount}
                      </div>
                    </div>
                    <div className="rounded-lg border p-4">
                      <div className="text-muted-foreground text-sm">Best Score</div>
                      <div className="font-bold text-2xl">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>{modelSelection.summary.recommendation}</AlertDescription>
                  </Alert>
              ...
              {trainingStatus && (
                <div className="space-y-4">
                  <Alert variant={trainingStatus.ready ? 'default' : 'destructive'}>
                    ...
                    <AlertTitle>{trainingStatus.ready ? 'Ready to Train' : 'Not Ready'}</AlertTitle>
                    <AlertDescription>{trainingStatus.reason}</AlertDescription>
                  </Alert>

                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded border p-3">
                      <div className="text-muted-foreground text-sm">Total Trajectories</div>
                      <div className="font-bold text-xl">
                        {trainingStatus.stats.totalTrajectories}
                      </div>
                    </div>
                    <div className="rounded border p-3">
                      <div className="text-muted-foreground text-sm">Unscored</div>
                      <div className="font-bold text-xl">
                        {trainingStatus.stats.unscoredTrajectories}
                      </div>
                    </div>
                    <div className="rounded border p-3">
                      <div className="text-muted-foreground text-sm">Scenario Groups</div>
                      <div className="font-bold text-xl">{trainingStatus.stats.scenarioGroups}</div>
                    </div>
                    <div className="rounded border p-3">
        ...
        <TabsContent value="actions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manually trigger training and other operations</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="rounded-lg border p-4">
                ...
                <div className="flex gap-2">
                  <Button onClick={() => triggerTraining(false)} disabled={!trainingStatus?.ready}>
              ...
              <div className="rounded-lg border p-4">
                ...
                <div className="space-y-2">
                  ...
                  {models.filter((m) => !m.benchmarkScore && m.status === 'ready').length === 0 && (
                    <div className="text-muted-foreground text-sm">No models need benchmarking</div>
                  )}
                </div>
              </div>
            </CardContent>
