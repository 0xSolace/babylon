...
import { FeedSkeleton, ProfileHeaderSkeleton } from '@/components/shared/Skeleton';
...
export default function ActorProfilePage() {
  ...
  useLayoutEffect(() => {
    if (authenticated && user?.username && !isUsernameParam) {
      ...
      const viewingOwnId =
        user.id === actorId || user.id === decodedIdentifier || user.id === identifier;
  ...
  const loadActorInfo = useCallback(async () => {
    ...
    const userResponse = await fetch(`/api/users/${encodeURIComponent(actorId)}/profile`, {
      headers,
    }).catch((error: Error) => {
    ...
    if (
      isUsernameParam ||
      (!actorId.startsWith('did:privy:') && actorId.length <= 42 && !actorId.includes('-'))
    ) {
    ...
    if (actor) {
      ...
      const statsResponse = await fetch(`/api/actors/${encodeURIComponent(actor.id)}/stats`).catch(
        (error: Error) => {
      ...
      setActorInfo({
        ...
        username: ('username' in actor ? (actor.username as string) : actor.id) as
          | string
          | undefined, // Use username if available, fallback to ID
        stats,
      });
      setLoading(false);
      return;
    ...
    if (org) {
      ...
      let stats = { followers: 0, following: 0, posts: 0 };
      const statsResponse = await fetch(`/api/actors/${encodeURIComponent(org.id)}/stats`).catch(
        (error: Error) => {
    ...
    setActorInfo(null);
    setLoading(false);
  }, [actorId, allGames, isOwnProfile, isUsernameParam, router, getAccessToken]);
      ...
      if (!apiPostIds.has(gamePost.post.id)) {
        combined.push({
          ...
          post: {
            ...gamePost.post,
            authorProfileImageUrl:
              gamePost.post.authorProfileImageUrl || actorInfo?.profileImageUrl || null,
          },
        });
      }
    });
  ...
  const filteredPosts = useMemo(() => {
    ...
    return tabFilteredPosts.filter((item) => item.post.content?.toLowerCase().includes(query));
      ...
      <PageContainer noPadding className="flex flex-col">
        <div className="sticky top-0 z-10 bg-background">
          <div className="flex items-center gap-4 px-4 py-3">
            <Link href="/feed" className="rounded-full p-2 transition-colors hover:bg-muted/50">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="border-border border-b">
              ...
              <div className="relative h-[200px] bg-muted">
                {(() => {
                  const bannerUrl =
                    actorInfo.isUser && actorInfo.type === 'user' && 'coverImageUrl' in actorInfo
                  ...
                  if (!bannerUrl || bannerImageFailed) {
                    return null;
                  }
                ...
                <div
                  className={cn(
                    'h-full w-full bg-gradient-to-br from-primary/20 to-primary/5',
                    actorInfo.type === 'actor' || actorInfo.type === 'organization'
                      ? bannerImageFailed
                        ? ''
                        : 'hidden'
                      : ''
              ...
              <div className="px-4 pb-4">
                        ...
                        <FollowButton
                          ...
                          onFollowerCountChange={(delta) => {
                            ...
                            setOptimisticFollowerCount((prev) => {
                              const currentCount =
                                prev !== null ? prev : actorInfo.stats?.followers || 0;
                ...
                <div className="mb-3">
                  <div className="mb-0.5 flex items-center gap-1">
                    ...
                    {actorInfo.type === 'actor' && !actorInfo.isUser && <VerifiedBadge size="md" />}
                    {actorInfo.type === 'user' && (
                      <OnChainBadge
                  ...
                  {actorInfo.username && (
                    <p className="text-[15px] text-muted-foreground">@{actorInfo.username}</p>
                  )}
                </div>
                  ...
                  {filteredPosts.map((item, i) => {
                    const postData = {
                      ...
                      originalAuthorProfileImageUrl:
                        item.post.originalAuthorProfileImageUrl || null,
