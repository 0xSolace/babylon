  ...
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    logger.info(
      'Buy request START',
      {
        ...
        isNumber: !Number.isNaN(Number(marketId)),
      },
      'POST /api/markets/predictions/[id]/buy'
    );
    ...
    const {
      ...
      if (!market) {
        ...
        if (!question && !Number.isNaN(Number(marketId))) {
          logger.info(
            'Step 4d: Trying question by number',
            { questionNumber: parseInt(marketId, 10) },
            'POST /api/markets/predictions/[id]/buy'
          );
        ...
        if (new Date(question.resolutionDate) < new Date()) {
          throw new BusinessLogicError('Question has expired', 'QUESTION_EXPIRED', {
            marketId,
      ...
      if (market.resolved) {
        throw new BusinessLogicError('Market has already resolved', 'MARKET_RESOLVED', {
          marketId,
        });
      }
