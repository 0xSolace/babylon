...
export const GET = withErrorHandling(async (request: Request) => {
  ...
  if (following && userId) {
    ...
    const allFollowedIds = await getCacheOrFetch(
      ...
      async () => {
        ...
        legacyActorFollows.forEach((f) => {
          followedActorIds.add(f.npcId);
        });
    ...
    const posts = await cachedDb.getPostsForFollowing(userId, allFollowedIds, limit, cursor);
    ...
    const authorIds = [
      ...new Set(
        filteredPosts.map((p) => p.authorId).filter((id): id is string => id !== undefined)
      ),
    ];
    ...
    const originalAuthorsMap = new Map<
      string,
      {
        id: string;
        name: string;
    ...
    if (originalUsernames.size > 0) {
      ...
      for (const user of originalUsers) {
        ...
        originalAuthorsMap.set(username, {
          id: user.id,
      ...
      const reposterIds = posts
        .filter((p) => repostDataMap.has(p.id))
        .map((p) => p.authorId)
  ...
  if (type) {
    ...
    logger.info(
      ...
      { count: posts.length, limit, cursor },
      'GET /api/posts'
    );
  }
  ...
  const authorIds = [
    ...new Set(posts.map((p) => p.authorId).filter((id): id is string => id !== undefined)),
  ];
  ...
  const formattedPosts = await Promise.all(
    posts.map(async (post) => {
      ...
      const actor = actorMap.get(authorId);
      const org = orgMap.get(authorId);
          ...
          repostMetadata = {
            ...
            originalAuthorName:
              'name' in originalAuthor
                ? originalAuthor.name
                : (originalAuthor.displayName ?? originalAuthor.id),
            originalAuthorUsername:
  ...
  const nextCursor =
    formattedPosts.length > 0 ? formattedPosts[formattedPosts.length - 1]?.timestamp : null;
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const authorName = canonicalUser.username || canonicalUser.displayName || canonicalUser.id;
  ...
  logger.info(
    'Sent mention notifications',
    {
      ...
      mentionedUsernames: mentionedUsers.map((u) => u.username ?? u.id),
    },
    'POST /api/posts'
  );
