  ...
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    if (!post) {
      ...
      if (gamePost) {
        ...
        const [likeCount, commentCount, shareCount, actor, userRecord] = await asPublic(
          async (db) => {
        ...
        const originalPostIdFromGame =
          'originalPostId' in gamePost
            ? ((gamePost as Record<string, JsonValue>).originalPostId as string | undefined)
            : undefined;
        if (parsedRepostData || originalPostIdFromGame) {
                ...
                effectiveRepostData = {
                  ...
                  originalAuthorUsername:
                    'username' in originalAuthor
                      ? (originalAuthor.username ?? originalPost.authorId)
          ...
          if (originalAuthor && effectiveRepostData) {
            repostMetadata = {
              ...
              originalAuthorName:
                'name' in originalAuthor ? originalAuthor.name : originalAuthor.displayName,
              originalAuthorUsername:
                'username' in originalAuthor ? originalAuthor.username : originalAuthor.id,
