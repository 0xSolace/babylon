...
export const POST = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    if (!userIds || !Array.isArray(userIds) || userIds.length === 0) {
      throw new BusinessLogicError('At least one user ID is required', 'USER_IDS_REQUIRED');
    }
    ...
    const chat = await asUser(user, async (db) => {
      ...
      const isParticipant = chat.ChatParticipant.some((p) => p.userId === user.userId);
    ...
    const addedUsers = await asSystem(async (db) => {
      ...
      const newUsers = usersToAdd.filter((u) => !existingParticipantIds.includes(u.id));

      if (newUsers.length === 0) {
        throw new BusinessLogicError('All users are already in the chat', 'ALREADY_PARTICIPANTS');
      }
