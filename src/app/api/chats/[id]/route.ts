...
export const GET = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    if (isGameChat && debugMode) {
      ...
      if (!ensuredAuthUser) {
        throw new AuthorizationError('Authentication required', 'chat', 'read');
      }
      userId = ensuredAuthUser.userId;

      const isMember = await asUser(ensuredAuthUser, async (db) => {
    ...
    const fullChat = await (authUser
      ? asUser(authUser, async (db) => {
          return await db.chat.findUnique({
            where: { id: chatId },
            include: {
    ...
    const { users, actors } = await (authUser
      ? asUser(authUser, async (db) => {
          // Get participant details - need to check both users and actors
          const participantUserIds = fullChat.ChatParticipant.map(
            (p: (typeof fullChat.ChatParticipant)[number]) => p.userId
          );
          ...
          const senderIds = [
            ...new Set(fullChat.Message.map((m: (typeof fullChat.Message)[number]) => m.senderId)),
          ];
    ...
    const actorsMap = new Map<string, (typeof actors)[number]>(
      actors.map((a: (typeof actors)[number]) => [a.id, a])
    );
    ...
    const participantsInfo =
      fullChat.ChatParticipant.length > 0
    ...
    if (!fullChat.isGroup && !fullChat.name && userId) {
      const otherParticipant = fullChat.ChatParticipant.find(
        (p: (typeof fullChat.ChatParticipant)[number]) => p.userId !== userId
      );
      if (otherParticipant) {
