        ...
        const activeQuestions = await db.question.findMany({
          where: {
            ...
            resolutionDate: {
              ...
              lte: new Date(
                Date.now() + FEED_WIDGET_CONFIG.UPCOMING_EVENTS_DAYS * 24 * 60 * 60 * 1000
              ),
            },
          },
          orderBy: {
            ...
            ? Array.from(
                new Set(
                  upcomingEventTypes
                    ...
                    .filter(
                      (type): type is string => typeof type === 'string' && type !== undefined
                    )
                )
              )
            : [];
        ...
        for (const event of upcomingWorldEvents) {
          ...
          eventsList.push({
            ...
            source: event.relatedQuestion
              ...
              : 'World Event',
            relatedQuestion:
              typeof event.relatedQuestion === 'number' ? event.relatedQuestion : undefined,
          });
        }
