    ...
    ? await asUser(authUser, async (db) => {
        ...
        const significantEvents = recentEvents
          .filter((event) => {
            ...
            return newsWorthyTypes.some(
              (type) => eventType.includes(type) || availableEventTypes.includes(eventType)
            );
          })
        ...
        const significantPriceUpdates = priceUpdates
          .filter((update) => {
            ...
            return (
              Math.abs(changePercent) >= FEED_WIDGET_CONFIG.SIGNIFICANT_PRICE_CHANGE_PERCENT ||
              (changePercent > 0 &&
                update.changePercent &&
                update.changePercent >= FEED_WIDGET_CONFIG.MIN_PRICE_CHANGE_PERCENT)
            );
          })
          .slice(0, 3);
        ...
        for (const update of significantPriceUpdates) {
          ...
          const isTrending =
            new Date(update.timestamp).getTime() > Date.now() - priceTrendingThreshold;
