...
async function getAgentJsonRpcHandler(agentId: string): Promise<JsonRpcTransportHandler> {
  let jsonRpcHandler = agentJsonRpcHandlers.get(agentId);
  if (jsonRpcHandler) {
    return jsonRpcHandler;
  }

  let requestHandler = agentRequestHandlers.get(agentId);
  if (!requestHandler) {
  ...
  agentJsonRpcHandlers.set(agentId, jsonRpcHandler);
  return jsonRpcHandler;
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  const requestingAgentId =
    req.headers.get('x-agent-id') || req.headers.get('x-agent-address') || 'anonymous';
  ...
  if (officialMethods.includes(body.method)) {
    try {
      ...
      if (body.method === 'tasks/list') {
        ...
        const handler = jsonRpcHandler as unknown as {
          requestHandler: DefaultRequestHandlerType;
        };
        const taskStore = (handler.requestHandler as unknown as { taskStore: ExtendedTaskStore })
          .taskStore;
