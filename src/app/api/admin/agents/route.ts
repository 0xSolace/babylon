...
export async function GET(_req: NextRequest) {
  try {
    ...
    const stats = {
      ...
      running: formattedAgents.filter((a) => a.autonomousEnabled && a.agentStatus === 'running')
        .length,
      paused: formattedAgents.filter((a) => !a.autonomousEnabled || a.agentStatus === 'paused')
        .length,
      error: formattedAgents.filter((a) => a.agentStatus === 'error' || a.recentErrorsCount > 0)
        .length,
