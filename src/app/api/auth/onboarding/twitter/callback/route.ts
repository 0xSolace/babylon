...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  if (!userResponse.ok) {
    ...
    return NextResponse.redirect(new URL('/?error=failed_to_get_user', request.url));
  }
  ...
  if (!userData.data?.id || !userData.data?.username) {
    ...
    return NextResponse.redirect(new URL('/?error=invalid_twitter_data', request.url));
  }
  ...
  if (existingUser) {
    ...
    return NextResponse.redirect(new URL('/?error=twitter_already_linked', request.url));
  }
  ...
  return NextResponse.redirect(new URL(`/?social_import=twitter&data=${encodedData}`, request.url));
});
