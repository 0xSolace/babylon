...
export const GET = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ userId: string }> }) => {
    ...
    const referredUsers = referrals
      ...
      .map((r) => {
        const referredUser = r.User_Referral_referredUserIdToUser;
        const referredUserId = referredUser?.id;
        return {
          id: referredUserId,
          username: referredUser?.username,
          displayName: referredUser?.displayName,
          profileImageUrl: referredUser?.profileImageUrl,
          createdAt: referredUser?.createdAt,
          reputationPoints: referredUser?.reputationPoints,
          isFollowing: referredUserId ? followingUserIds.has(referredUserId) : false,
    ...
    logger.info(
      ...
      { userId: canonicalUserId, totalReferrals: referrals.length },
      'GET /api/users/[userId]/referrals'
    );
