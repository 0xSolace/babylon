...
import { isBackendSigningEnabled, updateProfileBackendSigned } from '@/lib/profile/backend-signer';
...
export const POST = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ userId: string }> }) => {
    ...
    const requiresOnchainUpdate =
      hasOnchainProfileChanges && currentUser?.onChainRegistered && currentUser?.nftTokenId;
    ...
    if (requiresOnchainUpdate) {
      if (isBackendSigningEnabled()) {
        ...
        const metadata = {
          name: normalizedDisplayName ?? currentUser?.displayName ?? '',
        ...
        if (!walletAddress) {
          throw new Error('Wallet address is required for on-chain profile update');
        }
        ...
        if (!txHash) {
          throw new Error('Missing on-chain transaction hash');
        }
