...
import { Prisma, type UserGroupInvite } from '@prisma/client';
...
export const POST = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    if (invitee.isActor) {
      return NextResponse.json({ error: 'Cannot invite NPCs to user groups' }, { status: 400 });
    }
