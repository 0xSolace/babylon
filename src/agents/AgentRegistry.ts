...
export class AgentRegistry extends EventEmitter {
  ...
  register(agent: AutonomousAgent): void {
    ...
    logger.info(
      `Registered agent: ${status.name} (${status.agentId})`,
      undefined,
      'AgentRegistry'
    );
  }
  ...
  search(criteria: AgentSearchCriteria): RegisteredAgent[] {
    ...
    for (const [agentId, registration] of this.agents.entries()) {
      ...
      if (criteria.minReputation !== undefined) {
        if (
          registration.profile.reputation.accuracyScore < criteria.minReputation
        ) {
          continue;
        }
      }
      ...
      if (criteria.minPerformance !== undefined) {
        const performance =
          registration.performance.totalPredictions > 0
    ...
    results.sort((a, b) => {
      const reputationDiff =
        b.profile.reputation.accuracyScore - a.profile.reputation.accuracyScore;
  ...
  updatePerformance(
    ...
    if (updates.reputationChange !== undefined) {
      ...
      registration.profile.reputation.accuracyScore = Math.max(
        0,
        Math.min(1, registration.profile.reputation.accuracyScore)
      );
    }
    ...
    if (registration.performance.totalPredictions > 0) {
      registration.profile.reputation.accuracyScore =
        registration.performance.correctPredictions /
        registration.performance.totalPredictions;
      registration.profile.reputation.totalBets =
        registration.performance.totalPredictions;
      registration.profile.reputation.winningBets =
        registration.performance.correctPredictions;
    }
  ...
  getStats(): {
    ...
    return {
      ...
      averageReputation:
        agents.length > 0 ? totalReputation / agents.length : 0,
      totalPredictions,
    };
  }
  ...
  findCoalitionPartners(
    ...
    return candidates
      ...
      .sort(
        (a, b) =>
          b.profile.reputation.accuracyScore -
          a.profile.reputation.accuracyScore
      )
      .slice(0, maxPartners);
  }
