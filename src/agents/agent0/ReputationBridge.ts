...
export class ReputationBridge implements IReputationBridge {
   ...
   */
  private calculateWeightedAccuracy(local: AgentReputation, agent0: AgentReputation): number {
   ...
   */
  private calculateTrustScore(local: AgentReputation, agent0: AgentReputation): number {
    // If one source has no data, use the other
    if (local.totalBets === 0 && agent0.totalBets === 0) {
      return 0;
    }
