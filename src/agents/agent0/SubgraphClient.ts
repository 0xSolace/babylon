...
export class SubgraphClient {
  ...
  private readonly isLocalnet: boolean;
  ...
  private parseAgentCapabilities(agent: SubgraphAgent) {
    ...
    try {
      ...
      logger.warn('Failed to parse agent capabilities', {
        error,
  ...
  async getAgent(tokenId: number): Promise<SubgraphAgent> {
    ...
    if (!agentData) {
      throw new Error(`Agent ${tokenId} not found on subgraph`);
    }
  ...
  async searchAgents(filters: {
    ...
    if (filters.strategies && filters.strategies.length > 0) {
      results = results.filter((agent) => {
        const parsed = this.parseAgentCapabilities(agent);
        if (!parsed) {
          return false;
        }
        const parsedCaps = parseCapabilities(parsed);
        const agentStrategies = parsedCaps.strategies ?? [];
    ...
    if (filters.markets && filters.markets.length > 0) {
      results = results.filter((agent) => {
        ...
        const parsedCaps = parseCapabilities(parsed);
        const agentMarkets = parsedCaps.markets ?? [];
