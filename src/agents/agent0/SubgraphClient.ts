...
export class SubgraphClient {
  ...
  constructor() {
    ...
    if (!subgraphUrl) {
      logger.warn(
        'AGENT0_SUBGRAPH_URL not configured, subgraph queries will fail',
        undefined,
        'SubgraphClient'
      );
    }
  ...
  async getAgent(tokenId: number): Promise<SubgraphAgent | null> {
    ...
    const data = (await this.client.request(query, { tokenId })) as {
  ...
  async searchAgents(filters: {
    ...
    if (filters.minTrustScore !== undefined) {
      whereConditions.push(
        `reputation_trustScore_gte: ${filters.minTrustScore}`
      );
    }
    ...
    const data = (await this.client.request(query)) as {
