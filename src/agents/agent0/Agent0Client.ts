...
export class Agent0Client implements IAgent0Client {
  ...
  async registerAgent(
    ...
    return {
      ...
      metadataCID:
        registrationFile.agentURI?.replace('ipfs://', '') || undefined,
    };
  }
  ...
  async searchAgents(
    ...
    return items.map((agent: AgentSummary) => ({
      ...
      capabilities: {
        strategies:
          (agent.extras?.capabilities as { strategies?: string[] })
            ?.strategies || [],
        markets:
          (agent.extras?.capabilities as { markets?: string[] })?.markets || [],
        actions:
          (agent.extras?.capabilities as { actions?: string[] })?.actions || [],
        version:
          (agent.extras?.capabilities as { version?: string })?.version ||
          '1.0.0',
      },
      reputation: {
...
export function getAgent0Client(): Agent0Client {
  if (!agent0ClientInstance) {
    ...
    agent0ClientInstance = new Agent0Client({
      ...
      privateKey,
      ipfsProvider:
        (process.env.AGENT0_IPFS_PROVIDER as
          | 'node'
          | 'filecoinPin'
          | 'pinata') || 'node',
