...
import type { AgentSummary, RegistrationFile, SDKConfig, SearchParams } from 'agent0-sdk';
...
export class Agent0Client implements IAgent0Client {
  ...
  async registerAgent(params: Agent0RegistrationParams): Promise<Agent0RegistrationResult> {
    ...
    const agent = this.sdk.createAgent(params.name, params.description, params.imageUrl);
    ...
    if (!tokenPart) {
      throw new Error(`Unexpected agentId format: ${agentId}`);
    }
    const tokenId = parseInt(tokenPart, 10);
  ...
  async registerBabylonGame(): Promise<Agent0RegistrationResult> {
    ...
    return this.registerAgent({
      ...
      description:
        process.env.BABYLON_GAME_DESCRIPTION || 'AI-powered prediction market game on Base network',
  ...
  async searchAgents(filters: Agent0SearchFilters): Promise<Agent0SearchResult[]> {
    ...
    if (!this.sdk) {
      throw new Error('Agent0 SDK not initialized');
    }
...
export function getAgent0Client(): Agent0Client {
  if (!agent0ClientInstance) {
    ...
    const network =
      networkEnv && validNetworks.includes(networkEnv as (typeof validNetworks)[number])
    ...
    // Validate IPFS provider configuration
    const ipfsProvider =
      (process.env.AGENT0_IPFS_PROVIDER as 'node' | 'filecoinPin' | 'pinata') || 'node';
