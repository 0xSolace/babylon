...
async function main() {
  ...
  if (existingGames.length > 0) {
    ...
    if (!lastGame) {
      throw new Error('No previous game found when expected.');
    }
    ...
    nextStartDate = isoDate || nextDate.toISOString();
  ...
  if (options.verbose) {
    ...
    logger.info(
      ...
      game.setup.questions.map((q) => `${q.id}. ${q.text} - Answer: ${q.outcome ? 'YES' : 'NO'}`),
      'CLI'
    );
