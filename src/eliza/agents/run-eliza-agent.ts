...
async function initCharacter({
  ...
  if (options.autoTrade) {
    ...
    if (
      typedTradingService &&
      typeof typedTradingService.enableAutoTrading === 'function'
    ) {
...
async function main() {
  ...
  if (!postgresUrl) {
    ...
    logger.warn(
      'POSTGRES_URL not set, using PGlite embedded database for development'
    );
    logger.warn(
      'For production, set POSTGRES_URL=postgresql://user:password@host:5432/database'
    );
  ...
  const characterModelProvider =
    (character as { modelProvider?: string }).modelProvider || 'openai';
  const modelProvider = hasGroqKey ? 'groq' : characterModelProvider;

  if (hasGroqKey && characterModelProvider !== 'groq') {
    logger.info(
      `Overriding modelProvider from "${characterModelProvider}" to "groq" (GROQ_API_KEY found in .env)`,
      undefined,
      'run-eliza-agent'
    );
  }
  ...
  logger.info(
    `Using model provider: ${modelProvider}`,
    undefined,
    'run-eliza-agent'
  );
  logger.info(
    `Model provider details: groqKeyAvailable=${hasGroqKey}, selectedProvider=${modelProvider}, originalProvider=${characterModelProvider}`,
    undefined,
    'run-eliza-agent'
  );
  ...
  if (options.autoTrade) {
    logger.info(
      '   - Service monitors markets every 60s, reviews portfolio every 5m'
    );
  } else {
    logger.info(
      '   - Interactive mode: Send messages to trigger analysis and trading'
    );
  }
