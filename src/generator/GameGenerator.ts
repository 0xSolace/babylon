...
function generatePreviousMonthContext(previousHistory: GameHistory[]): string {
  ...
  const lastGame = previousHistory.at(-1);
  if (!lastGame) {
    return '';
  }
...
${previousDays
  .map(
    (d) => `Day ${d.day}: ${d.summary}
Events: ${d.events.map((e) => e.description).join('; ')}`
  )
  .join('\n\n')}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
`;
...
export class GameGenerator {
  ...
  async generateCompleteGame(startDate = '2025-11-01'): Promise<GeneratedGame> {
    ...
    if (selectedActors.mains.length > 0) {
      logger.info(
        'Main cast:',
        selectedActors.mains.map((a) => `${a.name} - ${(a.description || '').substring(0, 60)}...`),
        'GameGenerator'
      );
    }
    ...
    logger.info(
      ...
      {
        avgReliability: (
          Array.from(personas.values()).reduce((sum, p) => sum + p.reliability, 0) / personas.size
        ).toFixed(2),
  ...
  private async generateGenesisEvents(
    ...
    for (let i = 0; i < eventCount; i++) {
      ...
      const type = eventTypes[randomIndex] ?? 'meeting';
  ...
  private async generateBaselineEvent(
    ...
    >(prompt, undefined, { temperature: 0.7, maxTokens: 5000 });
...
`;
   ...
   */
  private selectActors(): SelectedActorsByTier {
    ...
    const mainIds = Array.from(new Set(shuffledMains.map((a) => a.id))).slice(0, 3);
    const uniqueMains = mainIds
      .map((id) => allActors.find((a) => a.id === id))
      .filter((actor): actor is Actor => actor !== undefined)
      .map((actor) => ({
        ...actor,
        tier: (actor.tier ?? 'C_TIER') as ActorTier,
        role: 'main',
    ...
    const supportingIds = Array.from(new Set(shuffledSupport.map((a) => a.id))).slice(0, 15);
    const uniqueSupporting = supportingIds
      .map((id) => allActors.find((a) => a.id === id))
      .filter((actor): actor is Actor => actor !== undefined)
      .map((actor) => ({
        ...actor,
    ...
    const extraIds = Array.from(new Set(shuffledExtras.map((a) => a.id))).slice(0, 50);
    const uniqueExtras = extraIds
      .map((id) => allActors.find((a) => a.id === id))
      .filter((actor): actor is Actor => actor !== undefined)
      .map((actor) => ({
        ...actor,
...
Otherwise, start fresh.`;
  ...
  private generateConnections(selectedActors: SelectedActorsByTier): ActorConnection[] {
    ...
    selectedActors.mains.forEach((main: SelectedActor) => {
      ...
      connected.forEach((supporting: SelectedActor) => {
        const relationships = ['advisor', 'source', 'critic', 'ally', 'friend'] as const;
        const relationIndex = Math.floor(Math.random() * relationships.length);
        const relationship = relationships[relationIndex] ?? 'ally';
        connections.push({
          actor1: main.id,
    ...
    selectedActors.supporting.forEach((supporting: SelectedActor, i: number) => {
      ...
      connected.forEach((other: SelectedActor) => {
        const relationships = ['ally', 'friend', 'source', 'critic'] as const;
        const relationIndex = Math.floor(Math.random() * relationships.length);
        const relationship = relationships[relationIndex] ?? 'ally';
        connections.push({
  ...
  private async createGroupChats(
    ...
    const getActorById = (id: string): SelectedActor | undefined => {
      return [...selectedActors.mains, ...selectedActors.supporting, ...selectedActors.extras].find(
        (a: SelectedActor) => a.id === id
      );
    };
    ...
    for (const main of selectedActors.mains) {
      ...
      const members = memberIds
        ...
        .filter((actor): actor is SelectedActor => actor !== null && actor !== undefined);
  ...
  private async generateDay(
    ...
    const previousMonthContext =
      this.gameHistory.length > 0 ? generatePreviousMonthContext(this.gameHistory) : '';
    ...
    for (let i = 0; i < eventCount; i++) {
      ...
      const type = eventTypes[eventTypeIndex] ?? 'meeting';
   ...
   One sentence, max 120 chars, satirical but plausible.`;
    ...
    if ('response' in rawResponse && rawResponse.response && rawResponse.response.events) {
      if (Array.isArray(rawResponse.response.events)) {
        ...
        typeof rawResponse.response.events === 'object' &&
        'event' in rawResponse.response.events
      ) {
        const nested = (
          rawResponse.response.events as {
            event: Array<{
              eventNumber: number;
              event: string;
              pointsToward: 'YES' | 'NO' | null;
...
Max 120 characters, one sentence.`;
    ...
    const rawResponse = await this.llm.generateJSON<
      { event: string } | { response: { event: string } }
    >(prompt, undefined, { temperature: 0.9 });
  ...
  private async generateDayGroupMessagesBatch(
    ...
    const recentEvent =
      events.length > 0 ? events[Math.floor(Math.random() * events.length)] : null;
    ...
    const getEmotionalState = (actorId: string): string => {
      ...
      const moodDesc =
        state.mood > 0.3 ? 'confident' : state.mood < -0.3 ? 'pessimistic' : 'neutral';
      const luckDesc =
        state.luck === 'high'
          ? '🍀 lucky streak'
          : state.luck === 'low'
...
${req.members
  ...
  .join('\n')}${getRelationshipContext(req.members)}
   ...
   ${shuffleArray(allActors.filter((a) => !req.members.find((m) => m.actorId === a.id)))
     .slice(0, 12)
     .map((a) => a.name)
     .join(', ')}
...
`
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      >(prompt, { required: ['groups'] }, { temperature: 1.0, maxTokens: 5000 });
      ...
      const response =
        'response' in rawResponse && rawResponse.response
          ...
          : (rawResponse as {
              groups: Array<{
                groupId: string;
                messages: Array<{ actorId: string; message: string }>;
              }>;
            });

      const groups = response.groups || [];
      if (
        groups.length === groupRequests.length &&
        groups.every((g) => g.messages && g.messages.length > 0)
      ) {
        ...
        groups.forEach((group, i) => {
          ...
          messages[group.groupId] = group.messages.map((msg, j) => ({
            ...
            clueStrength:
              req.members.find((m) => m.actorId === msg.actorId)?.role === 'main' ? 0.7 : 0.4,
          }));
        });
      ...
      logger.warn(
        `Invalid group messages batch for day ${day} (attempt ${attempt + 1}/${maxRetries}). Expected ${groupRequests.length}, got ${groups.length}`,
        undefined,
        'GameGenerator'
      );
  ...
  public async generateGroupMessage(
    ...
    >(prompt, undefined, { temperature: 1.0 });
        ...
        if (Math.random() > 0.7) {
          ...
          if (current) {
            ...
            const isNegativeEvent =
              event.type === 'scandal' || event.type === 'conflict' || event.pointsToward === 'NO';
   ...
   */
  private generateResolution(questions: Question[], timeline: DayTimeline[]): GameResolution {
