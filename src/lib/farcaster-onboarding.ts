...
export async function openFarcasterOnboardingPopup(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      try {
        logger.info(
          'Received Farcaster auth message',
          { fid: data.fid },
          'FarcasterOnboarding'
        );
        ...
        if (!response.ok) {
          ...
          throw new Error(
            error.error || 'Failed to verify Farcaster authentication'
          );
        }
        ...
        logger.error(
          'Failed to process Farcaster auth',
          {
            error: errorMessage,
            stack: errorStack,
          },
...
export async function fetchFarcasterProfile(
  ...
  return data.profile as FarcasterOnboardingProfile;
}
