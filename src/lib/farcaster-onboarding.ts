...
export type FarcasterOnboardingProfile = {
  fid: number;
  username: string;
  displayName?: string;
  pfpUrl?: string;
...
export async function openFarcasterOnboardingPopup(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (!response.ok) {
        let errorResponse: { error?: string } | undefined;
        try {
          errorResponse = (await response.json()) as { error?: string };
        } catch {
          throw new Error('Failed to verify Farcaster authentication');
        }
        throw new Error(errorResponse?.error || 'Failed to verify Farcaster authentication');
      }
 ...
 */
export async function openNeynarFarcasterAuth(userId: string): Promise<FarcasterOnboardingProfile> {
