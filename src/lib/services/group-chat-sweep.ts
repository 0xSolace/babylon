...
export class GroupChatSweep {
  ...
  static async calculateKickChance(
    ...
    if (hoursSinceLastMessage > this.INACTIVITY_GRACE_PERIOD_HOURS) {
      const excessHours =
        hoursSinceLastMessage - this.INACTIVITY_GRACE_PERIOD_HOURS;
      const range =
        this.INACTIVITY_MAX_HOURS - this.INACTIVITY_GRACE_PERIOD_HOURS;
  ...
  static async sweepChat(chatId: string): Promise<{
    ...
    for (const membership of memberships) {
      const decision = await this.calculateKickChance(
        membership.userId,
        chatId
      );
