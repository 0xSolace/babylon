...
import { type Address, createPublicClient, createWalletClient, http, parseEther } from 'viem';
...
export class ReputationService {
  ...
  static async updateReputationForResolvedMarket(
    ...
    logger.info(
      `Updating reputation for ${positions.length} positions in market ${resolution.marketId}`,
      { count: positions.length, marketId: resolution.marketId },
      'ReputationService'
    );
  ...
  static async batchUpdateReputation(
    ...
    for (const resolution of resolutions) {
      const results = await ReputationService.updateReputationForResolvedMarket(resolution);
