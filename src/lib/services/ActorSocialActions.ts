    ...
    for (const actor of actors) {
      ...
      for (const { userId, interactions } of actorInteractions) {
        ...
        const existingMembership =
          await db.prisma.groupChatMembership.findFirst({
            where: {
              userId,
              npcAdminId: actor.id,
              isActive: true,
        ...
        const hasExistingDM =
          ...
          existingDMChat.participants.some(
        ...
        const qualityFactor = Math.min(
          avgQuality / this.MIN_INTERACTION_QUALITY,
          1.5
        );
        const countFactor = Math.min(
          interactions.length / this.MIN_INTERACTIONS_FOR_ACTION,
          2.0
        );

        const inviteProbability =
          this.BASE_INVITE_PROBABILITY * qualityFactor * countFactor;
        const dmProbability =
          this.BASE_DM_PROBABILITY * qualityFactor * countFactor;
