...
If no good tags can be extracted, return an empty array: []`;
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are a trending topics extraction expert. You analyze social media posts and extract organic, searchable tags that would appear in trending sections.',
      },
      {
        role: 'user',
        content: prompt,
  ...
  const tags = JSON.parse(jsonContent) as Array<{
    displayName: string;
    category?: string;
  }>;
