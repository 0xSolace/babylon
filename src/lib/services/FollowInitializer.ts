...
export class FollowInitializer {
  ...
  static async initializeActorFollows(): Promise<void> {
    logger.info(
      'Initializing actor follow relationships...',
      undefined,
      'FollowInitializer'
    );
  ...
  static async loadAndInitialize(filePath: string): Promise<void> {
    ...
    logger.info(
      'Follower verification complete',
      verification,
      'FollowInitializer'
    );
  }
   ...
   */
  static async importRelationships(
    relationships: RelationshipData[]
  ): Promise<void> {
    logger.info(
      'Importing relationships to database...',
      undefined,
      'FollowInitializer'
    );
  ...
  static async verifyFollowerCounts(): Promise<VerificationReport> {
    ...
    const actorsWithoutFollowers = actors.filter(
      (a) => a.followedBy.length === 0
    );

    // Calculate stats by tier
    const tierStats: Record<
      string,
      { counts: number[]; min: number; max: number; avg: number }
    > = {};
    ...
    const followerCountByTier: Record<
      string,
      { min: number; max: number; avg: number }
    > = {};
    Object.keys(tierStats).forEach((tier) => {
  ...
  static async ensureMinimumFollowers(minFollowers: number): Promise<void> {
    const actorsWithNoFollowers =
      await RelationshipManager.getActorsWithNoFollowers();
  ...
  private static determineFollowDirection(
    ...
    if (sentiment < -0.7) {
      return {
        actor1FollowsActor2: false,
        actor2FollowsActor1: false,
        isMutual: false,
      };
    }
    ...
    switch (relationshipType) {
      ...
      case 'competitors':
        ...
        return {
          actor1FollowsActor2: followEachOther,
          actor2FollowsActor1: followEachOther,
          isMutual: followEachOther,
        };
      ...
      default:
        ...
        return {
          actor1FollowsActor2: defaultFollow,
          actor2FollowsActor1: defaultFollow,
          isMutual: defaultFollow,
        };
    }
  }
