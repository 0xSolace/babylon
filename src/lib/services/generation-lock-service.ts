...
export async function acquireGenerationLock(processId?: string): Promise<boolean> {
  ...
  if (existingLock && existingLock.expiresAt > now) {
    logger.info(
      'Generation lock held by another process',
      {
  ...
  logger.info(
    'Generation lock acquired by another process (race condition)',
    {
...
export async function releaseGenerationLock(processId?: string): Promise<void> {
  if (!processId) {
    logger.warn(
      'releaseGenerationLock called without processId - cannot safely release',
      undefined,
      'GenerationLock'
    );
    return;
