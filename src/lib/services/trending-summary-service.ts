...
One sentence summary:`;
  ...
  let cleanSummary = response.choices[0].message.content
    .trim()
    .replace(/^["']|["']$/g, '')
    .replace(/\.$/, '')
    .trim();
...
export async function generateTrendingSummaries(
  ...
  for (const tag of tags) {
    const summary = await generateTrendingSummary(tag.displayName, tag.category, tag.recentPosts);
