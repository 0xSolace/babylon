...
export const IDENTITY_REGISTRY = process.env
  .NEXT_PUBLIC_IDENTITY_REGISTRY_BASE_SEPOLIA as Address;
export const REPUTATION_SYSTEM = process.env
...
export async function processOnchainRegistration({
  ...
  if (user.isAgent) {
    ...
    try {
      ...
      logger.warn(
        'Unable to check onchain registration status, using database state',
        {
      ...
      try {
        ...
        logger.info(
          'Synced on-chain registration status to database',
          {
            userId: dbUser.id,
            tokenId,
            wasRegistered: dbUser.onChainRegistered,
          },
  ...
  if (user.isAgent) {
    ...
    const baseEndpoint =
      endpoint || `https://babylon.game/agent/${user.userId}`;
    agentEndpoint = `${baseEndpoint}?agentId=${user.userId}`;
  } else {
    registrationAddress = walletAddress! as Address;
    agentEndpoint =
      endpoint || `https://babylon.game/agent/${walletAddress!.toLowerCase()}`;
  }
  ...
  if (walletClient) {
    logger.info(
      'Setting initial reputation to 70...',
      undefined,
      'OnboardingOnchain'
    );
    ...
    logger.info(
      'Initial reputation set to 70 (7 wins out of 10 bets)',
      undefined,
      'OnboardingOnchain'
    );
  } else {
  ...
  try {
    ...
    if (dbError && typeof dbError === 'object' && 'code' in dbError) {
      ...
      if (
        ...
        prismaError.meta?.target?.includes('nftTokenId')
      ) {
        ...
        if (existingUser?.nftTokenId === tokenId) {
          ...
          logger.info(
            'User already has this nftTokenId, continuing',
            { userId: dbUser.id, tokenId },
            'processOnchainRegistration'
          );
        } else {
  ...
  if (user.isAgent) {
    const agent0Client = new Agent0Client({
      network:
        (process.env.AGENT0_NETWORK as 'sepolia' | 'mainnet') || 'sepolia',
      rpcUrl:
    ...
    try {
      ...
      logger.error(
        'Failed to sync Agent0 reputation after registration',
        {
          userId: dbUser.id,
          agent0TokenId: agent0Result.tokenId,
          error: syncError,
  ...
  const balanceBefore =
    userWithBalance?.virtualBalance ?? new Prisma.Decimal(0);
...
export async function confirmOnchainProfileUpdate({
  ...
  if (!tokenId || Number.isNaN(tokenId)) {
    throw new InternalServerError(
      'Unable to determine token ID from profile update transaction',
      { txHash }
    );
  }
...
export async function getOnchainRegistrationStatus(
  ...
  if (!user.isAgent && userRecord.walletAddress) {
    try {
      ...
      logger.warn(
        'Failed to query on-chain registration status',
