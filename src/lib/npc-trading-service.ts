...
export class NPCTradingService {
  ...
  private static extractTradingSignals(
    ...
    for (const market of marketContext.perpMarkets) {
      ...
      const orgPattern = new RegExp(
        market.organizationId.replace(/-/g, '\\s*'),
        'i'
      );
    ...
    for (const market of marketContext.predictionMarkets) {
      ...
      const mentionsMarket = keywords.some((kw) =>
   ...
   */
  private static getBaseTradeAmount(actor: {
    tier?: string;
    personality?: string;
  }): number {
  ...
  private static async executePersonalTrade(
    ...
    logger.info(
      `Personal NPC trade (simulated): ${actor.name} ${signal.action} ${signal.ticker || signal.marketId}`,
      {},
      'NPCTradingService'
    );
  }
  ...
  private static async executePoolTrade(
    ...
    if (balance < signal.amount) {
      logger.warn(
        `Insufficient pool balance for ${actor.name}: ${balance} < ${signal.amount}`
      );
      return;
    }

    const currentPrice =
      signal.type === 'perp' ? await this.getMarketPrice(signal.ticker!) : 50;

    if (signal.type === 'prediction' && signal.marketId) {
      await this.executePredictionTrade(
        signal.marketId,
    ...
    await prisma.nPCTrade.create({
      data: {
        ...
        sentiment:
          signal.confidence *
          (signal.side === 'NO' || signal.side === 'short' ? -1 : 1),
    ...
    logger.info(
      `Pool trade executed: ${actor.name} ${signal.action} ${signal.ticker || signal.marketId}`
    );
  }
  ...
  private static async executePredictionTrade(
    ...
    if (availableBalance < amount) {
      throw new Error(
        `Insufficient pool balance: ${availableBalance} < ${amount}`
      );
    }
  ...
  static async processRecentPosts(marketContext: MarketContext): Promise<void> {
    ...
    logger.info(
      `Processing ${recentPosts.length} recent posts for NPC trading`
    );
