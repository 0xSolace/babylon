...
export async function calculateTradeMetrics(
  ...
  if (!position || !position.question) {
    logger.warn(
      'Position or question not found for trade metrics',
      { positionId },
      'TradeFeedback'
    );
    return null;
  ...
  if (!position.questionId) {
    logger.warn(
      'Position has no questionId for trade metrics',
      { positionId },
      'TradeFeedback'
    );
    return null;
  ...
  const userBalance =
    Number(position.user.virtualBalance) + Number(position.user.totalDeposited);
