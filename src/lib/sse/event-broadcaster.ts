...
import { isRedisAvailable, safePublish } from '@/lib/redis';
...
class InMemoryBroadcaster extends EventEmitter {
  ...
  addClient(client: SSEClient) {
    ...
    logger.debug(
      `SSE client connected: ${client.id} (userId: ${client.userId})`,
      { clientId: client.id },
      'InMemoryBroadcaster'
    );
  }
  ...
  broadcast(message: BroadcastMessage) {
    ...
    for (const client of this.clients.values()) {
      ...
      try {
        ...
        logger.debug(
          `Failed to send message to client ${client.id}, will remove`,
          { clientId: client.id },
          'InMemoryBroadcaster'
        );
        clientsToRemove.push(client.id);
    ...
    if (sentCount > 0) {
      logger.debug(
        `Broadcasted to ${sentCount} clients on channel ${channel}`,
        { channel, sentCount },
        'InMemoryBroadcaster'
      );
    }
  ...
  getStats() {
    return {
      ...
      clientsByChannel: Array.from(this.clients.values()).reduce(
        (acc, client) => {
...
class ServerlessBroadcaster extends EventEmitter {
  ...
  constructor() {
    ...
    if (this.redisAvailable) {
      logger.info(
        'Serverless broadcaster initialized with Redis pub/sub',
        undefined,
        'ServerlessBroadcaster'
      );
      this.setupRedisPolling();
    } else {
      logger.warn(
        'Serverless broadcaster initialized (local-only) - Set UPSTASH_REDIS_REST_URL for cross-instance SSE',
        undefined,
        'ServerlessBroadcaster'
      );
    }
  ...
  private localBroadcast(message: BroadcastMessage) {
    ...
    if (sentCount > 0) {
      logger.debug(
        `Broadcasted to ${sentCount} local clients on channel ${channel}`,
        { channel, sentCount },
        'ServerlessBroadcaster'
      );
    }
  ...
  private async publishToRedis(message: BroadcastMessage) {
    const published = await safePublish(`sse:${message.channel}`, JSON.stringify(message));

    if (published) {
      logger.debug(
        `Published to Redis channel ${message.channel}`,
...
export function getEventBroadcaster(): InMemoryBroadcaster | ServerlessBroadcaster {
  if (!broadcasterInstance) {
    ...
    if (isProduction) {
      logger.info(
        'Using serverless broadcaster for production (local-only)',
        undefined,
        'getEventBroadcaster'
      );
...
if (module?.hot) {
  module.hot.dispose(() => {
    if (broadcasterInstance) {
      logger.debug(
        'HMR: Cleaning up SSE broadcaster before hot reload',
        undefined,
        'event-broadcaster'
      );
 ...
 */
export function broadcastToChannel(channel: Channel, data: Record<string, JsonValue>): void {
