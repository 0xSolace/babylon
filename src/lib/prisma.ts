...
function createPrismaClient() {
  ...
  if (!databaseUrl && process.env.NODE_ENV === 'production') {
    logger.error(
      '[Prisma] ERROR: Neither PRISMA_DATABASE_URL nor DATABASE_URL is set',
      undefined,
      'prisma'
    );
  }
  ...
  if (databaseUrl) {
    ...
    logger.info(
      '[Prisma] Enforced connection pool settings: connection_limit=50, pool_timeout=30, connect_timeout=10',
      undefined,
      'prisma'
    );
  }
...
function checkNodeJsTestEnvironment(): boolean {
  ...
  try {
    ...
    const argvProp = 'argv';
    const processArgv = (process as unknown as Record<string, unknown>)[argvProp] as
      | string[]
      | undefined;
...
function getPrismaClient(): PrismaClient | null {
  ...
  if (isBuildTime && !isTestEnvCheck) {
    if (!globalForPrisma.prisma) {
      logger.info(
        '[Prisma] Build time detected - skipping Prisma initialization',
        undefined,
        'prisma'
      );
    }
  ...
  if (!globalForPrisma.prisma) {
    ...
    if (databaseUrl) {
      try {
        ...
        if (isTestEnv) {
          logger.error(
            '[Prisma] Failed to initialize Prisma client in test environment',
            { error },
            'prisma'
          );
          throw error;
...
if (isTestEnv && !basePrismaClient) {
  logger.error(
    '[Prisma] ERROR: Prisma client is not initialized in test environment. Check DATABASE_URL environment variable.',
    undefined,
    'prisma'
  );
  throw new Error(
    'Prisma client is not initialized in test environment. Check DATABASE_URL environment variable.'
  );
}
...
function getPrismaWithRetry(): PrismaClient {
  ...
  if (!client) {
    ...
    if (!isTestEnv) {
      if (!isBuildTime) {
        logger.error(
          '[Prisma] ERROR: Prisma client is not initialized. Check DATABASE_URL environment variable.',
          undefined,
          'prisma'
        );
        throw new Error(
  ...
  if (!globalForPrisma.prismaWithRetry) {
    ...
    globalForPrisma.prismaWithRetry = createRetryProxy(client, retryOptions) as ReturnType<
      typeof createRetryProxy<PrismaClient>
    >;
  }
...
function createLazyPrismaProxy(): PrismaClient {
  return new Proxy({} as PrismaClient, {
    ...
    set<K extends keyof PrismaClient>(
      _target: PrismaClient,
      prop: K,
      value: PrismaClient[K]
    ): boolean {
      // Allow property assignment for testing/mocking purposes
      const client = getPrismaWithRetry();
      (client as PrismaClient)[prop] = value;
      return true;
    },
