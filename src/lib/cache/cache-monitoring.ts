...
class CacheMonitoring {
  private stats: Map<
    string,
    { hits: number; misses: number; totalResponseTime: number }
  > = new Map();

  recordHit(cacheKey: string, responseTime: number): void {
    const current = this.stats.get(cacheKey) || {
      hits: 0,
      misses: 0,
      totalResponseTime: 0,
    };
    ...
    logger.debug(
      `Cache hit for ${cacheKey} (${responseTime}ms)`,
      undefined,
      'CacheMonitoring'
    );
  }
  ...
  recordMiss(cacheKey: string, responseTime: number): void {
    ...
    logger.debug(
      `Cache miss for ${cacheKey} (${responseTime}ms)`,
      undefined,
      'CacheMonitoring'
    );
  }
  ...
  getAllStats() {
    const result: Record<
      string,
      { hits: number; misses: number; hitRate: number; avgResponseTime: number }
    > = {};
