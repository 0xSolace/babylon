...
export const sendMessageAction: Action = {
  ...
  handler: async (
    ...
    const messageMatch =
      content.match(/(?::|with)\s*["'](.+?)["']/) || content.match(/message\s+(.+)$/i);
    ...
    if (!chatId || !messageContent) {
      if (callback) {
        callback({
          text: 'Missing chat ID or message content.',
          action: 'SEND_MESSAGE',
        });
      }
      return;
    ...
    try {
      const result = (await babylonRuntime.a2aClient.sendMessage(chatId, messageContent)) as {
        success?: boolean;
        messageId?: string;
        message?: string;
      };
...
export const createGroupAction: Action = {
  ...
  handler: async (
    ...
    try {
      const result = (await babylonRuntime.a2aClient.createGroup(groupName, memberIds)) as {
        success?: boolean;
        chatId?: string;
        message?: string;
      };
