  ...
  get: async (runtime: IAgentRuntime, _message: Memory, _state: State): Promise<ProviderResult> => {
    ...
    try {
      ...
      const withChanges = organizations
        .filter(
          (org) => org.currentPrice && (org.initialPrice || org.priceChangePercentage !== undefined)
        )
        .map((org) => {
...
${losersText}`,
      ...
      return {
        text: `Error fetching market movers: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  },
};
