...
async function initializeA2ASdkClient(agentUserId: string): Promise<A2AClient> {
  ...
  const baseUrl =
    process.env.BABYLON_A2A_ENDPOINT || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
  ...
  try {
    ...
    if (isTestEnv) {
      logger.warn(
        'A2A client initialization failed (server may not be running) - using database fallback',
        {
          agentUserId,
          error: error instanceof Error ? error.message : String(error),
          agentCardUrl,
...
export class BabylonA2AClient {
   ...
   */
  private async executeViaA2A(action: string, params: Record<string, unknown>): Promise<unknown> {
    if (!this.sdkClient) {
    ...
    try {
      ...
      while (Date.now() - startTime < maxWaitMs) {
        ...
        if (state === 'failed' || state === 'canceled' || state === 'rejected') {
          ...
          const errorText =
            messagePart && 'text' in messagePart ? messagePart.text : 'Unknown error';
...
export async function initializeAgentA2AClient(
  ...
  return new BabylonA2AClient(sdkClient, agentUserId, walletAddress, agent0TokenId);
}
...
export async function enhanceRuntimeWithBabylon(
  ...
  try {
    ...
    if (!sdkClient) {
      logger.warn(
        'A2A SDK client is null - using database fallback',
        {
          agentUserId,
    ...
    logger.error(
      '❌ A2A client initialization FAILED',
      {
        agentUserId,
        error: errorMessage,
        stack: errorStack,
