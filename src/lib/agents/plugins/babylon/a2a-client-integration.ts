 ...
 */
export async function initializeA2AClient(endpoint?: string): Promise<A2AClient> {
  ...
  try {
    ...
    throw new Error(
      `Could not initialize A2A client: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
export async function waitForTaskCompletion(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const task =
      'result' in response && response.result && 'task' in response.result
