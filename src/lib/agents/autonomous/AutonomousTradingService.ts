...
${contextString}`;
    ...
    if (estimatedTokens > 30000) {
      ...
      logger.warn(`Trading prompt too long: ${estimatedTokens} tokens, truncating`, {
        agentUserId,
      });
    ...
    if (trade.type === 'prediction' && predictionMarkets.length > 0) {
      ...
      if (market && trade.amount <= Number(balance.balance)) {
        ...
        try {
          ...
          logger.error(
            `Trade execution failed for agent ${agentUserId}`,
            error,
            'AutonomousTrading'
          );
      ...
      if (org && trade.amount <= Number(balance.balance)) {
        try {
          ...
          logger.error(
            `Perp trade execution failed for agent ${agentUserId}`,
            error,
            'AutonomousTrading'
          );
          throw error; // Fail fast - don't continue on trade errors
