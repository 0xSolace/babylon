...
export class AgentRuntimeManager {
  private static instance: AgentRuntimeManager | null = null;
  private constructor() {
  ...
  public async getRuntime(agentUserId: string): Promise<AgentRuntime> {
    ...
    const parseBio = (): string[] => {
      ...
      try {
        ...
        const displayValue =
          typeof exampleValue === 'string' ? exampleValue.substring(0, 50) : String(exampleValue);
    ...
    const parseStyle = (): Record<string, JsonValue> | undefined => {
      ...
      try {
        ...
        const displayValue =
          typeof styleValue === 'string' ? styleValue.substring(0, 50) : String(styleValue);
    ...
    if (process.env.WANDB_API_KEY) {
      try {
        ...
        if (aiConfig.wandbEnabled) {
          wandbModel =
            aiConfig.wandbModel || process.env.WANDB_MODEL || 'OpenPipe/Qwen3-14B-Instruct';
          useWandb = true;
          logger.info(
    ...
    (runtime as unknown as { currentModel?: string }).currentModel =
      wandbModel || (useWandb ? 'wandb' : 'groq');
    ...
    if (groqPlugin.models) {
      const modelDelegates =
        ((runtime as unknown as Record<string, unknown>).modelDelegates as Record<
          string,
          unknown
        >) || {};
