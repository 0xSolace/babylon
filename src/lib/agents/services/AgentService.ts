...
export class AgentServiceV2 {
  ...
  async getPerformance(agentUserId: string): Promise<AgentPerformance> {
    ...
    const avgTradeSize =
      trades.length > 0 ? trades.reduce((sum, t) => sum + t.amount, 0) / trades.length : 0;
    ...
    return {
      ...
      winRate:
        trades.length > 0 ? trades.filter((t) => t.pnl && t.pnl > 0).length / trades.length : 0,
      avgTradeSize,
    };
  }
  ...
  async getLogs(agentUserId: string, filters?: { type?: string; level?: string; limit?: number }) {
    const where: { agentUserId: string; type?: string; level?: string } = {
      agentUserId,
    };
