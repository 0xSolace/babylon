...
export class SimulationEngine {
    ...
    switch (type) {
      ...
      case 'open_perp': {
        ...
        if (market) {
          ...
          const futurePrice =
            priceHistory?.[Math.min(this.currentTick + 10, priceHistory.length - 1)]?.price;
      ...
      predictionMetrics: {
        ...
        accuracy:
          this.predictionPositions.size > 0
            ...
            : 0,
        avgPnlPerPosition:
          this.predictionPositions.size > 0 ? predictionPnl / this.predictionPositions.size : 0,
      },
      perpMetrics: {
        totalTrades: this.perpPositions.size,
        profitableTrades,
        winRate: this.perpPositions.size > 0 ? profitableTrades / this.perpPositions.size : 0,
  ...
  private calculateOptimalityScore(): number {
    ...
    return optimalActions.length > 0 ? (matchedActions / optimalActions.length) * 100 : 0;
  }
    ...
    for (const action of this.actions) {
      ...
      if (tick) {
        ...
        if (action.type === 'buy_prediction') {
          const positionId = Object.keys(Object.fromEntries(this.predictionPositions)).find(
            (id) => {
              const pos = this.predictionPositions.get(id);
              return Boolean(pos && pos.openedAt === action.tick);
            }
          );
