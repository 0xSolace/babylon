...
import { type SimulationConfig, SimulationEngine, type SimulationResult } from './SimulationEngine';
...
export class BenchmarkRunner {
  ...
  static async runSingle(config: BenchmarkRunConfig): Promise<SimulationResult> {
    ...
    if (config.benchmarkPath) {
      ...
      if (!config.generatorConfig) {
        throw new Error(
          'Benchmark generator configuration is required when no benchmark path is provided'
        );
      }
    ...
    while (!engine.isComplete()) {
      ...
      if (
        ...
        (tickResult.actionsExecuted.trades > 0 || tickResult.actionsExecuted.posts > 0)
      ) {
  ...
  static async compareAgents(
    ...
    const delta = {
      ...
      accuracy:
        result1.metrics.predictionMetrics.accuracy - result2.metrics.predictionMetrics.accuracy,
