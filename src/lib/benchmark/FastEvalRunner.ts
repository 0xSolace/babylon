...
export type FastEvalConfig = {
  ...
  /** Progress callback */
  onProgress?: (progress: { completed: number; total: number; currentRun?: string }) => void;
};

export type FastEvalResult = {
  /** All run results */
...
export class FastEvalRunner {
  ...
  static async run(config: FastEvalConfig): Promise<FastEvalResult> {
    ...
    const avgAccuracy =
      results.reduce((sum, r) => sum + r.metrics.predictionMetrics.accuracy, 0) / results.length;
    const avgOptimality =
