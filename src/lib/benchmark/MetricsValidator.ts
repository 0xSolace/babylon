...
export class MetricsValidator {
  ...
  private static validatePredictionMetrics(
    ...
    const predictionActions = actions.filter((action) => action.type === 'buy_prediction');

    for (const action of predictionActions) {
      const data = action.data as { marketId: string; outcome: string };
      if (!data?.marketId) {
        warnings.push('Prediction action missing marketId');
        continue;
      }

      if (!(data.marketId in groundTruth.marketOutcomes)) {
        warnings.push(`No ground truth for market ${data.marketId}`);
      }
    }
  ...
  static sanityCheck(metrics: SimulationMetrics): boolean {
    ...
    if (metrics.predictionMetrics.accuracy < 0 || metrics.predictionMetrics.accuracy > 1)
      return false;
