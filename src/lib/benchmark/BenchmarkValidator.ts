...
export class BenchmarkValidator {
  ...
  static validate(snapshot: unknown): BenchmarkValidationResult {
    ...
    if (
      ...
      const markets = (
        Array.isArray(initialState.predictionMarkets) ? initialState.predictionMarkets : []
      ) as Array<Record<string, unknown>>;
      const outcomes = (
        groundTruth.marketOutcomes && typeof groundTruth.marketOutcomes === 'object'
  ...
  static sanityCheck(snapshot: unknown): snapshot is BenchmarkGameSnapshot {
    ...
    const snap = snapshot as Record<string, unknown>;
    return !!(snap.id && snap.initialState && Array.isArray(snap.ticks) && snap.groundTruth);
  }
