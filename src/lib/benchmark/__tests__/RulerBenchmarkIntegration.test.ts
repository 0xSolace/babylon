...
import { type BenchmarkConfig, BenchmarkDataGenerator } from '../BenchmarkDataGenerator';
import {
  createRulerContext,
...
describe('RulerBenchmarkIntegration', () => {
  let snapshot: ReturnType<typeof BenchmarkDataGenerator.prototype.generate> extends Promise<
    infer T
  >
    ? T
    : never;
  ...
  it('should check if decision was optimal', () => {
    ...
    if (optimalActions.length > 0) {
      ...
      if (!action) return;
      const wasOptimal = wasDecisionOptimal(snapshot, action.tick, action.type, action.target);
