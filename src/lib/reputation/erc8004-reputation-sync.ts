      ...
      if (!feedbackPrivateKey) {
        ...
        if (!agentUser?.walletAddress) {
          ...
          try {
            ...
            onChainError =
              submitError instanceof Error ? submitError.message : 'Unknown submission error';
            logger.warn(
...
export async function batchSyncReputationsToERC8004(
  ...
  const { limit = 100, offset = 0, forceRecalculate = false, prioritizeNew = true } = options;
  ...
  logger.info(
    `Batch syncing ${users.length} user reputations`,
    {
      limit,
      offset,
      prioritizeNew,
 ...
 */
async function recordReputationSync(userId: string, score: number, tags: string[]): Promise<void> {
