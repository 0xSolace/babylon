 ...
 */
export async function getCachedAgent0ReputationScore(userId: string): Promise<number> {
 ...
 */
export async function calculateAgent0ReputationScore(userId: string): Promise<number> {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: {
      id: true,
  ...
  const hasActivity =
    metrics &&
    (metrics.gamesPlayed > 0 || metrics.totalFeedbackCount > 0 || metrics.lastActivityAt !== null);
