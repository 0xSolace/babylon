...
export class AnalysisService {
  ...
  storeAnalysis(marketId: string, analysis: MarketAnalysis): string {
    ...
    if (marketAnalyses.length > this.MAX_ANALYSES_PER_MARKET) {
      this.analyses.set(
        marketId,
        marketAnalyses.slice(-this.MAX_ANALYSES_PER_MARKET)
      );
    }

    logger.debug(
      `Stored analysis ${analysisId} for market ${marketId}`,
      undefined,
      'AnalysisService'
    );
    return analysisId;
  ...
  cleanupOldAnalyses(): void {
    ...
    for (const [marketId, analyses] of this.analyses.entries()) {
      const validAnalyses = analyses.filter(
        (a) => now - a.timestamp < this.ANALYSIS_TTL
      );
    ...
    if (totalCleaned > 0) {
      logger.info(
        `Cleaned up ${totalCleaned} old analyses`,
        undefined,
        'AnalysisService'
      );
    }
  ...
  getStatistics(): {
    ...
    const totalAnalyses = Array.from(this.analyses.values()).reduce(
      (sum, analyses) => sum + analyses.length,
      0
    );
