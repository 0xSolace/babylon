...
import { A2AEventType, ErrorCode } from '../types';
...
export class A2AWebSocketServer extends EventEmitter {
  ...
  private connections: Map<string, AgentConnection & { ws: WebSocket }> =
    new Map();
  ...
  constructor(config: A2AServerOptions) {
    ...
    if (existing) {
      this.logger.info(
        'Reusing existing A2A server from singleton',
        'A2AWebSocketServer'
      );
    ...
    this.wss.on('error', (error: Error & { code?: string }) => {
      if (error.code === 'EADDRINUSE') {
        this.logger.error(
          `Port ${this.config.port} is already in use. Please stop the process using this port or change the port configuration.`,
          error
        );
    ...
    this.wss.on('listening', () => {
      this._ready = true;
      this.logger.info(
        `A2A WebSocket server listening on ${this.config.host}:${this.config.port}`,
        'A2AWebSocketServer'
      );
    ...
    this.setupServer();
    this.logger.info(
      `A2A WebSocket server created on ${this.config.host}:${this.config.port}`,
      'A2AWebSocketServer'
    );
  }
  ...
  private setupServer(): void {
    this.wss.on('connection', (ws: WebSocket, req) => {
      ...
      ws.on('close', (code, reason) => {
        ...
        if (connection) {
          this.logger.info(
            `Agent disconnected: ${connection.agentId || tempId} (code: ${code})`
          );
  ...
  private getServerCapabilities(): string[] {
    const capabilities = ['discovery', 'market-data', 'subscriptions'];
