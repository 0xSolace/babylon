...
export class AuthManager {
  private sessions: Map<
    string,
    { address: string; tokenId: number; expiresAt: number }
  > = new Map();
  ...
  async authenticate(credentials: AgentCredentials): Promise<AuthResult> {
    ...
    if (this.registryClient?.verifyAgent) {
      isValid = await this.registryClient.verifyAgent(
        credentials.address,
        credentials.tokenId
      );
      ...
      if (isValid) {
        logger.debug(
          `Agent verified via ERC-8004: ${credentials.address}:${credentials.tokenId}`
        );
      }
    }

    if (!isValid && this.agent0Client) {
      const profile = await this.agent0Client.getAgentProfile(
        credentials.tokenId
      );
      ...
      if (
        ...
        isValid = true;
        logger.debug(
          `Agent verified via Agent0: ${credentials.address}:${credentials.tokenId}`
        );
      }
    }
    ...
    if (!this.registryClient && !this.agent0Client) {
      logger.warn(
        'No registry client or Agent0 client available - skipping agent verification (development mode)'
      );
      isValid = true;
    }
    ...
    if (!isValid) {
      return {
        success: false,
        error: 'Agent does not own the specified token ID',
      };
    }
  ...
  getSession(
    sessionToken: string
  ): { address: string; tokenId: number } | null {
    const session = this.sessions.get(sessionToken);
