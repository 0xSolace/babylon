...
export class MessageRouter {
  ...
  private server: {
    broadcast: (agentIds: string[], message: unknown) => void;
  } | null = null; // WebSocket server for broadcasting
  ...
  private async handleDiscover(
    ...
    if (agents.length === 0 && this.registryClient?.discoverAgents) {
      agents = await this.registryClient.discoverAgents(
        discoverRequest.filters
      );
  ...
  private async handleGetAgentInfo(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    if (agentInfo.agentId.startsWith('agent0-')) {
      ...
      if (!isNaN(tokenId)) {
        ...
        if (this.unifiedDiscovery) {
          const profile = await this.unifiedDiscovery.getAgent(
            agentInfo.agentId
          );
          if (profile) {
            return {
  ...
  private async handleGetMarketData(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
  ...
  private async handleGetMarketPrices(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
  ...
  private async handleSubscribeMarket(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    const subscriptionRequest =
      request.params as unknown as SubscribeMarketParams;

    if (
      !subscriptionRequest.marketId ||
      typeof subscriptionRequest.marketId !== 'string'
    ) {
      return this.errorResponse(
        request.id,
        ErrorCode.INVALID_PARAMS,
  ...
  private async handleProposeCoalition(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
  ...
  private async handleJoinCoalition(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    if (
      !joinRequest.coalitionId ||
      typeof joinRequest.coalitionId !== 'string'
    ) {
      return this.errorResponse(
        request.id,
        ErrorCode.INVALID_PARAMS,
  ...
  private async handleCoalitionMessage(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    const recipients = coalition.members.filter(
      (memberId) => memberId !== agentId
    );
    ...
    if (this.server && recipients.length > 0) {
      ...
      logger.debug(
        `Broadcasted coalition message to ${recipients.length} members`
      );
    }
  ...
  private async handleLeaveCoalition(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    if (
      !leaveRequest.coalitionId ||
      typeof leaveRequest.coalitionId !== 'string'
    ) {
      return this.errorResponse(
        request.id,
        ErrorCode.INVALID_PARAMS,
  ...
  private async handleShareAnalysis(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
    ...
    const analysisId = this.analysisService.storeAnalysis(
      analysis.marketId,
      analysis
    );

    logger.info(
      `Agent ${agentId} shared analysis for market ${analysis.marketId}`
    );
    ...
    if (subscribers && subscribers.size > 0 && this.server) {
      ...
      if (recipients.length > 0) {
        ...
        logger.debug(
          `Distributed analysis to ${recipients.length} subscribers`
        );
      }
    }
  ...
  private async handleRequestAnalysis(
    ...
    logger.info(
      `Agent ${agentId} requesting analysis for market ${analysisRequest.marketId}, deadline: ${analysisRequest.deadline}`
    );
    ...
    if (subscribers && subscribers.size > 0 && this.server) {
      ...
      if (recipients.length > 0) {
        ...
        logger.debug(
          `Broadcasted analysis request to ${recipients.length} agents`
        );
      }
    }
   ...
   */
  private async handleGetAnalyses(
    agentId: string,
    request: JsonRpcRequest
  ): Promise<JsonRpcResponse> {
  ...
  private async handlePaymentReceipt(
    ...
    const storedPaymentRequest = this.x402Manager.getPaymentRequest(
      receipt.requestId
    );
    ...
    const verificationResult: PaymentVerificationResult =
      await this.x402Manager.verifyPayment(verificationData);
