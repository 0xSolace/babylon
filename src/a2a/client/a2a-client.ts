...
import { A2AEventType, A2AMethod } from '../types';
...
export class A2AClient extends EventEmitter {
   ...
   */
  private sendRequest<T = JsonRpcResult>(
    method: string,
  ...
  async discoverAgents(
    ...
    return this.sendRequest(A2AMethod.DISCOVER_AGENTS, {
      filters,
      limit,
    } as JsonRpcParams);
  }
  ...
  async getMarketPrices(
    marketId: string
  ): Promise<{ marketId: string; prices: number[]; timestamp: number }> {
    return this.sendRequest(A2AMethod.GET_MARKET_PRICES, { marketId });
  }

  async subscribeMarket(
    marketId: string
  ): Promise<{ subscribed: boolean; marketId: string }> {
    const result = await this.sendRequest<{
      subscribed: boolean;
      marketId: string;
    }>(A2AMethod.SUBSCRIBE_MARKET, { marketId });
  ...
  async joinCoalition(
    coalitionId: string
  ): Promise<{ joined: boolean; coalition: Coalition }> {
  ...
  ): Promise<{ shared: boolean; analysisId: string }> {
    return this.sendRequest(A2AMethod.SHARE_ANALYSIS, {
      analysis,
    } as unknown as JsonRpcParams);
  }
