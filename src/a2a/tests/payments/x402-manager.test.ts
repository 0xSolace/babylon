...
describe('X402Manager', () => {
  ...
  describe('Payment Request Creation', () => {
    ...
    test('should set expiration time correctly', () => {
      ...
      expect(request.expiresAt).toBeGreaterThanOrEqual(
        before + testConfig.paymentTimeout
      );
      expect(request.expiresAt).toBeLessThanOrEqual(
        after + testConfig.paymentTimeout + 100
      );
    });
  ...
  describe('Pending Payments', () => {
    test('should get pending payments for agent', () => {
      ...
      expect(pending.some((p) => p.requestId === request1.requestId)).toBe(
        true
      );
      expect(pending.some((p) => p.requestId === request2.requestId)).toBe(
        true
      );
    });

    test('should return empty array for agent with no pending payments', () => {
      const pending = x402Manager.getPendingPayments(
        '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb'
      );
