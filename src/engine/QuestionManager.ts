...
import type { DayTimeline, Organization, Question, Scenario, SelectedActor } from '@/shared/types';
  ...
  async generateDailyQuestions(params: QuestionCreationParams): Promise<Question[]> {
    ...
    const questions: Question[] = response.questions.slice(0, numToGenerate).map((q, index) => {
      ...
      return {
        ...
        resolutionDate: dateOnly || isoString,
        status: 'active',
      };
    });
...
`
    ...
    const shuffledOrgs = shuffleArray(organizations.filter((o) => o.type === 'company'));
  ...
  async generateResolutionEvent(
    ...
    const rawResponse = await this.llm.generateJSON<
      { event: string; type: string } | { response: { event: string; type: string } }
    ...
    return (
      response.event || `Resolution: ${question.text} outcome is ${question.outcome ? 'YES' : 'NO'}`
    );
  }
