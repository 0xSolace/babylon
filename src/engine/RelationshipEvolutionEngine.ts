      ...
      for (let j = i + 1; j < actors.length && relationshipCount < targetCount; j++) {
        ...
        if (hasSharedContext || randomChance) {
          ...
          if (this.llm && sharedOrgs.length > 0) {
            ...
            const primaryOrgId = sharedOrgs[0];
            const org = primaryOrgId ? orgMap.get(primaryOrgId) : undefined;
...
Return JSON: { "description": "...", "type": "...", "sentiment": 0.0 }`;
    ...
    if (!response) {
      throw new Error('LLM did not return relationship summary');
    }
  ...
  async trackInteraction(interaction: Omit<Interaction, 'timestamp'>): Promise<void> {
    ...
    const id1 = sorted[0] ?? interaction.actor1Id;
    const id2 = sorted[1] ?? interaction.actor2Id;
...
Return JSON: { "description": "...", "type": "...", "sentiment": 0.0 }`;
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          logger.warn(
            `Relationship description generation attempt ${attempt + 1}/${maxRetries} failed`,
            {
              error,
              actor1: actor1.name,
              actor2: actor2.name,
