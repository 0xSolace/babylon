...
import type { Actor, FeedPost, GeneratedGame, WorldEvent } from '@/shared/types';
...
function calculateCertainty(
  ...
  const relevantEvents = events.filter(
    (e) =>
      e.relatedQuestion === questionId && e.pointsToward !== null && e.pointsToward !== undefined
  );
  ...
  const correctSignals = relevantEvents.filter(
    (e) =>
      (e.pointsToward === 'YES') === actualOutcome || (e.pointsToward === 'NO') === !actualOutcome
  ).length;
 ...
 */
function _calculateGroupAccuracy(game: GeneratedGame, npcs: Actor[]): number {
  let total = 0;
  let correct = 0;
...
describe('Game Learnability Integration Tests', () => {
  test(
    'CRITICAL: information gradient exists (early unclear, late clear)',
    async () => {
      if (!hasLLMKey) {
        console.log(
          '⏭️  Skipping - No LLM API key available (WANDB_API_KEY, GROQ_API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY required)'
        );
        return;
      }
      ...
      for (const question of game.setup.questions) {
        ...
        expect(lateCertainty).toBeGreaterThan(0.7); // Late: clear (>70%)
  ...
  test(
    'NPCs with high reliability are consistently accurate',
    async () => {
      if (!hasLLMKey) {
      ...
      const highReliabilityNPCs = allActors.filter((a) => a.persona && a.persona.reliability > 0.7);
      ...
      for (const npc of highReliabilityNPCs) {
        const posts = game.timeline
          ...
          .filter(
            (post) =>
              post.author === npc.id && post.pointsToward !== null && post.relatedQuestion !== null
          );
  ...
  test(
    'simple betting strategy beats random guessing',
    async () => {
      if (!hasLLMKey) {
        console.log('⏭️  Skipping - No LLM API key available');
        return;
      ...
      logger.info(
        'Generating 3 test games (this takes ~3-5 minutes)...',
        undefined,
        'LearnabilityTest'
      );
  ...
  test(
    'group chat information provides measurable advantage',
    async () => {
      if (!hasLLMKey) {
        console.log('⏭️  Skipping - No LLM API key available');
        return;
  ...
  test(
    'questions have resolution verification events',
    async () => {
      if (!hasLLMKey) {
        console.log('⏭️  Skipping - No LLM API key available');
        return;
