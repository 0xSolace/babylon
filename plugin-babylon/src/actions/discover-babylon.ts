...
export const discoverBabylonAction: Action = {
  ...
  description:
    'Discover or refresh Babylon game endpoints from Agent0 registry. Use when you need to find the game or update endpoint information.',
  ...
  handler: async (
    ...
    const discoveryService = runtime.getService('babylon-discovery') as {
      discoverAndConnect: () => Promise<{
        name: string;
        endpoints: { api: string; a2a: string; mcp: string };
        capabilities: { markets: string[] };
        tokenId: number;
        reputation?: { trustScore: number };
      }>;
    };
