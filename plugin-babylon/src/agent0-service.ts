...
import {
  getUnifiedDiscoveryService,
  UnifiedDiscoveryService,
} from '../../src/agents/agent0/UnifiedDiscovery';
...
export class Agent0Service extends Service {
  ...
  async initialize(): Promise<void> {
    ...
    const privateKey =
      process.env.BABYLON_AGENT_PRIVATE_KEY || process.env.AGENT0_PRIVATE_KEY;
  ...
  static override async stop(runtime: IAgentRuntime): Promise<void> {
    ...
    const service = runtime.getService<Agent0Service>(
      Agent0Service.serviceType
    )!;
    await service.stop();
  }
}
