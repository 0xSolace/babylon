...
describe('Blockhash', () => {
  ...
  for (const supported of [true, false]) {
    describe(`${supported ? 'supported' : 'unsupported'} chain`, () => {
      beforeEach(async function () {
        if (supported) {
          await this.systemSigner.sendTransaction({
            to: predeploy.eip2935,
            data: this.latestBlock.hash,
          });
        } else {
      ...
      it('future block', async function () {
        ...
        await expect(this.mock.$blockHash(this.latestBlock.number + 10)).to.eventually.equal(
          ethers.ZeroHash
        );
      });
    });
