...
describe('DoubleEndedQueue', () => {
  ...
  describe('when empty', () => {
    ...
    it('try pops return false/zero on empty', async function () {
      await expect(this.mock.$tryPopFront(0))
        .to.emit(this.mock, 'return$tryPopFront')
        .withArgs(false, ethers.ZeroHash);
      await expect(this.mock.$tryPopBack(0))
        .to.emit(this.mock, 'return$tryPopBack')
        .withArgs(false, ethers.ZeroHash);
    });

    it('try pushes succeed on empty', async function () {
      await expect(this.mock.$tryPushFront(0, bytesA))
        .to.emit(this.mock, 'return$tryPushFront')
        .withArgs(true);
      await expect(this.mock.$tryPushBack(0, bytesB))
        .to.emit(this.mock, 'return$tryPushBack')
        .withArgs(true);
  ...
  describe('when not empty', () => {
    ...
    it('try getters return true/value when not empty', async function () {
      ...
      await expect(this.mock.$tryBack(0)).to.eventually.deep.equal([
        true,
    ...
    it('tryAt returns false/zero on out of bounds', async function () {
      await expect(this.mock.$tryAt(0, this.content.length)).to.eventually.deep.equal([
        false,
        ethers.ZeroHash,
      ]);
    });
