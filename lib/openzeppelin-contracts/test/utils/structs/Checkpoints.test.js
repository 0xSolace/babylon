...
describe('Checkpoints', () => {
  for (const opt of OPTS) {
    describe(opt.historyTypeName, () => {
      const fixture = async () => {
        const mock = await ethers.deployContract('$Checkpoints');
        const methods = {
          at: (...args) => mock.getFunction(`$at_Checkpoints_${opt.historyTypeName}`)(0, ...args),
          latest: (...args) =>
            mock.getFunction(`$latest_Checkpoints_${opt.historyTypeName}`)(0, ...args),
          latestCheckpoint: (...args) =>
            mock.getFunction(`$latestCheckpoint_Checkpoints_${opt.historyTypeName}`)(0, ...args),
          length: (...args) =>
            mock.getFunction(`$length_Checkpoints_${opt.historyTypeName}`)(0, ...args),
          push: (...args) =>
            mock.getFunction(`$push(uint256,${opt.keyTypeName},${opt.valueTypeName})`)(0, ...args),
          lowerLookup: (...args) =>
            mock.getFunction(`$lowerLookup(uint256,${opt.keyTypeName})`)(0, ...args),
          upperLookup: (...args) =>
            mock.getFunction(`$upperLookup(uint256,${opt.keyTypeName})`)(0, ...args),
