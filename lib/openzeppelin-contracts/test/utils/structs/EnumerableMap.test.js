...
const {
  ...
  typeDescr,
  toMapTypeDescr,
} = require('../../../scripts/generate/templates/Enumerable.opts');
...
MAP_TYPES.unshift(
  toMapTypeDescr({
    key: typeDescr({ type: 'bytes32' }),
    value: typeDescr({ type: 'bytes32' }),
  })
);
      ...
      name,
      {
        ...
        error:
          key.memory || value.memory
            ? `EnumerableMapNonexistent${key.name}Key`
...
describe('EnumerableMap', () => {
  ...
  for (const { name, key, value } of MAP_TYPES) {
    describe(`${name} (enumerable map from ${key.type} to ${value.type})`, () => {
      beforeEach(async function () {
