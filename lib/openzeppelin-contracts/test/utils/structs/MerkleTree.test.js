  ...
  describe('push', () => {
    it('pushing correctly updates the tree', async function () {
      ...
      for (const i in range(2 ** DEPTH)) {
        ...
        // push value to tree
        await expect(this.mock.push(hash))
          .to.emit(this.mock, 'LeafInserted')
          .withArgs(hash, i, tree.root);
  ...
  describe('update', () => {
    for (const { leafCount, leafIndex } of range(2 ** DEPTH + 1).flatMap((leafCount) =>
      ...
      it(`updating a leaf correctly updates the tree (leaf #${leafIndex + 1}/${leafCount})`, async function () {
        ...
        // perform update
        await expect(
          this.mock.update(leafIndex, oldLeafHash, newLeafHash, oldTree.getProof(leafIndex))
        )
    ...
    it('replacing a leaf using an invalid proof reverts', async function () {
      ...
      await expect(
        this.mock.update(leafIndex, oldLeafHash, newLeafHash, proof)
      ).to.be.revertedWithCustomError(this.mock, 'MerkleTreeUpdateInvalidProof');
    });
  });
