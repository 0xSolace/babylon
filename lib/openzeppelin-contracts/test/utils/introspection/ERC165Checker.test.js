...
describe('ERC165Checker', () => {
  ...
  describe('ERC165 missing return data', () => {
    ...
    it('does not support mock interface via supportsAllInterfaces', async function () {
      await expect(this.mock.$supportsAllInterfaces(this.target, [DUMMY_ID])).to.eventually.be
        .false;
    });

    it('does not support mock interface via getSupportedInterfaces', async function () {
      await expect(
        this.mock.$getSupportedInterfaces(this.target, [DUMMY_ID])
      ).to.eventually.deep.equal([false]);
    });

    it('does not support mock interface via supportsERC165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsERC165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually
        .be.false;
    });
  });
  ...
  describe('ERC165 revert on invalid interface', () => {
    beforeEach(async function () {
    ...
    it('support mock interface via supportsERC165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsERC165InterfaceUnchecked(this.target, '0xffffffff')).to
        .eventually.be.false;
      await expect(this.mock.$supportsERC165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually
        .be.true;
    });
  ...
  describe('ERC165 and many interfaces supported', () => {
    ...
    it('supports all interfaceIds via supportsAllInterfaces', async function () {
      await expect(this.mock.$supportsAllInterfaces(this.target, supportedInterfaces)).to.eventually
        .be.true;
    });
    ...
    it('supports none of the interfaces queried via supportsAllInterfaces', async function () {
      ...
      await expect(this.mock.$supportsAllInterfaces(this.target, interfaceIdsToTest)).to.eventually
        .be.false;
    });
    ...
    it('supports all interfaceIds via getSupportedInterfaces', async function () {
      ...
      ).to.eventually.deep.equal(supportedInterfaces.map((i) => supportedInterfaces.includes(i)));
    });
    ...
    it('supports none of the interfaces queried via getSupportedInterfaces', async function () {
      ...
      ).to.eventually.deep.equal(interfaceIdsToTest.map((i) => supportedInterfaces.includes(i)));
    });
    ...
    it('supports each interfaceId via supportsERC165InterfaceUnchecked', async function () {
      for (const interfaceId of supportedInterfaces) {
        await expect(this.mock.$supportsERC165InterfaceUnchecked(this.target, interfaceId)).to
          .eventually.be.true;
      }
    });
  });
  ...
  describe('account address does not support ERC165', () => {
    ...
    it('does not support mock interface via supportsAllInterfaces', async function () {
      await expect(this.mock.$supportsAllInterfaces(DUMMY_ACCOUNT, [DUMMY_ID])).to.eventually.be
        .false;
    });

    it('does not support mock interface via getSupportedInterfaces', async function () {
      await expect(
        this.mock.$getSupportedInterfaces(DUMMY_ACCOUNT, [DUMMY_ID])
      ).to.eventually.deep.equal([false]);
    });

    it('does not support mock interface via supportsERC165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsERC165InterfaceUnchecked(DUMMY_ACCOUNT, DUMMY_ID)).to
        .eventually.be.false;
    });
  });
