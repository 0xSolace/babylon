...
describe('ERC7739Utils', () => {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('encodeTypedDataSig', () => {
  ...
  describe('personalSignStructhash', () => {
    it('should produce a personal signature EIP-712 nested type', async function () {
      ...
      await expect(this.mock.$personalSignStructHash(ethers.hashMessage(text))).to.eventually.equal(
        ethers.TypedDataEncoder.hashStruct(
          'PersonalSign',
          { PersonalSign },
          ERC4337Utils.preparePersonalSign(text)
        )
      );
    });
  });

  describe('typedDataSignStructHash', () => {
  ...
  describe('typedDataSignTypehash', () => {
    it('should match', async function () {
      const typedDataSignType = ethers.TypedDataEncoder.from(details.allTypes).encodeType(
        'TypedDataSign'
      );
