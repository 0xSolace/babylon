...
describe('SignatureChecker (ERC1271)', () => {
    ...
    for (const fn of [
      ...
      describe(fn, () => {
        ...
        it('with invalid signer', async function () {
          await expect(
            this.mock.getFunction(`$${fn}`)(
              ethers.Typed.address(this.mock.target),
              TEST_MESSAGE_HASH,
              this.signature
            )
          ).to.eventually.be.false;
  ...
  describe('ERC7913', () => {
    describe('isValidSignatureNow', () => {
      ...
      describe('with ERC-1271 wallet', () => {
        it('with matching signer and signature', async function () {
          const walletSigner = ethers.zeroPadValue(this.wallet.target, 20);
          const signature = await this.signer.signMessage(TEST_MESSAGE);
          await expect(
            this.mock.$isValidSignatureNow(
              ethers.Typed.bytes(walletSigner),
              TEST_MESSAGE_HASH,
              signature
            )
          ).to.eventually.be.true;
      ...
      describe('with ERC-7913 verifier', () => {
        it('with matching signer and signature', async function () {
          ...
          await expect(
            this.mock.$isValidSignatureNow(ethers.Typed.bytes(signer), TEST_MESSAGE_HASH, signature)
          ).to.eventually.be.true;
        });
    ...
    describe('areValidSignaturesNow', () => {
      ...
      it('should validate a single signature', async function () {
        ...
        await expect(this.mock.$areValidSignaturesNow(TEST_MESSAGE_HASH, [signer], [signature])).to
          .eventually.be.true;
      });
      ...
      it('should pass with empty arrays', async function () {
        await expect(this.mock.$areValidSignaturesNow(TEST_MESSAGE_HASH, [], [])).to.eventually.be
          .true;
      });
    });
  });
