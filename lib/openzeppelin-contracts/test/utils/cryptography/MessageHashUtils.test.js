...
describe('MessageHashUtils', () => {
  ...
  describe('toEthSignedMessageHash', () => {
    it('prefixes bytes32 data correctly', async function () {
      ...
      await expect(
        this.mock.getFunction('$toEthSignedMessageHash(bytes32)')(message)
    ...
    it('prefixes dynamic length data correctly', async function () {
      ...
      await expect(
        this.mock.getFunction('$toEthSignedMessageHash(bytes)')(message)
      ).to.eventually.equal(expectedHash);
    });
  ...
  describe('toDataWithIntendedValidatorHash', () => {
    it('returns the digest of `bytes32 messageHash` correctly', async function () {
      ...
      await expect(
        this.mock.getFunction('$toDataWithIntendedValidatorHash(address,bytes32)')(
          verifier,
          message
        )
      ).to.eventually.equal(expectedHash);
    ...
    it('version match for bytes32', async function () {
      const verifier = ethers.Wallet.createRandom().address;
      const message = ethers.randomBytes(32);
      const fixed = await this.mock.getFunction('$toDataWithIntendedValidatorHash(address,bytes)')(
        verifier,
        message
      );
      const dynamic = await this.mock.getFunction(
        '$toDataWithIntendedValidatorHash(address,bytes32)'
      )(verifier, message);
