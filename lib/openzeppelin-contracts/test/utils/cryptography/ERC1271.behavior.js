...
function shouldBehaveLikeERC1271({ erc7739 = false } = {}) {
  ...
  describe(`supports ERC-${erc7739 ? 7739 : 1271}`, () => {
    beforeEach(async function () {
      // if deploy function is present, check that code is already in place
      if (this.mock.deploy) {
        await ethers.provider
          .getCode(this.mock.address)
          .then((code) => code !== '0x' || this.mock.deploy());
      }
    ...
    describe('PersonalSign', () => {
      ...
      it('returns false for an invalid personal signature', async function () {
        ...
        await expect(this.mock.isValidSignature(hash, signature)).to.eventually.not.equal(
          MAGIC_VALUE
        );
      });
    });

    describe('TypedDataSign', () => {
      beforeEach(async function () {
