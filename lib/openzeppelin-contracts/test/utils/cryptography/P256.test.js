...
describe('P256', () => {
  ...
  describe('with signature', () => {
    ...
    it('verify valid signature', async function () {
      await expect(this.mock.$verify(this.messageHash, ...this.signature, ...this.publicKey)).to
        .eventually.be.true;
      await expect(
        this.mock.$verifySolidity(this.messageHash, ...this.signature, ...this.publicKey)
    ...
    it('verify improper signature', async function () {
      ...
      await expect(this.mock.$verify(this.messageHash, ...signature, ...this.publicKey)).to
        .eventually.be.false;
      await expect(this.mock.$verifySolidity(this.messageHash, ...signature, ...this.publicKey)).to
        .eventually.be.false;
      await expect(this.mock.$verifyNative(this.messageHash, ...signature, ...this.publicKey)).to
        .eventually.be.false;
    });
    ...
    it('recover public key', async function () {
      ...
      ).to.eventually.deep.equal(this.publicKey);
    });

    it('recovers (0,0) for invalid recovery bit', async function () {
      await expect(
        this.mock.$recovery(this.messageHash, 2, ...this.signature)
      ).to.eventually.deep.equal([ethers.ZeroHash, ethers.ZeroHash]);
    });
    ...
    it('recovers (0,0) for improper signature', async function () {
      ...
      this.signature[0] = ethers.toBeHex(N, 0x20); // r = N
      await expect(
        this.mock.$recovery(this.messageHash, this.recovery, ...signature)
      ).to.eventually.deep.equal([ethers.ZeroHash, ethers.ZeroHash]);
    });
    ...
    for (const { key, tests } of require('./ecdsa_secp256r1_sha256_p1363_test.json').testGroups) {
      ...
      for (const { tcId, comment, msg, sig, result } of tests) {
        ...
        it(`${tcId}: ${comment}`, async function () {
          ...
          // check verify
          await expect(this.mock.$verify(messageHash, r, s, x, y)).to.eventually.equal(
            result === 'valid'
          );
        });
      }
