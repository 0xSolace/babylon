...
describe('ECDSA', () => {
  ...
  describe('recover with valid signature', () => {
    describe('using <signer>.sign', () => {
      it('returns signer address with correct signature', async function () {
        ...
        await expect(
          this.mock.$recover(ethers.hashMessage(TEST_MESSAGE), signature)
        ).to.eventually.equal(this.signer);
        await expect(
          this.mock.$recoverCalldata(ethers.hashMessage(TEST_MESSAGE), signature)
        ).to.eventually.equal(this.signer);
      });
      ...
      it('returns signer address with correct signature for arbitrary length message', async function () {
        ...
        // Recover the signer address from the generated message and signature.
        await expect(
          this.mock.$recover(ethers.hashMessage(NON_HASH_MESSAGE), signature)
        ).to.eventually.equal(this.signer);
        await expect(
      ...
      it('returns a different address', async function () {
        const signature = await this.signer.signMessage(TEST_MESSAGE);
        await expect(this.mock.$recover(WRONG_MESSAGE, signature)).to.eventually.not.equal(
          this.signer
        );
        await expect(this.mock.$recoverCalldata(WRONG_MESSAGE, signature)).to.eventually.not.equal(
          this.signer
        );
      });
      ...
      it('reverts with invalid signature', async function () {
        ...
        await expect(
          this.mock.$recoverCalldata(TEST_MESSAGE, signature)
    ...
    describe('with v=27 signature', () => {
      ...
      it('works with correct v value', async function () {
        ...
        await expect(this.mock.$recoverCalldata(TEST_MESSAGE, signature)).to.eventually.equal(
          signer
        );
      ...
      it('rejects incorrect v value', async function () {
        ...
        await expect(this.mock.$recoverCalldata(TEST_MESSAGE, signature)).to.eventually.not.equal(
          signer
        );
  ...
  describe('parse signature', () => {
    ...
    it('with short signature', async function () {
      ...
      await expect(this.mock.$parse(signature)).to.eventually.deep.equal([
        0n,
        ethers.ZeroHash,
        ethers.ZeroHash,
      ]);
