...
describe('Memory', () => {
  ...
  describe('Slices', () => {
    ...
    it('slice(Slice,uint256)', async function () {
      ...
      await expect(
        this.mock.getFunction('$slice(bytes32,uint256)')(
          formatSlice({ length: 10n, ptr: 256n }),
          0n
        )
    ...
    it('slice(Slice,uint256,uint256)', async function () {
      await expect(
        this.mock.getFunction('$slice(bytes32,uint256,uint256)')(
          formatSlice({ length: 0n, ptr: 256n }),
          0n,
          0n
        )
      ).to.eventually.equal(formatSlice({ length: 0n, ptr: 256n }));
