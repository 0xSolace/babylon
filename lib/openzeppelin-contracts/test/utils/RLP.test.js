...
describe('RLP', () => {
  ...
  it('encodes array (bytes[])', async function () {
    for (const input of [
      ...
      await expect(this.mock.$encode_list(input.map(ethers.encodeRlp))).to.eventually.equal(
        ethers.encodeRlp(input)
      );
    }
  });
  ...
  invalidTests.forEach(({ name, input }) => {
    it(`rejects ${name}`, async function () {
      await expect(this.mock.$decodeBytes(input)).to.be.revertedWithCustomError(
        this.mock,
        'RLPInvalidEncoding'
      );
    });
