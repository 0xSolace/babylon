...
describe('ERC7390', () => {
    ...
    for (const { title, name } of [
      ...
      it(title, async function () {
        ...
        if (chainType === 'eip155') {
          ...
          if (!address) {
            await expect(
              this.mock.$formatEvmV1(ethers.Typed.uint256(reference))
  ...
  describe('invalid format', () => {
    for (const [title, binary] of Object.entries({
      ...
      it(title, async function () {
        ...
        await expect(this.mock.$parseEvmV1Calldata(binary))
          .to.be.revertedWithCustomError(this.mock, 'InteroperableAddressParsingError')
          .withArgs(binary);
        await expect(this.mock.$tryParseV1(binary)).to.eventually.deep.equal([
          false,
          '0x0000',
          '0x',
          '0x',
    ...
    for (const [title, binary] of Object.entries({
      ...
      'not an evm format: address in not 20 bytes':
        '0x00010000010112d8da6bf26964af9d7eed9e03e53415d37aa9',
    })) {
      it(title, async function () {
        await expect(this.mock.$parseEvmV1(binary))
          .to.be.revertedWithCustomError(this.mock, 'InteroperableAddressParsingError')
