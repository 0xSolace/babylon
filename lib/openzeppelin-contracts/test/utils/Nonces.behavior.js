...
function shouldBehaveLikeNoncesKeyed() {
  describe('should support nonces with keys', () => {
    ...
    it('gets a nonce', async function () {
      await expect(this.mock.nonces(sender, ethers.Typed.uint192(0n))).to.eventually.equal(
        keyOffset(0n) + 0n
      );
      await expect(this.mock.nonces(sender, ethers.Typed.uint192(17n))).to.eventually.equal(
        keyOffset(17n) + 0n
      );
    });
