...
describe('Math', () => {
  ...
  describe('saturatingAdd', () => {
    ...
    it('bounds on addition overflow', async function () {
      ...
      await expect(
        this.mock.$saturatingAdd(ethers.MaxUint256, ethers.MaxUint256)
  ...
  describe('saturatingSub', () => {
    it('subtracts correctly', async function () {
      ...
      await expect(this.mock.$saturatingSub(ethers.MaxUint256, 1n)).to.eventually.equal(
    ...
    it('bounds on subtraction overflow', async function () {
      ...
      await expect(
        this.mock.$saturatingSub(ethers.MaxUint256 - 1n, ethers.MaxUint256)
      ).to.eventually.equal(0n);
    });
  });

  describe('saturatingMul', () => {
    it('multiplies correctly', async function () {
      const a = 1234n;
      const b = 5678n;
  ...
  describe('mulDiv', () => {
    ...
    describe('does round down', () => {
      ...
      it('large values', async function () {
        for (const rounding of RoundingDown) {
          await expect(
            this.mock.$mulDiv(42n, ethers.MaxUint256 - 1n, ethers.MaxUint256, rounding)
          ).to.eventually.equal(41n);

          await expect(
            this.mock.$mulDiv(17n, ethers.MaxUint256, ethers.MaxUint256, rounding)
          ).to.eventually.equal(17n);
  ...
  describe('mulShr', () => {
    ...
    describe('does round down', () => {
      ...
      it('large values', async function () {
        for (const rounding of RoundingDown) {
          await expect(
            this.mock.$mulShr(42n, ethers.MaxUint256, 255n, rounding)
          ).to.eventually.equal(83n);

          await expect(
            this.mock.$mulShr(17n, ethers.MaxUint256, 255n, rounding)
          ).to.eventually.equal(33n);

          await expect(
            this.mock.$mulShr(ethers.MaxUint256, ethers.MaxInt256 + 1n, 255n, rounding)
          ).to.eventually.equal(ethers.MaxUint256);

          await expect(
            this.mock.$mulShr(ethers.MaxUint256, ethers.MaxInt256, 255n, rounding)
          ).to.eventually.equal(ethers.MaxUint256 - 2n);
        }
      });
    });
    ...
    describe('does round up', () => {
      ...
      it('large values', async function () {
        for (const rounding of RoundingUp) {
          await expect(
            this.mock.$mulShr(42n, ethers.MaxUint256, 255n, rounding)
          ).to.eventually.equal(84n);

          await expect(
            this.mock.$mulShr(17n, ethers.MaxUint256, 255n, rounding)
          ).to.eventually.equal(34n);
    ...
    for (const [name, type] of Object.entries({ uint256, bytes })) {
      describe(`with ${name} inputs`, () => {
        it('is correctly calculating modulus', async function () {
          ...
          await expect(this.mock.$modExp(type(b), type(e), type(m))).to.eventually.equal(
            type(b ** e % m).value
          );
        });
      ...
      for (const [[b, log2b], [e, log2e], [m, log2m]] of product(
        ...
        it(`calculates b ** e % m (b=2**${log2b}+1) (e=2**${log2e}+1) (m=2**${log2m}+1)`, async function () {
          ...
          await expect(this.mock.$tryModExp(bytes(b), bytes(e), bytes(m))).to.eventually.deep.equal(
            [true, bytes(modExp(b, e, m), mLength).value]
          );
        });
      }
    });
