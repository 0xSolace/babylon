...
describe('RelayedCall', () => {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('default (zero) salt', () => {
    beforeEach(async function () {
    ...
    it('automatic relayer deployment', async function () {
      ...
      await expect(this.mock.$getRelayer())
        .to.emit(this.mock, 'return$getRelayer')
    ...
    describe('relayed call', () => {
      ...
      it('target success (with value)', async function () {
        ...
        await expect(tx)
          .to.emit(this.mock, 'return$relayCall_address_uint256_bytes')
          .withArgs(true, '0x');
      });
      ...
      it('target revert', async function () {
        ...
        await expect(tx)
          ...
          .withArgs(
            false,
            this.target.interface.encodeErrorResult('AccessManagedUnauthorized', [this.relayer])
          );
      });
    });
    ...
    it('input format', async function () {
      ...
      await expect(
        mockAsWallet.sendTransaction({
          to: this.relayer,
          data: '0x7859821024E633C5dC8a4FcF86fC52e7720Ce5',
        })
      ).to.be.revertedWithoutReason();

      // 0 bytes (not enough for an address) - REVERT
      await expect(
        mockAsWallet.sendTransaction({ to: this.relayer, data: '0x' })
      ).to.be.revertedWithoutReason();
    });
  });
  ...
  describe('random salt', () => {
    ...
    describe('relayed call', () => {
      ...
      it('target success (with value)', async function () {
        ...
        await expect(tx)
          .to.emit(this.mock, 'return$relayCall_address_uint256_bytes_bytes32')
          .withArgs(true, '0x');
      });
