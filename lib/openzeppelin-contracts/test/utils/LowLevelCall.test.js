...
describe('LowLevelCall', () => {
  ...
  describe('call', () => {
    describe('without any return', () => {
      ...
      it('calls the requested function with value and returns true', async function () {
        ...
        await expect(tx)
          .to.emit(this.mock, 'return$callNoReturn_address_uint256_bytes')
          .withArgs(true);
      });
    ...
    describe('with 64 bytes return in the scratch space', () => {
      it('calls the requested function and returns true', async function () {
        await expect(
          this.mock.$callReturn64Bytes(
            this.target,
            this.target.interface.encodeFunctionData('mockFunctionWithArgsReturn', [
              returnValue1,
              returnValue2,
            ])
          )
      ...
      it('calls the requested function and returns true', async function () {
        await expect(
          this.mock.$staticcallReturn64Bytes(
            this.target,
            this.target.interface.encodeFunctionData('mockStaticFunctionWithArgsReturn', [
              returnValue1,
              returnValue2,
            ])
          )
  ...
  describe('delegate', () => {
    describe('without any return', () => {
      it('calls the requested function and returns true', async function () {
        await expect(ethers.provider.getStorage(this.mock, storageSlot)).to.eventually.equal(
          ethers.ZeroHash
        );
