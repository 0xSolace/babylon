...
describe('Arrays', () => {
          ...
          describe(name, () => {
            it('[deprecated] findUpperBound', async function () {
              ...
              if (hasDuplicates(array)) {
                await expect(this.instance.findUpperBound(input)).to.eventually.equal(
            ...
            it('lowerBound', async function () {
              await expect(this.instance.lowerBound(input)).to.eventually.equal(
                lowerBound(array, input)
              );
              await expect(this.instance.lowerBoundMemory(array, input)).to.eventually.equal(
                lowerBound(array, input)
              );
            });

            it('upperBound', async function () {
              await expect(this.instance.upperBound(input)).to.eventually.equal(
                upperBound(array, input)
              );
              await expect(this.instance.upperBoundMemory(array, input)).to.eventually.equal(
                upperBound(array, input)
              );
            });
          });
  ...
  for (const { name, isValueType } of TYPES) {
    ...
    describe(name, () => {
      const fixture = async () => {
        return {
          instance: await ethers.deployContract(`${capitalize(name)}ArraysMock`, [elements]),
        };
      };
      ...
      if (isValueType) {
        describe('sort', () => {
          for (const length of [0, 1, 2, 8, 32, 128]) {
            describe(`${name}[] of length ${length}`, () => {
              beforeEach(async function () {
              ...
              afterEach(async function () {
                ...
                await expect(this.instance.sortReverse(this.array)).to.eventually.deep.equal(
                  reversed
                );
              });
        ...
        for (const fn of ['slice', 'splice']) {
          ...
          describe(fn, () => {
            ...
            it(`${fn} from start to end of array`, async function () {
              const start = 3;
              await expect(this.mock[fragment](array, start)).to.eventually.deep.equal(
                array.slice(start)
              );
            });
            ...
            it(`${fn} entire array`, async function () {
              ...
              await expect(
                this.mock[rangeFragment](array, 0, array.length)
            ...
            it(`${fn} with out of bounds indices`, async function () {
              ...
              const start = 5;
              await expect(
                this.mock[rangeFragment](array, start, array.length + 10)
              ).to.eventually.deep.equal(array.slice(start));
            });
            ...
            it(`${fn} single element`, async function () {
              ...
              await expect(this.mock[fragment](singleArray, 0)).to.eventually.deep.equal(
                singleArray
              );
              await expect(this.mock[fragment](singleArray, 1)).to.eventually.deep.equal([]);
              await expect(this.mock[rangeFragment](singleArray, 0, 1)).to.eventually.deep.equal(
                singleArray
              );
            });
          });
        ...
        describe('memory', () => {
          ...
          it('unsafeMemoryAccess loop around', async function () {
            for (let i = 251n; i < 256n; ++i) {
              ...
              await expect(this.mock[fragment](elements, 2n ** i + 0n)).to.eventually.equal(
                elements[0]
              );
              await expect(this.mock[fragment](elements, 2n ** i + 1n)).to.eventually.equal(
                elements[1]
