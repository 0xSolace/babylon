...
describe('Bytes', () => {
  ...
  describe('indexOf', () => {
    it('first', async function () {
      await expect(this.mock.$indexOf(lorem, ethers.toBeHex(present))).to.eventually.equal(
    ...
    it('absent', async function () {
      await expect(this.mock.$indexOf(lorem, ethers.toBeHex(absent))).to.eventually.equal(
        ethers.MaxUint256
      );
    });
    ...
    it('empty buffer', async function () {
      ...
      await expect(this.mock.$indexOf('0x', '0x00', ethers.Typed.uint256(17))).to.eventually.equal(
        ethers.MaxUint256
      );
    });
  });
  ...
  describe('lastIndexOf', () => {
    ...
    it('absent', async function () {
      await expect(this.mock.$lastIndexOf(lorem, ethers.toBeHex(absent))).to.eventually.equal(
        ethers.MaxUint256
      );
    });
    ...
    it('empty buffer', async function () {
      ...
      await expect(
        this.mock.$lastIndexOf('0x', '0x00', ethers.Typed.uint256(17))
      ).to.eventually.equal(ethers.MaxUint256);
    });
  });

  describe('slice & splice', () => {
    describe('slice(bytes, uint256) & splice(bytes, uint256)', () => {
    ...
    describe('slice(bytes, uint256, uint256) & splice(bytes, uint256, uint256)', () => {
      for (const [descr, [start, end]] of Object.entries({
        ...
        it(descr, async function () {
          ...
          await expect(
            this.mock.$slice(lorem, start, ethers.Typed.uint256(end))
          ).to.eventually.equal(result);
          await expect(
            this.mock.$splice(lorem, start, ethers.Typed.uint256(end))
          ).to.eventually.equal(result);
        });
      }
    });
  ...
  describe('clz bytes', () => {
    ...
    it('all zeros of various lengths', async function () {
      ...
      await expect(this.mock.$clz(`0x${'00'.repeat(40)}01${'00'.repeat(9)}`)).to.eventually.equal(
        327
      ); // 40*8+7
      ...
      await expect(this.mock.$clz(`0x${'00'.repeat(32)}80${'00'.repeat(31)}`)).to.eventually.equal(
        256
      ); // 32*8
      await expect(this.mock.$clz(`0x${'00'.repeat(32)}01${'00'.repeat(31)}`)).to.eventually.equal(
        263
      ); // 32*8+7
      ...
      await expect(this.mock.$clz(`0x${'00'.repeat(16)}01${'00'.repeat(15)}`)).to.eventually.equal(
        135
      ); // 16*8+7
      await expect(this.mock.$clz(`0x${'00'.repeat(32)}01${'00'.repeat(31)}`)).to.eventually.equal(
        263
      ); // 32*8+7
      await expect(this.mock.$clz(`0x${'00'.repeat(48)}01${'00'.repeat(47)}`)).to.eventually.equal(
        391
      ); // 48*8+7
      await expect(this.mock.$clz(`0x${'00'.repeat(64)}01${'00'.repeat(63)}`)).to.eventually.equal(
        519
      ); // 64*8+7
    });
  });
  ...
  describe('reverseBits', () => {
    ...
    describe('reverseBytes16', () => {
      it('reverses bytes correctly', async function () {
        await expect(this.mock.$reverseBytes16(bytes16(0))).to.eventually.equal(bytes16(0));
        await expect(this.mock.$reverseBytes16(bytes16(MAX_UINT128))).to.eventually.equal(
          bytes16(MAX_UINT128)
        );

        // Test complex pattern that clearly shows byte reversal
        await expect(
          this.mock.$reverseBytes16('0x0123456789abcdef0123456789abcdef')
      ...
      it('double reverse returns original', async function () {
        ...
        for (const value of values) {
          ...
          await expect(this.mock.$reverseBytes16(reversed)).to.eventually.equal(
            bytes16(value & MAX_UINT128)
          );
        }
      });
    });

    describe('reverseBytes8', () => {
      it('reverses bytes correctly', async function () {
        await expect(this.mock.$reverseBytes8(bytes8(0))).to.eventually.equal(bytes8(0));
        await expect(this.mock.$reverseBytes8(bytes8(MAX_UINT64))).to.eventually.equal(
          bytes8(MAX_UINT64)
        );

        // Test known pattern: 0x123456789ABCDEF0 -> 0xF0DEBC9A78563412
        await expect(this.mock.$reverseBytes8('0x123456789abcdef0')).to.eventually.equal(
          '0xf0debc9a78563412'
        );
      });
      ...
      it('double reverse returns original', async function () {
        ...
        for (const value of values) {
          ...
          await expect(this.mock.$reverseBytes8(reversed)).to.eventually.equal(
            bytes8(value & MAX_UINT64)
          );
        }
      });
    });

    describe('reverseBytes4', () => {
      it('reverses bytes correctly', async function () {
        await expect(this.mock.$reverseBytes4(bytes4(0))).to.eventually.equal(bytes4(0));
        await expect(this.mock.$reverseBytes4(bytes4(MAX_UINT32))).to.eventually.equal(
          bytes4(MAX_UINT32)
        );

        // Test known pattern: 0x12345678 -> 0x78563412
        await expect(this.mock.$reverseBytes4(bytes4(0x12345678))).to.eventually.equal(
          bytes4(0x78563412)
        );
      });
      ...
      it('double reverse returns original', async function () {
        ...
        for (const value of values) {
          ...
          await expect(this.mock.$reverseBytes4(reversed)).to.eventually.equal(
            bytes4(value & MAX_UINT32)
          );
        }
      });
    });

    describe('reverseBytes2', () => {
      it('reverses bytes correctly', async function () {
        await expect(this.mock.$reverseBytes2(bytes2(0))).to.eventually.equal(bytes2(0));
        await expect(this.mock.$reverseBytes2(bytes2(MAX_UINT16))).to.eventually.equal(
          bytes2(MAX_UINT16)
        );
      ...
      it('double reverse returns original', async function () {
        ...
        for (const value of values) {
          ...
          await expect(this.mock.$reverseBytes2(reversed)).to.eventually.equal(
            bytes2(value & MAX_UINT16)
          );
        }
      });
    });
    ...
    describe('edge cases', () => {
      it('handles single byte values', async function () {
        ...
        await expect(this.mock.$reverseBytes4(bytes4(0x000000ff))).to.eventually.equal(
          bytes4(0xff000000)
        );
      });
      ...
      it('handles alternating patterns', async function () {
        ...
        await expect(this.mock.$reverseBytes4(bytes4(0xaaaaaaaa))).to.eventually.equal(
          bytes4(0xaaaaaaaa)
        );
        await expect(this.mock.$reverseBytes4(bytes4(0x55555555))).to.eventually.equal(
          bytes4(0x55555555)
        );
      });
    });
