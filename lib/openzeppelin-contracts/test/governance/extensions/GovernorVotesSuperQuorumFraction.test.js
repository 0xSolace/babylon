...
describe('GovernorVotesSuperQuorumFraction', () => {
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`using ${Token}`, () => {
      ...
      it('deployment check', async function () {
        ...
        await expect(
          time.clock[mode]().then((clock) => this.mock.superQuorum(clock - 1n))
        ).to.eventually.equal((tokenSupply * superQuorumRatio) / 100n);
      });
      ...
      it('proposal remains active until super quorum is reached', async function () {
        ...
        await expect(this.mock.state(this.proposal.id)).to.eventually.equal(
          ProposalState.Succeeded
        );
      });

      describe('super quorum updates', () => {
        it('updateSuperQuorumNumerator is protected', async function () {
          await expect(
            this.mock.connect(this.owner).updateSuperQuorumNumerator(newSuperQuorumRatio)
          )
            ...
            [
              {
                ...
                data: this.mock.interface.encodeFunctionData('updateSuperQuorumNumerator', [
                  newSuperQuorumRatio,
                ]),
              },
            ],
