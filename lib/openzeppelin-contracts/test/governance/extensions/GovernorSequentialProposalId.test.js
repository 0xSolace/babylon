...
describe('GovernorSequentialProposalId', () => {
    ...
    describe(`using ${Token}`, () => {
      ...
      it('sequential proposal ids', async function () {
        for (const i of iterate.range(1, 10)) {
          ...
          await expect(this.mock.hashProposal(...this.proposal.shortProposal)).to.eventually.equal(
            this.proposal.hash
          );
          await expect(
            this.mock.getProposalId(...this.proposal.shortProposal)
          ).revertedWithCustomError(this.mock, 'GovernorNonexistentProposal');
          ...
          await expect(this.mock.getProposalId(...this.proposal.shortProposal)).to.eventually.equal(
            i
          );
