...
describe('ERC7786Recipient', () => {
  ...
  it('receives gateway relayed messages', async function () {
    await expect(
      this.gateway
        .connect(this.sender)
        .sendMessage(this.toErc7930(this.receiver), payload, attributes, {
          value,
        })
    )
      ...
      .withArgs(
        ethers.ZeroHash,
        this.toErc7930(this.sender),
        this.toErc7930(this.receiver),
        payload,
        value,
        attributes
      )
  ...
  it('multiple use of the same receiveId', async function () {
    ...
    await expect(
      this.receiver
        ...
        .receiveMessage(receiveId, this.toErc7930(this.sender), payload, {
          value,
        })
    )
      .to.emit(this.receiver, 'MessageReceived')
  ...
  it('unauthorized call', async function () {
    await expect(
      this.receiver
        ...
        .receiveMessage(ethers.ZeroHash, this.toErc7930(this.sender), payload)
    )
