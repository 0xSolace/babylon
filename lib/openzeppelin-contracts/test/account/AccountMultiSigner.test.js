...
async function fixture() {
  ...
  const makeMock = (signers, threshold) =>
    helper
      .newAccount('$AccountMultiSignerMock', [signers, threshold, 'AccountMultiSigner', '1'])
      .then((mock) => {
...
describe('AccountMultiSigner', () => {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('Multi ECDSA signers with threshold=1', () => {
    beforeEach(async function () {
  ...
  describe('Multi ECDSA signers with threshold=2', () => {
    beforeEach(async function () {
  ...
  describe('Mixed signers with threshold=2', () => {
    beforeEach(async function () {
      ...
      this.signer = new NonNativeSigner(
        new MultiERC7913SigningKey([signerECDSA1, signerP256, signerRSA])
      );
  ...
  describe('Signer management', () => {
    ...
    it('can add signers', async function () {
      ...
      const signersArrayBefore = await this.mock
        .getSigners(0, MAX_UINT64)
        .then((s) => s.map(ethers.getAddress));
      await expect(this.mock.$_addSigners(signers))
        .to.emit(this.mock, 'ERC7913SignerAdded')
        .withArgs(signerECDSA3.address);
      const signersArrayAfter = await this.mock
        .getSigners(0, MAX_UINT64)
        .then((s) => s.map(ethers.getAddress));
  ...
  describe('Signature validation', () => {
    ...
    it('rejects signatures from unauthorized signers', async function () {
      ...
      await expect(this.mock.$_rawSignatureValidation(TEST_MESSAGE, multiSignature)).to.eventually
        .be.false;
    });
    ...
    it('rejects invalid signatures from authorized signers', async function () {
      ...
      const invalidSignature = await signerECDSA2.signMessage(
