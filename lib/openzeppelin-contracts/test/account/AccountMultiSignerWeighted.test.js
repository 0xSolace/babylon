...
async function fixture() {
  ...
  const domain = {
    name: 'AccountMultiSignerWeighted',
    version: '1',
  ...
  const makeMock = (signers, weights, threshold) =>
    helper
      .newAccount('$AccountMultiSignerWeightedMock', [
        signers,
        weights,
        threshold,
...
describe('AccountMultiSignerWeighted', () => {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('Weighted signers with equal weights (1, 1, 1) and threshold=2', () => {
    beforeEach(async function () {
      ...
      this.mock = await this.makeMock(
        [signerECDSA1.address, signerECDSA2.address, signerECDSA3.address],
        [1, 1, 1],
        2
      );
    });
  ...
  describe('Weighted signers with varying weights (1, 2, 3) and threshold=3', () => {
    beforeEach(async function () {
  ...
  describe('Mixed weighted signers with threshold=4', () => {
    beforeEach(async function () {
  ...
  describe('Weight management', () => {
    ...
    it("no-op doesn't emit an event", async function () {
      await expect(this.mock.$_setSignerWeights([signer1], [1])).to.not.emit(
        this.mock,
    ...
    it('requires signers and weights arrays to have same length', async function () {
      ...
      await expect(
        this.mock.$_setSignerWeights([signer1, signer2], [1])
      ).to.be.revertedWithCustomError(this.mock, 'MultiSignerERC7913WeightedMismatchedLength');
    ...
    it('should revert if total weight to overflow (_setSignerWeights)', async function () {
      await expect(
        this.mock.$_setSignerWeights([signer1, signer2, signer3], [1n, 1n, MAX_UINT64 - 1n])
      )
