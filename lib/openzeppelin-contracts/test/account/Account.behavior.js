...
function shouldBehaveLikeAccountCore() {
  ...
  describe('validateUserOp', () => {
    ...
    it('should revert if the caller is not the canonical entrypoint', async function () {
      ...
      await expect(
        this.mock.connect(this.other).validateUserOp(operation.packed, operation.hash(), 0)
      )
        .to.be.revertedWithCustomError(this.mock, 'AccountUnauthorized')
        .withArgs(this.other);
    });

    describe('when the caller is the canonical entrypoint', () => {
      beforeEach(async function () {
      ...
      it('should return SIG_VALIDATION_SUCCESS if the signature is valid', async function () {
        ...
        expect(
          await this.mockFromEntrypoint.validateUserOp.staticCall(
            operation.packed,
            operation.hash(),
            0
          )
  ...
  describe('onReceived', () => {
    ...
    describe('onERC1155Received', () => {
      ...
      it('receives ERC1155 tokens from a single ID', async function () {
        await this.token
          .connect(this.other)
          .safeTransferFrom(this.other, this.mock, ids[0], values[0], data);
      ...
      it('receives ERC1155 tokens from a multiple IDs', async function () {
        ...
        await this.token
          .connect(this.other)
          .safeBatchTransferFrom(this.other, this.mock, ids, values, data);
        await expect(
          this.token.balanceOfBatch(
            ids.map(() => this.mock),
            ids
