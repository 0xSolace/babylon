...
describe('ERC7579Utils', () => {
  ...
  describe('execBatch', () => {
    ...
    it('default to calling self is target is address(0) (ERC-7821 calldata compression)', async function () {
      ...
      await expect(this.utils.$execBatch(data, EXEC_TYPE_DEFAULT))
        ...
        .withArgs([
          ethers.zeroPadBytes(CALL_TYPE_CALL, 32),
          ethers.zeroPadBytes(CALL_TYPE_BATCH, 32),
        ]);
    });
  ...
  describe('execDelegateCall', () => {
    it('delegate calls the target', async function () {
      ...
      await expect(ethers.provider.getStorage(this.utils.target, slot)).to.eventually.equal(
        ethers.ZeroHash
      );
  ...
  it('encodes single', async function () {
    ...
    await expect(this.utils.$encodeSingle(target, value, data)).to.eventually.equal(
  ...
  it('decodes batch', async function () {
    ...
    await expect(this.utils.$decodeBatch(encodeBatch(...entries))).to.eventually.deep.equal(
      entries
    );
  });
  ...
  it('encodes delegate', async function () {
    ...
    await expect(this.utils.$encodeDelegate(target, data)).to.eventually.equal(
  ...
  describe('global', () => {
    describe('eqCallTypeGlobal', () => {
      it('returns true if both call types are equal', async function () {
        await expect(this.utilsGlobal.$eqCallTypeGlobal(CALL_TYPE_BATCH, CALL_TYPE_BATCH)).to
          .eventually.be.true;
      });

      it('returns false if both call types are different', async function () {
        await expect(this.utilsGlobal.$eqCallTypeGlobal(CALL_TYPE_CALL, CALL_TYPE_BATCH)).to
          .eventually.be.false;
      });
    });
    ...
    describe('eqExecTypeGlobal', () => {
      it('returns true if both exec types are equal', async function () {
        await expect(this.utilsGlobal.$eqExecTypeGlobal(EXEC_TYPE_TRY, EXEC_TYPE_TRY)).to.eventually
          .be.true;
      });

      it('returns false if both exec types are different', async function () {
        await expect(this.utilsGlobal.$eqExecTypeGlobal(EXEC_TYPE_DEFAULT, EXEC_TYPE_TRY)).to
          .eventually.be.false;
      });
    });
    ...
    describe('eqModeSelectorGlobal', () => {
      it('returns true if both selectors are equal', async function () {
        await expect(this.utilsGlobal.$eqModeSelectorGlobal('0x12345678', '0x12345678')).to
          .eventually.be.true;
      });

      it('returns false if both selectors are different', async function () {
        await expect(this.utilsGlobal.$eqModeSelectorGlobal('0x12345678', '0x87654321')).to
          .eventually.be.false;
      });
    });
    ...
    describe('eqModePayloadGlobal', () => {
      it('returns true if both payloads are equal', async function () {
        await expect(
          this.utilsGlobal.$eqModePayloadGlobal(ethers.toBeHex(0, 22), ethers.toBeHex(0, 22))
        ).to.eventually.be.true;
      });

      it('returns false if both payloads are different', async function () {
        await expect(
          this.utilsGlobal.$eqModePayloadGlobal(ethers.toBeHex(0, 22), ethers.toBeHex(1, 22))
