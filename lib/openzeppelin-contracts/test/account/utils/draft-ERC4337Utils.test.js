...
describe('ERC4337Utils', () => {
  ...
  describe('getValidationData', () => {
    ...
    it('returns address(0) and false for validationData = 0', async function () {
      await expect(this.utils.$getValidationData(0n)).to.eventually.deep.equal([
        ethers.ZeroAddress,
        false,
      ]);
    });
  ...
  describe('userOp values', () => {
    describe('intiCode', () => {
      ...
      it('returns factory', async function () {
        ...
        await expect(this.utils.$factory(this.emptyUserOp.packed)).to.eventually.equal(
          ethers.ZeroAddress
        );
      });
    ...
    it('returns verificationGasLimit', async function () {
      ...
      await expect(this.utils.$verificationGasLimit(userOp.packed)).to.eventually.equal(
    ...
    it('returns maxPriorityFeePerGas', async function () {
      ...
      await expect(this.utils.$maxPriorityFeePerGas(userOp.packed)).to.eventually.equal(
    ...
    it('returns maxFeePerGas', async function () {
      ...
      await expect(this.utils.$maxFeePerGas(userOp.packed)).to.eventually.equal(
        userOp.maxFeePerGas
      );
    });
    ...
    describe('paymasterAndData', () => {
      ...
      it('returns paymaster', async function () {
        await expect(this.utils.$paymaster(this.userOp.packed)).to.eventually.equal(
          this.userOp.paymaster
        );
        await expect(this.utils.$paymaster(this.emptyUserOp.packed)).to.eventually.equal(
          ethers.ZeroAddress
        );
      });
      ...
      it('returns data', async function () {
        await expect(this.utils.$paymasterData(this.userOp.packed)).to.eventually.equal(
