...
function shouldBehaveLikeAccountERC7579({ withHooks = false } = {}) {
  describe('AccountERC7579', () => {
    beforeEach(async function () {
      await this.mock.deploy();
      await this.other.sendTransaction({
        to: this.mock.target,
        value: ethers.parseEther('1'),
      });

      this.modules = {};
      this.modules[MODULE_TYPE_VALIDATOR] = await ethers.deployContract('$ERC7579ModuleMock', [
        MODULE_TYPE_VALIDATOR,
      ]);
      this.modules[MODULE_TYPE_EXECUTOR] = await ethers.deployContract('$ERC7579ModuleMock', [
        MODULE_TYPE_EXECUTOR,
      ]);
      this.modules[MODULE_TYPE_FALLBACK] = await ethers.deployContract('$ERC7579ModuleMock', [
        MODULE_TYPE_FALLBACK,
      ]);
      this.modules[MODULE_TYPE_HOOK] = await ethers.deployContract('$ERC7579HookMock');

      this.mockFromEntrypoint = this.mock.connect(
        await impersonate(predeploy.entrypoint.v08.target)
      );
      this.mockFromExecutor = this.mock.connect(
        await impersonate(this.modules[MODULE_TYPE_EXECUTOR].target)
      );
    });
    ...
    describe('supportsExecutionMode', () => {
      for (const [callType, execType] of zip(
        ...
        it(`${
          ...
          await expect(
            this.mock.supportsExecutionMode(encodeMode({ callType, execType }))
    ...
    describe('supportsModule', () => {
      ...
      it(
        withHooks
          ? 'supports MODULE_TYPE_HOOK module type'
          : 'does not support MODULE_TYPE_HOOK module type',
        async function () {
    ...
    describe('module installation', () => {
      it('should revert if the caller is not the canonical entrypoint or the account itself', async function () {
        await expect(
          this.mock.connect(this.other).installModule(MODULE_TYPE_VALIDATOR, this.mock, '0x')
        )
      ...
      for (const moduleTypeId of [
        ...
        it(`should install a module of type ${moduleTypeId}`, async function () {
          ...
          await expect(
            this.mock.isModuleInstalled(moduleTypeId, instance, fullData)
          ).to.eventually.equal(false);
        ...
        it(`does not allow to install a module of ${moduleTypeId} id twice`, async function () {
          ...
          await expect(this.mockFromEntrypoint.installModule(moduleTypeId, instance, fullData))
            ...
            .withArgs(
              ...[moduleTypeId !== MODULE_TYPE_HOOK && moduleTypeId, instance].filter(Boolean)
            );
        });
      }
      ...
      withHooks &&
        describe('with hook', () => {
          beforeEach(async function () {
            await this.mockFromEntrypoint.$_installModule(
              MODULE_TYPE_HOOK,
              this.modules[MODULE_TYPE_HOOK],
              '0x'
            );
          });
          ...
          it('should call the hook of the installed module when performing an module install', async function () {
            ...
            await expect(
              this.mockFromEntrypoint.installModule(MODULE_TYPE_EXECUTOR, instance, initData)
            )
    ...
    describe('module uninstallation', () => {
      it('should revert if the caller is not the canonical entrypoint or the account itself', async function () {
        await expect(
          this.mock.connect(this.other).uninstallModule(MODULE_TYPE_VALIDATOR, this.mock, '0x')
        )
          .to.be.revertedWithCustomError(this.mock, 'AccountUnauthorized')
          .withArgs(this.other);
      });
      ...
      it('should revert uninstalling a module of type MODULE_TYPE_FALLBACK if a different module was installed for the provided selector', async function () {
        const instance = this.modules[MODULE_TYPE_FALLBACK];
        const anotherInstance = await ethers.deployContract('$ERC7579ModuleMock', [
          MODULE_TYPE_FALLBACK,
        ]);
        ...
        await expect(
          this.mockFromEntrypoint.uninstallModule(MODULE_TYPE_FALLBACK, anotherInstance, initData)
        )
          .to.be.revertedWithCustomError(this.mock, 'ERC7579UninstalledModule')
    ...
    describe('execution', () => {
      beforeEach(async function () {
        await this.mock.$_installModule(
          MODULE_TYPE_EXECUTOR,
          this.modules[MODULE_TYPE_EXECUTOR],
          '0x'
        );
      });
      ...
      for (const [execFn, mock] of [
        ...
        describe(`executing with ${execFn}`, () => {
          it('should revert if the call type is not supported', async function () {
            await expect(
              this[mock][execFn](
                encodeMode({ callType: CALL_TYPE_INVALID }),
                encodeSingle(this.other, 0, '0x')
              )
            )
          ...
          describe('single execution', () => {
            ...
            it('reverts when target reverts in default ExecType', async function () {
              ...
              await expect(
                this[mock][execFn](encodeMode({ callType: CALL_TYPE_CALL }), data)
          ...
          describe('batch execution', () => {
            it('calls the targets with value and args', async function () {
              ...
              const data = encodeBatch(
                ...
                [
                  this.anotherTarget,
                  value2,
                  this.anotherTarget.interface.encodeFunctionData('mockFunctionWithArgs', [
                    42,
                    '0x1234',
                  ]),
                ]
            ...
            it('reverts when any target reverts in default ExecType', async function () {
              ...
              await expect(
                this[mock][execFn](encodeMode({ callType: CALL_TYPE_BATCH }), data)
              ).to.be.revertedWith('CallReceiverMock: reverting');
            });
          ...
          describe('delegate call execution', () => {
            it('delegate calls the target', async function () {
              ...
              await expect(ethers.provider.getStorage(this.mock.target, slot)).to.eventually.equal(
                ethers.ZeroHash
              );
            ...
            it('reverts when target reverts in default ExecType', async function () {
              ...
              await expect(
                this[mock][execFn](encodeMode({ callType: CALL_TYPE_DELEGATE }), data)
              ).to.be.revertedWith('CallReceiverMock: reverting');
            });
          ...
          withHooks &&
            describe('with hook', () => {
              ...
              it(`should call the hook of the installed module when executing ${execFn}`, async function () {
                const caller =
                  execFn === 'execute'
                    ? predeploy.entrypoint.v08
                    : this.modules[MODULE_TYPE_EXECUTOR];
                const value = 17;
                const data = this.target.interface.encodeFunctionData('mockFunctionWithArgs', [
                  42,
                  '0x1234',
                ]);
    ...
    describe('fallback', () => {
      ...
      describe('with a fallback module installed', () => {
        beforeEach(async function () {
          await Promise.all(
            [
        ...
        it('forwards the call to the fallback handler', async function () {
          ...
          await expect(
            this.fallbackHandler.attach(this.mock).connect(this.other).callPayable({ value })
          )
        ...
        it('returns answer from the fallback handler', async function () {
          await expect(
            this.fallbackHandler.attach(this.mock).connect(this.other).callView()
          ).to.eventually.deep.equal([this.mock.target, this.other.address]);
        });
