  ...
  describe('supports ERC-7821', () => {
    ...
    if (deployable) {
      describe('when not deployed', () => {
        it('should be created with handleOps and increase nonce', async function () {
          ...
          await expect(predeploy.entrypoint.v08.getNonce(this.mock.target, 0)).to.eventually.equal(
            0
          );
        ...
        it('should revert if the signature is invalid', async function () {
          ...
          await expect(predeploy.entrypoint.v08.handleOps([operation.packed], this.beneficiary)).to
            .be.reverted;
        });
      });
    }
    ...
    describe('when deployed', () => {
      ...
      it('should support sending eth to an EOA', async function () {
        ...
        await expect(this.mock.getNonce()).to.eventually.equal(0);
        await expect(
          predeploy.entrypoint.v08.handleOps([operation.packed], this.beneficiary)
        ).to.changeEtherBalance(this.other, 42);
        await expect(this.mock.getNonce()).to.eventually.equal(1);
      });
