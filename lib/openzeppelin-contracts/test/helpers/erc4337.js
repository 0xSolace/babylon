...
class ERC4337Helper {
  ...
  async newAccount(name, extraArgs = [], params = {}) {
    ...
    if (params.erc7702signer) {
      ...
      const instance = await params.erc7702signer
        .getAddress()
        .then((address) => accountFactory.attach(address));
      const authorization = await params.erc7702signer.authorize({
        address: delegate.target,
      });
      ...
      const initCode = await accountFactory
        ...
        .then((tx) =>
          factory.interface.encodeFunctionData('$deploy', [
            0,
            params.salt ?? ethers.randomBytes(32),
            tx.data,
          ])
        )
