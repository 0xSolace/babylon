...
class GovernorHelper {
  ...
  get id() {
    return this.governor.latestProposalId
      ? this.governor.getProposalId(...this.shortProposal)
      : this.hash;
  }
  ...
  async overrideVote(vote = {}) {
    ...
    if (vote.signature) {
      const sign = await this.forgeMessage(vote).then((msg) =>
        vote.signature(this.governor, { reason: '', ...msg })
      );
