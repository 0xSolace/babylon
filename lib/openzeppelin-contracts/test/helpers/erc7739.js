...
class ERC7739Signer extends ethers.AbstractSigner {
  ...
  static from(signer, domain) {
    return new ERC7739Signer(signer, domain);
  }
  ...
  async signMessage(message) {
    return this.#signer.signTypedData(
      this.#domain,
      { PersonalSign },
      ERC4337Utils.preparePersonalSign(message)
    );
  }
  ...
  async signTypedData(domain, types, value) {
    ...
    return Promise.resolve(
      this.#signer.signTypedData(
        domain,
        allTypes,
        ERC4337Utils.prepareSignTypedData(value, this.#domain)
      )
    ).then((signature) =>
      ethers.concat([
        signature,
...
class ERC4337Utils {
  ...
  static getContentsDetail(contentsTypes, contentsTypeName = Object.keys(contentsTypes).at(0)) {
    ...
    const allTypes = {
      TypedDataSign: TypedDataSign(contentsTypeName),
      ...contentsTypes,
    };
    ...
    const contentsDescr =
      contentsType + (contentsType.startsWith(contentsTypeName) ? '' : contentsTypeName);
