...
const { generateKeyPairSync, privateEncrypt } = require('node:crypto');
...
class P256SigningKey {
  ...
  sign(digest /*: BytesLike*/) /*: ethers.Signature*/ {
    ethers.assertArgument(
      ethers.dataLength(digest) === 32,
      'invalid digest length',
      'digest',
      digest
    );

    const sig = secp256r1.sign(ethers.getBytesCopy(digest), ethers.getBytesCopy(this.#privateKey), {
      lowS: true,
    });
...
class RSASigningKey {
  ...
  constructor(keyPair) {
    ...
    this.#publicKey = {
      e: ethers.decodeBase64(jwk.e),
      n: ethers.decodeBase64(jwk.n),
    };
  }
  ...
  get publicKey() {
    return {
      e: ethers.hexlify(this.#publicKey.e),
      n: ethers.hexlify(this.#publicKey.n),
    };
  }
...
class WebAuthnSigningKey extends P256SigningKey {
  sign(digest /*: BytesLike*/) /*: { serialized: string } */ {
    ...
    const { r, s } = super.sign(
      ethers.sha256(
        ethers.concat([authenticatorData, ethers.sha256(ethers.toUtf8Bytes(clientDataJSON))])
      )
    );
...
class MultiERC7913SigningKey {
  ...
  constructor(signers) {
    ...
    this.#signers = signers.sort(
      (s1, s2) =>
        ethers.keccak256(s1.bytes ?? s1.address) - ethers.keccak256(s2.bytes ?? s2.address)
    );
  }
