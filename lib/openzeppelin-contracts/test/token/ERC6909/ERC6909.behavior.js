  ...
  describe('like an ERC6909', () => {
    describe('balanceOf', () => {
      ...
      describe('when accounts own some tokens', () => {
        ...
        it('returns amount owned by the given address', async function () {
          await expect(this.token.balanceOf(this.holder, firstTokenId)).to.eventually.be.equal(
            firstTokenSupply
          );
          await expect(this.token.balanceOf(this.holder, secondTokenId)).to.eventually.be.equal(
            secondTokenSupply
          );
    ...
    describe('approve', () => {
      it('emits an Approval event and updates allowance', async function () {
        await expect(
          this.token.connect(this.holder).approve(this.operator, firstTokenId, firstTokenSupply)
        )
        ...
        // approved
        await expect(
          this.token.allowance(this.holder, this.operator, firstTokenId)
        ).to.eventually.be.equal(firstTokenSupply);
        // other account is not approved
        await expect(
          this.token.allowance(this.other, this.operator, firstTokenId)
        ).to.eventually.be.equal(0n);
      });
      ...
      it('cannot give allowance to address(0)', async function () {
        await expect(
          this.token
            .connect(this.holder)
            .approve(ethers.ZeroAddress, firstTokenId, firstTokenSupply)
        )
          .to.be.revertedWithCustomError(this.token, 'ERC6909InvalidSpender')
          .withArgs(ethers.ZeroAddress);
      });
    ...
    describe('transfer', () => {
      ...
      it('transfers to the zero address are blocked', async function () {
        await expect(
          this.token
            .connect(this.holder)
            .transfer(ethers.ZeroAddress, firstTokenId, firstTokenSupply)
        )
      ...
      it('reverts when insufficient balance', async function () {
        await expect(
          this.token
            .connect(this.holder)
            .transfer(this.recipient, firstTokenId, firstTokenSupply + 1n)
        )
      ...
      it('emits event and transfers tokens', async function () {
        ...
        await expect(this.token.balanceOf(this.holder, firstTokenId)).to.eventually.equal(
          firstTokenSupply - amount
        );
        await expect(this.token.balanceOf(this.recipient, firstTokenId)).to.eventually.equal(
          amount
        );
      });
    });
    ...
    describe('transferFrom', () => {
      ...
      it('transfer from self', async function () {
        await expect(
          this.token
            .connect(this.holder)
            .transferFrom(this.holder, this.recipient, firstTokenId, amount)
        )
          .to.emit(this.token, 'Transfer')
          .withArgs(this.holder, this.holder, this.recipient, firstTokenId, amount);
      ...
      describe('with approval', async () => {
        ...
        it('reverts when insufficient allowance', async function () {
          await expect(
            this.token
              ...
              .transferFrom(this.holder, this.recipient, firstTokenId, amount + 1n)
          )
        ...
        it("shouldn't reduce allowance when infinite", async function () {
          await this.token
            .connect(this.holder)
            .approve(this.operator, firstTokenId, ethers.MaxUint256);
    ...
    describe('with operator approval', () => {
      beforeEach(async function () {
        await this.token.connect(this.holder).setOperator(this.operator, true);
        await this.token.$_mint(this.holder, firstTokenId, firstTokenSupply);
      });
      ...
      it('operator transfer does not reduce allowance', async function () {
        ...
        await this.token
          .connect(this.holder)
          .approve(this.operator, firstTokenId, firstTokenSupply);
        ...
        await expect(
          this.token.allowance(this.holder, this.operator, firstTokenId)
        ).to.eventually.equal(firstTokenSupply);
      });
    });
