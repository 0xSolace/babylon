...
describe('ERC6909Metadata', () => {
  ...
  describe('symbol', () => {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setSymbol(1n, 'MTK'))
        .to.emit(this.token, 'ERC6909SymbolUpdated')
        .withArgs(1n, 'MTK');
  ...
  describe('decimals', () => {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setDecimals(1n, 18))
        .to.emit(this.token, 'ERC6909DecimalsUpdated')
        .withArgs(1n, 18);
