        ...
        .map((source) => [
          source,
          {
            ...
            interface: Array.from(
              findAll('ContractDefinition', solcOutput.sources[source].ast)
            ).every(({ contractKind }) => contractKind === 'interface'),
          },
        ]);
    })
  );
    ...
    .forEach((file) => {
      ...
      metadata[file] = {
        pragma: fs
          ...
          .match(/pragma solidity (?<pragma>[<>=^]*[0-9]+\.[0-9]+\.[0-9]+);/)?.groups.pragma,
        sources: entries
          .flatMap((entry) => Object.keys(JSON.parse(fs.readFileSync(entry)).metadata.sources))
          .filter(
            (source) =>
              source !== file && match.all(source, pattern) && !match.any(source, skipPatterns)
          ),
