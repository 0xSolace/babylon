...
Promise.all(
  Object.entries(config).flatMap(([name, addr]) =>
    Promise.all([
      argv.etherscan &&
        request(
          `https://api.etherscan.io/api?module=contract&action=getabi&address=${addr}&apikey=${argv.etherscan}`
        )
          .then(({ body }) => body.json())
          .then(({ result: abi }) =>
            fs.writeFile(path.join(argv.output, `${name}.abi`), abi, 'utf-8', () => {})
          ),
      ethers
        ...
        .then((bytecode) =>
          fs.writeFile(
            path.join(argv.output, `${name}.bytecode`),
