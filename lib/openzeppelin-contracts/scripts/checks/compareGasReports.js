...
function formatCellShell(cell) {
  ...
  return [
    format((!Number.isFinite(cell?.value) ? '-' : cell.value.toString()).padStart(8)),
    format(
      (!Number.isFinite(cell?.delta) ? '-' : plusSign(cell.delta) + cell.delta.toString()).padStart(
        8
      )
    ),
    format(
...
function formatCellMarkdown(cell) {
  return [
    ...
    !Number.isFinite(cell?.delta) ? '-' : plusSign(cell.delta) + cell.delta.toString(),
    !Number.isFinite(cell?.prcnt)
      ? '-'
