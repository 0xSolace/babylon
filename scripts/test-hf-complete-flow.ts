...
async function generateMockData() {
  ...
  await fs.writeFile(path.join(outputDir, 'index.json'), JSON.stringify(metadata, null, 2));

  await fs.writeFile(path.join(outputDir, 'summary.json'), JSON.stringify(metadata, null, 2));
