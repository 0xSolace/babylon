    ...
    if (file.endsWith('.json') && file.startsWith('benchmark-')) {
      ...
      if (stats.size > 10000) {
        ...
        if (data.ticks && data.initialState) {
          console.log(
            `✅ ${file} - ${(stats.size / 1024).toFixed(0)} KB, ${data.ticks.length} ticks`
          );
...
async function main() {
  ...
  if (hasRealBenchmarks) {
    ...
    console.log(
      `   ${benchmarks.reduce((sum, b) => sum + b.ticks, 0).toLocaleString()} total ticks`
    );
    console.log(
      `   ${(benchmarks.reduce((sum, b) => sum + b.size, 0) / 1024 / 1024).toFixed(2)} MB`
    );
  }
