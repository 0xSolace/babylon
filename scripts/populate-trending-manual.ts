  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      if (matchingTags.length === 0) {
        matchingTags.push(
          sampleTags[Math.floor(Math.random() * sampleTags.length)].name
        );
      }

      // Create links
      for (const tagName of matchingTags.slice(0, 3)) {
        // Max 3 tags per post
    ...
    for (let i = 0; i < tagCounts.length; i++) {
      ...
      await prisma.trendingTag.create({
        data: {
          ...
          relatedContext:
            i < 3 && Math.random() > 0.5
              ? `Trending with ${sampleTags[Math.floor(Math.random() * sampleTags.length)].displayName}`
              : null,
        },
      });
    }
    ...
    topTrending.forEach((t) => {
      console.log(
        `  ${t.rank}. ${t.tag.displayName} (${t.postCount} posts, score: ${t.score.toFixed(2)})`
      );
    });
