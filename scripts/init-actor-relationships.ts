...
async function initializeActorRelationships() {
  ...
  if (actors.length === 0) {
    logger.error(
      'No actors found. Please run seed first.',
      undefined,
      'Script'
    );
    return;
  ...
  const _actorMap = new Map(actors.map((a) => [a.id, a]));

  // Helper function to calculate relationship strength
  function calculateStrength(
    actor1: ActorForRelationships,
    actor2: ActorForRelationships
  ): number {
    let strength = 0;
    ...
    const tier1Weight =
      TIER_WEIGHTS[actor1.tier as keyof typeof TIER_WEIGHTS] || 0;
    const tier2Weight =
      TIER_WEIGHTS[actor2.tier as keyof typeof TIER_WEIGHTS] || 0;
  ...
  for (const actor of actors) {
    ...
    const tierWeight =
      TIER_WEIGHTS[actor.tier as keyof typeof TIER_WEIGHTS] || 1;
    ...
    for (const { target, score } of toFollow) {
      ...
      try {
        ...
        if (error.code !== 'P2002') {
          console.error(
            `Error creating follow ${actor.name} -> ${target.name}:`,
            error.message
          );
        }
      }
  ...
  logger.info(
    `✅ Created ${followsCreated} actor follow relationships`,
    undefined,
    'Script'
  );
  ...
  const isolatedActors = await prisma.actor.findMany({
    where: {
      AND: [{ followedBy: { none: {} } }, { following: { none: {} } }],
    },
    select: {
      id: true,
      name: true,
  ...
  if (isolatedActors.length > 0) {
    logger.warn(
      `Found ${isolatedActors.length} isolated actors (no connections)`,
      undefined,
      'Script'
    );
    ...
    for (const isolated of isolatedActors) {
      ...
      for (const target of randomActors) {
        try {
          ...
          if (error.code !== 'P2002') {
            console.error(
              `Error fixing isolated actor ${isolated.name}:`,
              error.message
            );
          }
        }
    ...
    logger.info(
      `✅ Connected ${isolatedActors.length} isolated actors`,
      undefined,
      'Script'
    );
  }
