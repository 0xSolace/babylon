...
async function verifyFirstTick() {
  ...
  if (trendingTags.length > 0) {
    trendingTags.forEach((t, i) => {
      console.log(
        `  ${i + 1}. ${t.tag.displayName} (${t.postCount} posts, score: ${t.score.toFixed(2)})`
      );
    });
  } else {
  ...
  if (newsOrgs.length > 0) {
    ...
    console.log(
      `  ⚠️  No news organizations found - articles can't be generated`
    );
  }
  ...
  if (hasContent) {
    ...
    if (articles.length === 0)
      console.log('   - No articles (Latest News will be empty)');
    if (trendingTags.length === 0)
      console.log('   - No trending tags (Trending will be empty)');
    console.log('\nTo fix:');
