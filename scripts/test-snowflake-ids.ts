...
console.log(
  `  Performance: ${Math.round((1000 / duration) * 1000)} IDs/second\n`
);
...
console.log(
  `  Size savings: ${uuidExample.length - snowflakeId.length} characters`
);
...
console.log(
  `✓ Old format: http://localhost:3000/post/org-ambient-1762747396508-0.49887658185492456`
);
...
const concurrentIds = await Promise.all(
  Array(100)
    .fill(0)
    .map(() => Promise.resolve(generateSnowflakeId()))
);
