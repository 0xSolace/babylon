...
const postgresRunning =
  await $`docker ps --filter name=${POSTGRES_CONTAINER_NAME} --format "{{.Names}}"`
    .quiet()
    .text();
...
if (postgresRunning.trim() !== POSTGRES_CONTAINER_NAME) {
  ...
  while (attempts < 30) {
    try {
      const health =
        await $`docker inspect --format='{{.State.Health.Status}}' ${POSTGRES_CONTAINER_NAME}`
          .quiet()
          .text();
...
const redisRunning =
  await $`docker ps --filter name=${REDIS_CONTAINER_NAME} --format "{{.Names}}"`
    .quiet()
    .text();
...
if (redisRunning.trim() !== REDIS_CONTAINER_NAME) {
  ...
  try {
    ...
    while (attempts < 30) {
      try {
        const health =
          await $`docker inspect --format='{{.State.Health.Status}}' ${REDIS_CONTAINER_NAME}`
            .quiet()
            .text();
    ...
    if (attempts === 30) {
      logger.warn(
        'Redis health check timeout (continuing anyway)',
        undefined,
        'Script'
      );
      logger.info(
        'Redis is optional - SSE will use polling fallback',
        undefined,
        'Script'
      );
    }
    ...
    logger.warn(
      'Could not start Redis container (this is OK)',
      undefined,
      'Script'
    );
    logger.info(
      'Redis is optional - you can use existing Redis on port 6379',
      undefined,
      'Script'
    );
    logger.info(
      'Or the app will use polling fallback for real-time features',
      undefined,
      'Script'
    );
  }
...
const minioRunning =
  await $`docker ps --filter name=${MINIO_CONTAINER_NAME} --format "{{.Names}}"`
    .quiet()
    .text();
...
if (minioRunning.trim() !== MINIO_CONTAINER_NAME) {
  ...
  try {
    ...
    while (attempts < 30) {
      try {
        const health =
          await $`docker inspect --format='{{.State.Health.Status}}' ${MINIO_CONTAINER_NAME}`
            .quiet()
            .text();
    ...
    if (attempts === 30) {
      logger.warn(
        'MinIO health check timeout (continuing anyway)',
        undefined,
        'Script'
      );
      logger.info(
        'MinIO is optional - you can use Vercel Blob or other storage',
        undefined,
        'Script'
      );
    }
    ...
    logger.warn(
      'Could not start MinIO container (this is OK)',
      undefined,
      'Script'
    );
    logger.info(
...
try {
  ...
  if (
    ...
    logger.info(
      'Database tables not found, checking migration status...',
      undefined,
      'Script'
    );
    ...
    try {
      ...
      const migrationsTable = await prisma
      ...
      if (migrationsTable.length > 0) {
        ...
        if (failedMigration) {
          logger.info(
            `Found failed migration: ${failedMigration}, removing...`,
            undefined,
            'Script'
          );
      ...
      logger.info(
        'No migrations table or cleanup not needed',
        undefined,
        'Script'
      );
    }
    ...
    try {
      ...
      logger.info(
        'migrate deploy failed, syncing schema...',
        undefined,
        'Script'
      );
...
if (actorCount === 0) {
  ...
  logger.info(
    `Database has ${actorCount} actors and ${poolCount} pools`,
    undefined,
    'Script'
  );
  ...
  const needsPointsUpdate = traderActors.some(
    (a) => a.reputationPoints !== 10000
  );
  const needsImageUpdate = traderActors.some((a) => !a.profileImageUrl);

  if (needsPointsUpdate || needsImageUpdate) {
    logger.info(
      'Trader actors need initialization, updating...',
      undefined,
      'Script'
    );
    ...
    for (const actor of traderActors) {
      ...
      if (
        actor.reputationPoints !== 10000 ||
        (hasImage && !actor.profileImageUrl)
      ) {
...
if (trendingCount < MIN_TRENDING || newsCount < MIN_NEWS) {
  logger.info(
    `Trending (${trendingCount}/${MIN_TRENDING}) and news (${newsCount}/${MIN_NEWS}) not populated`,
    undefined,
    'Script'
  );
  logger.info(
    '💡 To initialize: bun run scripts/init-trending-and-news.ts',
    undefined,
    'Script'
  );
  logger.info(
  ...
  logger.info(
    `Trending (${trendingCount}) and news (${newsCount}) already populated`,
    undefined,
    'Script'
  );
}
...
if (untaggedPosts > 0 && totalPosts > 0) {
  ...
  logger.info(
    `Posts: ${postsWithTags}/${totalPosts} tagged (${taggedPercentage}%)`,
    undefined,
    'Script'
  );
  ...
  if (untaggedPosts > 10 && taggedPercentage < 80) {
    logger.info(
      '⚠️  Many posts are untagged. Trending may not work well.',
      undefined,
      'Script'
    );
    logger.info(
      '💡 To tag existing posts, run: bun run backfill:tags',
      undefined,
      'Script'
    );
    logger.info(
      '   New posts will be tagged automatically.',
      undefined,
      'Script'
    );
  }
