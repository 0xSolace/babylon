...
async function testBenchmarking() {
  ...
  try {
    ...
    addResult(
      ...
      modelsToTest.length > 0 ? `${modelsToTest.length} models available` : 'No models to benchmark'
    );

    // Test 3: Comparison logic (if models exist)
    if (modelsToTest.length > 0) {
      try {
        const candidateModelId = modelsToTest[0]?.modelId;
        if (candidateModelId) {
          const comparison = await benchmarkService.compareModels(candidateModelId);
          addResult(
            'Benchmarking',
            'Comparison Logic',
            true,
          ...
          addResult(
            ...
            'Comparison Logic',
            true,
            'Model missing identifier (skipped)'
          );
        }
      } catch (_error) {
...
async function testDatabase() {
  ...
  try {
    ...
    const trajectoryStats = {
      ...
      scored: await prisma.trajectory.count({
        where: { aiJudgeReward: { not: null } },
      }),
      training: await prisma.trajectory.count({
        where: { isTrainingData: true },
      }),
      unused: await prisma.trajectory.count({
    ...
    const modelStats = {
      ...
      deployed: await prisma.trainedModel.count({
        where: { status: 'deployed' },
      }),
      benchmarked: await prisma.trainedModel.count({
    ...
    const batchStats = {
      ...
      pending: await prisma.trainingBatch.count({
        where: { status: 'pending' },
      }),
      training: await prisma.trainingBatch.count({
        where: { status: 'training' },
      }),
      completed: await prisma.trainingBatch.count({
        where: { status: 'completed' },
      }),
