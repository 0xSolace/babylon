...
async function testFollowersFollowing() {
  ...
  try {
    ...
    console.log(
      `✅ Found test user: ${testUser.displayName || testUser.username} (${testUser.id})`
    );
    ...
    userFollowing.forEach((f) => {
      console.log(
        `   - ${f.following.displayName || f.following.username} (${f.following.id})`
      );
    });
    ...
    userFollowers.forEach((f) => {
      console.log(
        `   - ${f.follower.displayName || f.follower.username} (${f.follower.id})`
      );
    });
    ...
    actorFollowers.slice(0, 5).forEach((f) => {
      console.log(
        `   - ${f.follower.name} [${f.follower.tier}] (${f.follower.id})`
      );
    });
    ...
    userActorFollowers.forEach((f) => {
      console.log(
        `   - ${f.user.displayName || f.user.username} (${f.user.id})`
      );
    });
    ...
    actorFollowing.slice(0, 5).forEach((f) => {
      console.log(
        `   - ${f.following.name} [${f.following.tier}] (${f.following.id})`
      );
    });
    ...
    console.log(
      `✅ User-to-User Follows: Working (${userFollowing.length} following, ${userFollowers.length} followers)`
    );
    console.log(
      `✅ User-to-NPC Follows: Working (${actorFollows.length} following, ${userActorFollowers.length} total user followers of NPCs)`
    );
    console.log(
      `✅ NPC-to-NPC Follows: Working (${actorFollowing.length} following, ${actorFollowers.length} followers)`
    );
    console.log(
