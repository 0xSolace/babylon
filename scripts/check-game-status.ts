...
async function main() {
  ...
  if (actorCount === 0) {
    logger.error(
      '❌ No actors in database! Run: bun run db:seed',
      undefined,
      'Diagnostic'
    );
  }
  ...
  logger.info(
    `Questions: ${questionCount} total, ${activeQuestions} active`,
    undefined,
    'Diagnostic'
  );
  ...
  logger.info(
    `Posts: ${postCount} total, ${recentPosts} in last 5 minutes`,
    undefined,
    'Diagnostic'
  );
  ...
  if (recentPosts === 0 && postCount > 0) {
    logger.warn(
      '⚠️  No recent posts - daemon might not be generating content',
      undefined,
      'Diagnostic'
    );
  ...
  if (game) {
    ...
    if (!game.isRunning) {
      logger.warn(
        '⚠️  Game is paused! To start: UPDATE "Game" SET "isRunning" = true;',
        undefined,
        'Diagnostic'
      );
    }
  ...
  if (actorCount === 0) {
    logger.error(
      '❌ PROBLEM: No actors. Run: bun run db:seed',
      undefined,
      'Diagnostic'
    );
  } else if (recentPosts === 0 && postCount === 0) {
    logger.warn(
      '⚠️  No posts yet. Daemon starting or API key missing?',
      undefined,
      'Diagnostic'
    );
    logger.info(
      'Check GROQ_API_KEY or OPENAI_API_KEY in .env.local',
      undefined,
      'Diagnostic'
    );
  } else if (recentPosts === 0) {
    logger.warn(
      '⚠️  Posts exist but none recent. Is daemon running?',
      undefined,
      'Diagnostic'
    );
    logger.info(
      'Run: bun run daemon (in separate terminal)',
      undefined,
      'Diagnostic'
    );
  } else {
    logger.info(
      '✅ Everything looks good! Game is generating content.',
      undefined,
      'Diagnostic'
    );
  }
