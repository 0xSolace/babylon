...
async function debug() {
  ...
  for (const [key, impact] of impacts) {
    ...
    console.log(`${key}:`);
    console.log(
      `  Long: $${impact.longVolume.toFixed(0)} | Short: $${impact.shortVolume.toFixed(0)}`
    );
    console.log(
      `  Total: $${totalVol.toFixed(0)} | Sentiment: ${impact.netSentiment.toFixed(2)}`
    );
  ...
  if (allFound.length === impacts.size) {
    ...
    console.log(
      `❌ ${impacts.size - allFound.length}/${impacts.size} tickers have NO organization`
    );
