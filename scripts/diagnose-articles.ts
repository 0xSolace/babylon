...
async function diagnose() {
  ...
  if (postsWithTitles.length > 0) {
    ...
    postsWithTitles.forEach((p, i) => {
      console.log(
        `  ${i + 1}. [${p.type}] "${p.articleTitle}" by ${p.authorId}`
      );
    });
  ...
  if (gameState) {
    console.log(
      `  Last tick: ${gameState.lastTickAt?.toISOString() || 'Never'}`
    );
  ...
  if (latestTrending) {
    const ageMinutes = Math.floor(
      (Date.now() - latestTrending.calculatedAt.getTime()) / 60000
    );
    console.log(`  Last calculated: ${ageMinutes} minutes ago`);
    console.log(
      `  Top tag: "${latestTrending.tag.displayName}" (${latestTrending.postCount} posts)`
    );
  } else {
    console.log(
      '  ⚠️  Never calculated - will be forced on next tick with articles'
    );
  }
