...
async function main() {
  ...
  for (const traj of unscored) {
    try {
      ...
      if (score && score.overallScore !== null) {
        console.log(
          `✅ ${traj.trajectoryId.substring(0, 12)}... scored: ${score.overallScore.toFixed(3)}`
        );
        scored++;
      } else {
      ...
      console.log(
        `❌ ${traj.trajectoryId.substring(0, 12)}... error: ${error instanceof Error ? error.message : 'Failed'}`
      );
      failed++;
    }
  }
  ...
  console.log(
    `${readyForTraining >= 100 ? '✅ READY' : '⏳ NOT READY'} (need 100+ for training)\n`
  );
