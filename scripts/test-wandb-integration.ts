...
async function testWandBConfiguration() {
  ...
  addResult('WANDB_PROJECT', true, `Using project: ${project}`);
  ...
  const usesServerless = trainerCode.includes(
    'from art.serverless.backend import ServerlessBackend'
  );
  const createsServerless = trainerCode.includes('ServerlessBackend(api_key=wandb_key)');
  const preventsLocal =
    trainerCode.includes('raise ValueError') && trainerCode.includes('WANDB_API_KEY is required');
  ...
  addResult(
    ...
    createsServerless,
    createsServerless ? 'Creates ServerlessBackend(api_key=...) ✅' : 'Backend creation not found!',
    true
  );
...
async function testWandBInference() {
  ...
  if (trainedModels.length > 0) {
    ...
    addResult(
      ...
      hasEntity,
      hasEntity ? 'Format: entity/project/model:step ✅' : 'Missing entity/project structure'
    );
  }
...
async function testGitHubWorkflowConfiguration() {
  ...
  addResult(
    ...
    passesWandBKey,
    passesWandBKey ? 'Passes from secrets to Python ✅' : 'WANDB_API_KEY not passed!',
    true
  );
  ...
  addResult(
    ...
    passesProject,
    passesProject ? 'Configured in workflow ✅' : 'WANDB_PROJECT not set'
  );
}
...
async function printWandBInstructions() {
  ...
  console.log(
    '   Via Dashboard: http://localhost:3000/admin/rl-training → Actions → Force Training'
  );
