    ...
    try {
      ...
      try {
        ...
        if ('kind' in sendResponse && sendResponse.kind === 'task') {
          const task = sendResponse;
          const taskId =
            task && typeof task === 'object' && 'id' in task && task.id != null
              ? String(task.id)
              : undefined;

          if (!taskId) {
            addResult('A2A', 'tasks/get method', 'warning', 'Task ID missing', true);
            return;
          }
          try {
            const taskStatus = await client.getTask({ id: taskId });
            const taskState =
              taskStatus &&
              typeof taskStatus === 'object' &&
              'status' in taskStatus &&
              taskStatus.status
                ? ((taskStatus.status as { state?: string }).state ?? 'unknown')
                : 'unknown';
            addResult(
              'A2A',
              'tasks/get method',
