...
async function migratePointsSystem() {
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        const newReputationPoints =
          basePoints + invitePoints + earnedPoints + bonusPoints;
        ...
        console.log(
          `   P&L: $${lifetimePnL.toFixed(2)} → Earned: ${earnedPoints} points`
        );
        console.log(
          `   Referrals: ${user.referralCount} → Invite: ${invitePoints} points`
        );
        console.log(`   Bonus: ${bonusPoints} points`);
        console.log(
          `   Old Total: ${user.reputationPoints} → New Total: ${newReputationPoints}`
        );
        console.log();
      } catch (error) {
        errorCount++;
    ...
    topUsers.forEach((user, index) => {
      ...
      console.log(
        `   All: ${user.reputationPoints} | Invite: ${user.invitePoints} | Earned: ${user.earnedPoints} | Bonus: ${user.bonusPoints}`
      );
    });
