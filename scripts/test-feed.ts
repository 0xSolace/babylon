...
async function main() {
  ...
  if (totalPosts === 0) {
    ...
    logger.info(
      '1. Daemon not generating content (check [GAME] logs)',
      undefined,
      'Test'
    );
    logger.info(
      '2. LLM API key missing (check GROQ_API_KEY or OPENAI_API_KEY)',
      undefined,
      'Test'
    );
    logger.info(
      '3. Game is paused (check Game.isRunning in database)',
      undefined,
      'Test'
    );
    logger.info('', undefined, 'Test');
    ...
    logger.info(
      '• Trigger manual tick: curl http://localhost:3000/api/cron/game-tick',
      undefined,
      'Test'
    );
  ...
  recentPosts.forEach((post, i) => {
    ...
    logger.info(
      `${i + 1}. [${age}m ago] ${post.authorId}: ${post.content.substring(0, 80)}...`,
      undefined,
      'Test'
    );
  });
  ...
  try {
    ...
    logger.info(
      `✅ API returned ${data.posts.length} posts`,
      undefined,
      'Test'
    );
    ...
    if (data.posts.length === 0 && totalPosts > 0) {
      logger.error(
        '❌ DATABASE HAS POSTS BUT API RETURNS NONE!',
        undefined,
        'Test'
      );
    ...
    if ((error as { code?: string }).code === 'ECONNREFUSED') {
      ...
      logger.error(
        'Is the web server running? Start with: bun run dev',
        undefined,
        'Test'
      );
    } else {
  ...
  if (actorCount === 0) {
    logger.warn(
      '⚠️  No actors! Posts might not have author names',
      undefined,
      'Test'
    );
  ...
  logger.info(
    `Actors loaded: ${actorCount > 0 ? 'YES' : 'NO'}`,
    undefined,
    'Test'
  );
  logger.info('', undefined, 'Test');
  ...
  if (totalPosts > 0 && data.posts.length > 0) {
    logger.info(
      '🎉 Feed should be working! If you still see empty feed:',
      undefined,
      'Test'
    );
