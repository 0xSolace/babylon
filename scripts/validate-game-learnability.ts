...
function calculateGroupAccuracy(game: GeneratedGame, npcs: Actor[]): number {
  ...
  for (const npc of npcs) {
    const posts = game.timeline
      ...
      .filter((p) => p.author === npc.id && p.pointsToward !== null && p.relatedQuestion !== null);
...
async function main() {
  ...
  for (const game of games) {
    for (const question of game.setup.questions) {
      ...
      const pass = gradient > 0.15 && earlyCertainty < 0.65 && lateCertainty > 0.7;
  ...
  if (!referenceGame) {
    logger.warn('No games available for reliability analysis', undefined, 'Validator');
    return;
  }

  const highAccuracy =
    reliabilityBuckets.high.length > 0
      ? calculateGroupAccuracy(referenceGame, reliabilityBuckets.high)
      : 0;
  const mediumAccuracy =
    reliabilityBuckets.medium.length > 0
      ? calculateGroupAccuracy(referenceGame, reliabilityBuckets.medium)
      : 0;
  const lowAccuracy =
    reliabilityBuckets.low.length > 0
      ? calculateGroupAccuracy(referenceGame, reliabilityBuckets.low)
      : 0;
