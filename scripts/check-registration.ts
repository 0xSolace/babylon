...
async function main() {
  ...
  try {
    ...
    if (
      ...
      console.log(
        '🔗 Checking On-Chain Registration (Agent0 Official Registry):'
      );
      ...
      try {
        ...
        if (
          ...
          console.log(
            `   Expected: ${process.env.BABYLON_GAME_WALLET_ADDRESS}`
          );
        }
        console.log();
    ...
    if (process.env.BABYLON_REGISTRY_REGISTERED === 'true') {
      ...
      console.log(
        '   You can also manually check with: cast call 0x8004a6090Cd10A7288092483047B097295Fb8847 "ownerOf(uint256)(address)" <TOKEN_ID> --rpc-url https://ethereum-sepolia-rpc.publicnode.com'
      );
      console.log();
