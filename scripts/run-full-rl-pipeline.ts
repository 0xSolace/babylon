...
import { type SimulationConfig, SimulationEngine } from '@/lib/benchmark/SimulationEngine';
...
class PipelineRunner {
  ...
  async stepBenchmarkTrainedModel(): Promise<void> {
    ...
    if (!agentId) {
      throw new Error('No test agent ID available for benchmarking');
    }
    const runtime = await agentRuntimeManager.getRuntime(agentId);
  ...
  async compareAgainstBaselines(trainedResult: {
    ...
    for (const model of this.config.baselines.models) {
      ...
      if (
        ...
        const accuracyDiff =
          trainedResult.metrics.predictionMetrics.accuracy - baseline.metrics.predictionAccuracy;
        const optimalityDiff =
          trainedResult.metrics.optimalityScore - baseline.metrics.optimalityScore;
