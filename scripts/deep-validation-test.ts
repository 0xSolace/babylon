...
async function main() {
  ...
  const perpPnl =
    result.metrics.perpMetrics.avgPnlPerTrade * result.metrics.perpMetrics.totalTrades;
  ...
  if (predictionActions.length > 0) {
    ...
    if (!firstAction) {
      issues.push('❌ Unable to inspect first prediction action.');
    } else {
      ...
      if (!firstAction.correctness) {
        ...
        const expectedCorrect =
          (outcome === 'YES' && groundTruth) || (outcome === 'NO' && !groundTruth);
