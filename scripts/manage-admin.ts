...
async function grantAdmin(identifier: string) {
  ...
  if (user.isAdmin) {
    console.log(
      `ℹ️  User ${user.username || user.walletAddress || user.id} is already an admin`
    );
    process.exit(0);
  }
  ...
  console.log(
    `✅ Admin privileges granted to ${user.username || user.walletAddress || user.id}`
  );
  ...
  try {
    switch (command) {
      case 'grant':
        if (!identifier) {
          console.error(
            '❌ Please provide a wallet address, username, or user ID'
          );
          console.log(
