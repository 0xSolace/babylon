...
async function initPools() {
  try {
    ...
    const actorsData: ActorsData = JSON.parse(
      fs.readFileSync(actorsPath, 'utf-8')
    );
    ...
    for (const actor of poolActors) {
      ...
      const capitalAllocation =
        CapitalAllocationService.calculateCapital(actor);
      console.log(
        `  💰 Capital allocation: $${capitalAllocation.tradingBalance.toLocaleString()}`
      );
      ...
      if (!dbActor) {
        ...
        dbActor = await prisma.actor.create({
          data: {
            ...
            hasPool: true,
            tradingBalance: new Prisma.Decimal(
              capitalAllocation.tradingBalance
            ),
      ...
      if (existingPool) {
        ...
        if (
          existingPool.totalValue.toNumber() === 0 &&
          existingPool.availableBalance.toNumber() === 0
        ) {
          console.log(
            `  🔄 Updating pool with initial capital: ${existingPool.name}`
          );
          await prisma.pool.update({
            where: { id: existingPool.id },
            data: {
              totalValue: new Prisma.Decimal(
                capitalAllocation.initialPoolBalance
              ),
          ...
          console.log(
            `  ✅ Pool updated with $${capitalAllocation.initialPoolBalance.toLocaleString()}`
          );
        } else {
          console.log(
            `  ✅ Pool already exists: ${existingPool.name} ($${existingPool.totalValue.toNumber().toLocaleString()})`
          );
        }
        continue;
      }
...
function getStrategy(personality: string): string {
  const strategies: Record<string, string> = {
    'activist investor':
      'Focuses on event-driven trades and corporate activism',
    ...
    'evangelical investor':
      'Faith-based innovation investing with 5-year horizons',
  ...
  return (
    strategies[personality.toLowerCase()] || 'Strategic trading across markets'
  );
}
