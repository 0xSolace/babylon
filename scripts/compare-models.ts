...
async function main() {
  ...
  for (const model of MODELS) {
    const modelResult = await runModelBenchmark(agent.id, benchmarkPath, model, outputDir);
  ...
  const [llama, qwen] = results;
  if (!llama || !qwen) {
    console.error('❌ Missing benchmark results for one or more models.');
    process.exit(1);
  }

  console.log(
    `│ P&L                 │ $${llama.result.metrics.totalPnl.toFixed(0).padStart(7)} │ $${qwen.result.metrics.totalPnl.toFixed(0).padStart(7)} │`
  );
  console.log(
    `│ Accuracy            │ ${(llama.result.metrics.predictionMetrics.accuracy * 100).toFixed(1).padStart(6)}% │ ${(qwen.result.metrics.predictionMetrics.accuracy * 100).toFixed(1).padStart(6)}% │`
  );
  console.log(
    `│ Correct/Total       │ ${llama.result.metrics.predictionMetrics.correctPredictions.toString().padStart(2)}/${llama.result.metrics.predictionMetrics.totalPositions.toString().padEnd(2)}    │ ${qwen.result.metrics.predictionMetrics.correctPredictions.toString().padStart(2)}/${qwen.result.metrics.predictionMetrics.totalPositions.toString().padEnd(2)}    │`
  );
  console.log(
    `│ Optimality Score    │ ${llama.result.metrics.optimalityScore.toFixed(1).padStart(6)}% │ ${qwen.result.metrics.optimalityScore.toFixed(1).padStart(6)}% │`
  );
  console.log(
    `│ Perp Trades         │ ${llama.result.metrics.perpMetrics.totalTrades.toString().padStart(8)} │ ${qwen.result.metrics.perpMetrics.totalTrades.toString().padStart(8)} │`
  );
  console.log(
    `│ Duration            │ ${llama.duration.toFixed(1).padStart(7)}s │ ${qwen.duration.toFixed(1).padStart(7)}s │`
  );
  console.log('└─────────────────────┴──────────┴──────────┘\n');

  // Determine winner
  const winner =
    llama.result.metrics.totalPnl > qwen.result.metrics.totalPnl ? 'LLaMA 8B' : 'Qwen 32B';
  const pnlDelta = Math.abs(llama.result.metrics.totalPnl - qwen.result.metrics.totalPnl);
  const accuracyDelta = Math.abs(
    llama.result.metrics.predictionMetrics.accuracy - qwen.result.metrics.predictionMetrics.accuracy
  );
