...
async function main() {
  ...
  try {
    ...
    const perpChanges = perpPrices.some((p, i) => {
      if (i === 0) {
        return false;
      }
      const previous = perpPrices[i - 1];
      return typeof previous === 'number' && Math.abs(p - previous) > 0.01;
    });
  ...
  try {
    ...
    if (!market) {
      addResult('Actions Recorded', false, 'No prediction market available in snapshot');
    } else {
  ...
  try {
    ...
    const market = snapshot.initialState.predictionMarkets[0];
    if (!market) {
      addResult('Ground Truth Validation', false, 'No prediction market available in snapshot');
    } else {
  ...
  try {
    ...
    if (!agent) {
      addResult(
        ...
        'Run: bun run scripts/benchmark.ts setup'
      );
    } else {
      ...
      while (!engine.isComplete()) {
        ...
        try {
          ...
          console.log(
            `   ⚠️  Error at tick ${tickNum}: ${error instanceof Error ? error.message : String(error)}`
          );
        }
  ...
  try {
    ...
    if (!market1 || !market2) {
      addResult('P&L Calculation', false, 'Not enough prediction markets available in snapshot');
    } else {
  ...
  try {
    ...
    try {
      ...
      errorCaught = true;
      console.log(
        `   ✅ Invalid trade properly rejected: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  ...
  try {
    ...
    const consistent =
      run1.pnl === run2.pnl && run1.accuracy === run2.accuracy && run1.actions === run2.actions;

    console.log(
      `   Run 1: ${run1.actions} actions, $${run1.pnl.toFixed(2)}, ${(run1.accuracy * 100).toFixed(1)}%`
    );
    console.log(
      `   Run 2: ${run2.actions} actions, $${run2.pnl.toFixed(2)}, ${(run2.accuracy * 100).toFixed(1)}%`
    );
