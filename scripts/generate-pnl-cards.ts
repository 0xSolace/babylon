...
async function generateExampleCards() {
  ...
  exampleUsers.slice(0, 2).forEach((user, idx) => {
    ...
    examples.push({
      ...
      description:
        data.totalPnL >= 0
          ? 'Profitable portfolio example'
          : 'Loss portfolio example',
    });
  });
  ...
  exampleUsers.slice(0, 2).forEach((user, idx) => {
    ...
    examples.push({
      ...
      description:
        data.unrealizedPnL >= 0
          ? 'Profitable predictions example'
          : 'Loss predictions example',
    });
  });
