...
class ContractDeployer {
  ...
  async deploy(): Promise<DeploymentResult> {
    logger.info(
      `Starting contract deployment to Ethereum ${this.config.network}`,
      undefined,
      'Script'
    );
    ...
    const predictionMarketFacet = await this.deployContract(
  ...
  private async deployContract(
    ...
    try {
      ...
      const contractPaths: Record<string, string> = {
        ERC8004IdentityRegistry:
          'contracts/identity/ERC8004IdentityRegistry.sol',
        ERC8004ReputationSystem:
          'contracts/identity/ERC8004ReputationSystem.sol',
  ...
  private async verifyContracts(
    ...
    for (const [name, address] of Object.entries(contracts)) {
      try {
        const contractPath =
          contractPaths[name] || `contracts/**/*.sol:${name}`;
        ...
        logger.warn(
          `${name} verification failed (may already be verified)`,
          undefined,
          'Script'
        );
      }
...
async function main() {
  ...
  if (!config.privateKey) {
    logger.error(
      'Error: DEPLOYER_PRIVATE_KEY environment variable required',
      undefined,
      'Script'
    );
    logger.info(
      ...
      {
        example1:
          'DEPLOYER_PRIVATE_KEY=0x... bun run scripts/deploy-contracts.ts',
        example2:
          'NETWORK=sepolia DEPLOYER_PRIVATE_KEY=0x... bun run scripts/deploy-contracts.ts',
      },
      'Script'
    );
    process.exit(1);
