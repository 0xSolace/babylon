...
async function backfillAll() {
  ...
  try {
    ...
    console.log(
      `   Already tagged: ${postsWithTags} (${Math.round((postsWithTags / totalPosts) * 100)}%)`
    );
    console.log(
      `   Need tagging: ${postsWithoutTags} (${Math.round((postsWithoutTags / totalPosts) * 100)}%)\n`
    );
    ...
    if (postsWithoutTags > 100) {
      ...
      console.log(
        `   Estimated time: ~${Math.ceil(((postsWithoutTags / BATCH_SIZE) * DELAY_BETWEEN_BATCHES) / 1000 / 60)} minutes`
      );
    ...
    while (true) {
      ...
      console.log(
        `📦 Batch ${Math.floor(offset / BATCH_SIZE) + 1}: Processing ${batch.length} posts...`
      );
        ...
        for (const [postId, tags] of tagMap.entries()) {
          try {
            if (tags.length > 0) {
              ...
              console.log(
                `   ✓ ${postId.slice(0, 8)}: ${tags.map((t) => t.displayName).join(', ')}`
              );
            } else {
      ...
      console.log(
        `   Progress: ${offset}/${postsWithoutTags} (${progress}%)\n`
      );
