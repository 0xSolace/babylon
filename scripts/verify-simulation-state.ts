...
async function main() {
  ...
  for (let i = 0; i < Math.min(6, snapshot.ticks.length); i++) {
    // Get market data
    const predictions = (await a2a.sendRequest('a2a.getPredictions', {})) as {
      predictions: Array<{ id: string; yesPrice: number }>;
    };
    const perpetuals = (await a2a.sendRequest('a2a.getPerpetuals', {})) as {
      perpetuals: Array<{
        ticker: string;
        price: number;
    ...
    console.log(`Tick ${i}:`);
    console.log(
      `  Prediction market-0 YES: ${((predictions.predictions[0]?.yesPrice ?? 0) * 100).toFixed(2)}%`
    );
    console.log(
      `  Perp ${perpetuals.perpetuals[0]?.ticker ?? 'N/A'}: $${(perpetuals.perpetuals[0]?.price ?? 0).toFixed(2)} (${(perpetuals.perpetuals[0]?.priceChange24h ?? 0).toFixed(2)}% change)`
    );
    console.log('');
  ...
  if (perpPrices.length > 0) {
    console.log(
      `  Range: $${Math.min(...perpPrices).toFixed(2)} - $${Math.max(...perpPrices).toFixed(2)}`
    );
  }
  console.log('');
  ...
  if (predPrices.length > 0) {
    console.log(
      `  Range: ${(Math.min(...predPrices) * 100).toFixed(2)}% - ${(Math.max(...predPrices) * 100).toFixed(2)}%`
    );
  }
  console.log('');
