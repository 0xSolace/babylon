...
async function main() {
  ...
  const seed = parseInt(args.find((a) => a.startsWith('--seed'))?.split('=')[1] || '', 10) || 12345;
  const force = args.includes('--force');

  const config: BenchmarkConfig = {
    durationMinutes,
    tickInterval: parseInt(args.find((a) => a.startsWith('--interval'))?.split('=')[1] || '60', 10), // 60 seconds
    numPredictionMarkets: parseInt(
      args.find((a) => a.startsWith('--markets'))?.split('=')[1] || '5',
      10
    ),
    numPerpetualMarkets: parseInt(
      args.find((a) => a.startsWith('--perpetuals'))?.split('=')[1] || '3',
      10
    ),
    numAgents: parseInt(args.find((a) => a.startsWith('--agents'))?.split('=')[1] || '8', 10),
    seed,
  };
