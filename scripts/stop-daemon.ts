...
function getDaemonPids(): number[] {
  try {
    ...
    const result = execSync('pgrep -f "tsx.*src/cli/realtime-daemon"', {
      encoding: 'utf-8',
    }).trim();
    if (result) {
      const pids = result
...
async function main() {
  ...
  logger.info(
    `Found ${pids.length} daemon process(es): ${pids.join(', ')}`,
    undefined,
    'stop-daemon'
  );
  ...
  try {
    ...
    logger.info(
      `Sending ${signal} to daemon processes...`,
      undefined,
      'stop-daemon'
    );
  ...
  if (remainingCount > 0) {
    logger.warn(
      `${remainingCount} process(es) still running`,
      undefined,
      'stop-daemon'
    );
    if (!force) {
      logger.warn(
        'Try running with --force flag: bun run daemon:stop --force',
        undefined,
        'stop-daemon'
      );
    } else {
      logger.error(
        'Failed to stop processes even with --force',
        undefined,
        'stop-daemon'
      );
      logger.info(
        'You may need to manually kill them with: pkill -9 -f "realtime-daemon"',
        undefined,
        'stop-daemon'
      );
    }
