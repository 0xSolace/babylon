      ...
      if (stats.size > 10000) {
        ...
        try {
          ...
          if (data.ticks && data.initialState) {
            ...
            console.log(
              `   Markets: ${data.initialState.predictionMarkets?.length || 0} prediction, ${data.initialState.perpetualMarkets?.length || 0} perpetual`
            );
            console.log('');
          }
        } catch (error) {
          console.log(
            `⚠️  Skipping ${file}: ${error instanceof Error ? error.message : 'Invalid format'}`
          );
        }
      }
    }
...
async function main() {
  ...
  console.log(
    'Avg Ticks per Benchmark:',
    Math.round(totalTicks / benchmarks.length).toLocaleString()
  );
  console.log('');
