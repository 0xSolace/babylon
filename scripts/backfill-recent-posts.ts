...
async function backfillRecent() {
  ...
  try {
    const cutoffDate = new Date(
      Date.now() - DAYS_TO_BACKFILL * 24 * 60 * 60 * 1000
    );
    ...
    console.log(
      `   Est. time: ~${Math.ceil(((recentPosts.length / BATCH_SIZE) * 2) / 60)} minutes\n`
    );
    ...
    for (let i = 0; i < recentPosts.length; i += BATCH_SIZE) {
      ...
      const progress = Math.round(
        ((i + batch.length) / recentPosts.length) * 100
      );
      console.log(
        `   Progress: ${i + batch.length}/${recentPosts.length} (${progress}%)\n`
      );
