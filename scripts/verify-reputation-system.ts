...
async function verifyReputationCalculation() {
  ...
  try {
    ...
    recordResult('Reputation Calculation', score >= 0 && score <= 100, `Score: ${score}/100`, {
      score,
    });
...
async function verifyOnChainSubmission() {
  ...
  if (hasConfig) {
    ...
    if (agent) {
      ...
      recordResult('On-Chain Submission', false, 'No agent with wallet address found for testing');
    }
  }
}
...
async function verifyAdminAPI() {
  ...
  recordResult('Admin API Data Structure', agents.length > 0, `Found ${agents.length} agents`, {
...
async function verifyCronEndpoint() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      recordResult('Cron Endpoint', false, `Cron endpoint returned ${response.status}`, {
        status: response.status,
        statusText: response.statusText,
      });
    }
  } catch (error) {
    recordResult('Cron Endpoint', false, 'Server not available or endpoint not accessible', {
