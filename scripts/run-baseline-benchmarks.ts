...
async function runBaselineBenchmark(
  ...
  const baselineResult = {
    ...
    metrics: {
      ...
      totalTrades:
        result.metrics.predictionMetrics.totalPositions + result.metrics.perpMetrics.totalTrades,
...
async function main() {
  ...
  for (const model of BASELINE_MODELS) {
    await runBaselineBenchmark({ benchmarkPath, agentUserId, outputDir, forceRegenerate }, model);
