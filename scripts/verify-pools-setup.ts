...
async function verifyAndFixPools() {
  ...
  poolActors.forEach((a) =>
    console.log(`   - ${a.name} (${a.id}) - Tier: ${a.tier}`)
  );
  ...
  console.log(
    `   Found ${dbActors.length}/${poolActors.length} actors in database`
  );
  ...
  for (const pool of pools) {
    ...
    const status =
      totalValue > 0 && availableBalance > 0 ? '✅ OK' : '❌ NEEDS FIX';
  ...
  const totalCapital = pools.reduce(
    (sum, p) => sum + p.totalValue.toNumber(),
    0
  );
