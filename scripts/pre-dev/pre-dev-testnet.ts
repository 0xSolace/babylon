...
// Start PostgreSQL
const postgresRunning = await $`docker ps --filter name=babylon-postgres --format "{{.Names}}"`
  .quiet()
  .text();
...
const redisRunning = await $`docker ps --filter name=babylon-redis --format "{{.Names}}"`
  .quiet()
  .text();
