...
import { $ } from 'bun';
import { printValidationResult, validateEnvironment } from '../../src/lib/deployment/env-detection';
...
// 3. Start Anvil
const anvilRunning = await $`docker ps --filter name=${ANVIL_CONTAINER} --format "{{.Names}}"`
  .quiet()
  .text();
...
if (anvilRunning.trim() !== ANVIL_CONTAINER) {
  ...
  let attempts = 0;
  while (attempts < 30) {
    const health = await $`docker inspect --format='{{.State.Health.Status}}' ${ANVIL_CONTAINER}`
      .quiet()
      .text()
      .catch(() => '');
...
while (rpcAttempts < 30) {
  const rpcSuccess = await $`cast block-number --rpc-url http://localhost:8545`
    .text()
    .then(() => true)
    .catch((error: Error & { stderr?: Buffer }) => {
      // Not ready yet
      if (rpcAttempts === 0) {
        // Log the error only on the first attempt
...
if (!deployment || !deployment.contracts.diamond) {
  ...
  const code = await $`cast code ${deployment.contracts.diamond} --rpc-url http://localhost:8545`
    .quiet()
    .text()
    .catch(() => '0x');
...
if (needsDeployment) {
  ...
  if (updatedDeployment?.contracts.babylonOracle) {
    ...
    logger.info(
      `   BabylonOracle: ${updatedDeployment.contracts.babylonOracle}`,
      undefined,
      'Script'
    );
    logger.info(
      `   Predimarket:   ${updatedDeployment.contracts.predimarket}`,
      undefined,
      'Script'
    );
...
// 6. Start PostgreSQL
const postgresRunning = await $`docker ps --filter name=${POSTGRES_CONTAINER} --format "{{.Names}}"`
  .quiet()
  .text();
...
if (postgresRunning.trim() !== POSTGRES_CONTAINER) {
  ...
  let attempts = 0;
  while (attempts < 30) {
    const health = await $`docker inspect --format='{{.State.Health.Status}}' ${POSTGRES_CONTAINER}`
      .quiet()
      .text()
      .catch(() => '');
    if (health.trim() === 'healthy') {
...
const redisRunning = await $`docker ps --filter name=${REDIS_CONTAINER} --format "{{.Names}}"`
  .quiet()
  .text();
...
const minioRunning = await $`docker ps --filter name=${MINIO_CONTAINER} --format "{{.Names}}"`
  .quiet()
  .text();
