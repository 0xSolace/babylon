...
async function loadBenchmark(benchmarkPath: string): Promise<BenchmarkGameSnapshot> {
...
async function generateBenchmark(config: BenchmarkConfig): Promise<BenchmarkGameSnapshot> {
...
async function runElizaBenchmark(config: ElizaBenchmarkConfig): Promise<SimulationResult> {
  ...
  if (config.benchmarkPath) {
    snapshot = await loadBenchmark(config.benchmarkPath);
  } else if (config.generatorConfig) {
    snapshot = await generateBenchmark(config.generatorConfig);
  } else {
    throw new Error('Either benchmarkPath or generatorConfig must be provided');
  }
...
async function runMultiple(config: ElizaBenchmarkConfig) {
  ...
  const avgAccuracy =
    runs.reduce((sum, r) => sum + r.metrics.predictionMetrics.accuracy, 0) / runs.length;
...
async function main() {
  ...
  const markets = parseInt(args.find((a) => a.startsWith('--markets='))?.split('=')[1] || '5', 10);
  const perpetuals = parseInt(
    args.find((a) => a.startsWith('--perpetuals='))?.split('=')[1] || '3',
    10
  );
  const agents = parseInt(args.find((a) => a.startsWith('--agents='))?.split('=')[1] || '8', 10);
  const seed =
    parseInt(args.find((a) => a.startsWith('--seed='))?.split('=')[1] || '', 10) || Date.now();
