...
async function check() {
  ...
  if (game) {
    const minAgo = game.lastTickAt
      ? Math.floor((Date.now() - game.lastTickAt.getTime()) / 60000)
      : 999;
    console.log(
      `  Last tick: ${game.lastTickAt?.toISOString() || 'Never'} (${minAgo} min ago)`
    );
  ...
  const totalSize = pools.reduce(
    (sum, p) => sum + p.positions.reduce((s, pos) => s + pos.size, 0),
    0
  );
