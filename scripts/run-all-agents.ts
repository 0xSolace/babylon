...
const maxAgents = parseInt(
  args.find((arg) => arg.startsWith('--max='))?.split('=')[1] || '1'
);
...
function startAgent(characterFile: string, index: number): Promise<void> {
  return new Promise((resolve) => {
    ...
    logger.info(
      `[${index + 1}/${maxAgents}] Starting ${characterName}...`,
      undefined,
      'Script'
    );

    const args = [AGENT_SCRIPT, '--character', characterPath];
    ...
    agentProcess.stdout?.on('data', (data) => {
      ...
      if (
        data.toString().includes('Agent started') ||
        data.toString().includes('initialized')
      ) {
        agentInfo.status = 'running';
        activeCount++;
        logger.info(
          `${characterName} is running (${activeCount}/${maxAgents} active)`,
          undefined,
          'Script'
        );
      }
    ...
    agentProcess.on('exit', (code) => {
      if (code !== 0) {
        logger.warn(
          `${characterName} exited with code ${code}`,
          undefined,
          'Script'
        );
...
async function main() {
  ...
  logger.info(
    `Found ${characterFiles.length} character files:`,
    characterFiles.map((file, i) => `${i + 1}. ${file.replace('.json', '')}`),
    'Script'
  );

  logger.info('Starting agents...', undefined, 'Script');
  logger.info(
    'Starting agents with 2-second delay between each',
    undefined,
    'Script'
  );
  ...
  setInterval(() => {
    ...
    const starting = Array.from(agents.values()).filter(
      (a) => a.status === 'starting'
    ).length;
    const errors = Array.from(agents.values()).filter(
      (a) => a.status === 'error'
    ).length;
    const stopped = Array.from(agents.values()).filter(
      (a) => a.status === 'stopped'
    ).length;
...
if (args.includes('--help') || args.includes('-h')) {
  logger.info(
    `Usage: bun run scripts/run-all-agents.ts [OPTIONS]
