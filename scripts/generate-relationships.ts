...
function calculateCompatibility(actor1: ActorData, actor2: ActorData): number {
  ...
  const sharedOrgs = actor1.affiliations?.filter((org) => actor2.affiliations?.includes(org)) || [];
...
function determineRelationshipType(
  ...
  const sharedOrgs =
    actor1.affiliations?.filter((org) => actor2.affiliations?.includes(org))?.length || 0;
...
async function main() {
  ...
  for (let i = 0; i < actors.length; i++) {
    ...
    for (const actor2 of partners) {
      ...
      const [sortedActor1Id, sortedActor2Id] = ids;
      if (!sortedActor1Id || !sortedActor2Id) {
        continue;
      }
  ...
  const index = JSON.parse(require('node:fs').readFileSync(indexPath, 'utf-8'));
