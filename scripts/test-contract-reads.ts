...
async function main() {
  ...
  try {
    ...
    facets.forEach((facet, index) => {
      logger.info(
        `${index + 1}. ${facet.facetAddress} (${facet.functionSelectors.length} functions)`,
        undefined,
        'Script'
      );
    });
    ...
    logger.info(
      `Total facet addresses: ${facetAddresses.length}`,
      undefined,
      'Script'
    );
  } catch (error) {
  ...
  try {
    logger.info(
      `Identity Registry Address: ${CONTRACTS.identityRegistry}`,
      undefined,
      'Script'
    );
    ...
    const deployerAddress =
      '0xFfA6A2Ac8bcAE47af29b623B97071E676647556A' as Address;
    ...
    logger.info(
      `Deployer (${deployerAddress}) registered: ${isRegistered}`,
      undefined,
      'Script'
    );
  } catch (error) {
    logger.error(
      'Identity Registry read failed:',
      error instanceof Error ? error.message : error,
      'Script'
    );
  }
  ...
  try {
    logger.info(
      `Reputation System Address: ${CONTRACTS.reputationSystem}`,
      undefined,
      'Script'
    );
    ...
    logger.info(
      `Connected to Identity Registry: ${identityRegistryAddress}`,
      undefined,
      'Script'
    );
    ...
    if (isCorrect) {
      ...
      logger.warn(
        'Identity Registry address mismatch!',
        {
  ...
  try {
    const diamondCode = await publicClient.getCode({
      address: CONTRACTS.diamond,
    });
    const identityCode = await publicClient.getCode({
      address: CONTRACTS.identityRegistry,
    });
    const reputationCode = await publicClient.getCode({
      address: CONTRACTS.reputationSystem,
    });

    logger.info(
      `Diamond has deployed code: ${diamondCode && diamondCode !== '0x'}`,
      undefined,
      'Script'
    );
    logger.info(
      `Identity Registry has deployed code: ${identityCode && identityCode !== '0x'}`,
      undefined,
      'Script'
    );
    logger.info(
      `Reputation System has deployed code: ${reputationCode && reputationCode !== '0x'}`,
      undefined,
      'Script'
    );
  } catch (error) {
  ...
  logger.info('='.repeat(60), undefined, 'Script');
  logger.info(
    'All basic contract reads completed successfully!',
    undefined,
    'Script'
  );
  logger.info('Contracts are deployed and accessible', undefined, 'Script');
  logger.info(
    'Frontend integration should work correctly',
    undefined,
    'Script'
  );
  logger.info(
