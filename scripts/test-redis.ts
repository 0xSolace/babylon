...
async function testRedis() {
  ...
  if (!isRedisAvailable()) {
    ...
    logger.info(
      'Make sure REDIS_URL or UPSTASH_REDIS_REST_URL is configured',
      undefined,
      'Test'
    );
    process.exit(1);
  ...
  logger.info(
    `✅ Redis client initialized (type: ${redisClientType})`,
    undefined,
    'Test'
  );
  ...
  logger.info(
    '\nTest 1: Publishing message to channel "test:feed"...',
    undefined,
    'Test'
  );
  ...
  logger.info(
    '✅ TTL test message published (will expire in 60s)',
    undefined,
    'Test'
  );
  ...
  logger.info(
    `✅ Chat channel test: ${chatMessages.length} message(s) received`,
    undefined,
    'Test'
  );
