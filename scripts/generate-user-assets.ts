...
async function generateProfilePicture(index: number): Promise<string> {
  logger.info(
    `Generating profile picture ${index + 1}/${PROFILE_PICTURE_COUNT}...`,
    undefined,
    'CLI'
  );
  ...
  if (!result.data.images || result.data.images.length === 0) {
    throw new Error(
      `Fal.ai API returned no images for profile picture ${index}. Response: ${JSON.stringify(result.data)}`
    );
  }
  ...
  if (!firstImage || !firstImage.url) {
    throw new Error(
      `First image missing URL for profile picture ${index}. Image data: ${JSON.stringify(firstImage)}`
    );
  }

  const imageUrl = firstImage.url;
  logger.info(
    `Generated profile picture ${index + 1}: ${imageUrl}`,
    undefined,
    'CLI'
  );
...
async function generateBanner(index: number): Promise<string> {
  logger.info(
    `Generating banner ${index + 1}/${BANNER_COUNT}...`,
    undefined,
    'CLI'
  );
  ...
  if (!result.data.images || result.data.images.length === 0) {
    throw new Error(
      `Fal.ai API returned no images for banner ${index}. Response: ${JSON.stringify(result.data)}`
    );
  }
  ...
  if (!firstImage || !firstImage.url) {
    throw new Error(
      `First image missing URL for banner ${index}. Image data: ${JSON.stringify(firstImage)}`
    );
  }
...
async function main() {
  ...
  logger.info(
    `\n🎨 Generating ${PROFILE_PICTURE_COUNT} profile pictures...`,
    undefined,
    'CLI'
  );
  for (let i = 0; i < PROFILE_PICTURE_COUNT; i++) {
    try {
      ...
      logger.error(
        `Failed to generate profile picture ${i + 1}: ${errorMessage}`,
        { error, errorStack },
        'CLI'
      );
  ...
  // Generate banners
  logger.info(
    `\n🖼️  Generating ${BANNER_COUNT} profile banners...`,
    undefined,
    'CLI'
  );
  for (let i = 0; i < BANNER_COUNT; i++) {
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.error(
        `Failed to generate banner ${i + 1}: ${errorMessage}`,
        { error, errorStack },
        'CLI'
      );
      logger.warn(`Continuing with remaining images...`, undefined, 'CLI');
  ...
  logger.info(
    `   Generated ${generatedCount} images in ${elapsedTime} minutes`,
    undefined,
    'CLI'
  );
