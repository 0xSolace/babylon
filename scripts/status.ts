...
async function checkWalletStatus() {
  ...
  const blockTime =
    block?.timestamp !== undefined ? new Date(block.timestamp * 1000).toISOString() : 'N/A';
...
async function checkAgent0Config() {
  ...
  const requiredVars = ['AGENT0_ENABLED', 'BASE_SEPOLIA_RPC_URL', 'BABYLON_GAME_PRIVATE_KEY'];
