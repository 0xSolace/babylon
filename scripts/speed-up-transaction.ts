...
async function speedUpTransaction() {
  try {
    ...
    if (pendingNonce === confirmedNonce) {
      console.log(
        '✅ No pending transactions! The update may have already completed.'
      );
    ...
    console.log(`\n⛽ Current Network Gas:`);
    console.log(
      `   Max Fee: ${ethers.formatUnits(currentMaxFee, 'gwei')} gwei`
    );
    console.log(
      `   Priority Fee: ${ethers.formatUnits(currentPriorityFee, 'gwei')} gwei`
    );
    ...
    console.log(
      `   Priority Fee: ${ethers.formatUnits(newPriorityFee, 'gwei')} gwei`
    );
    ...
    const ipfsConfig = process.env.PINATA_JWT
      ...
      : {
          ipfsProvider: 'node' as const,
          ipfsNodeUrl: process.env.IPFS_NODE_URL || 'https://ipfs.io',
        };
    ...
    console.log(
      '   This will replace the stuck transaction at the same nonce\n'
    );
