...
function parseMetadata(metadata: AgentMetadata[]): Record<string, unknown> {
...
async function queryBabylonUsersAndAgents() {
  ...
  try {
    ...
    const babylonUsers = allAgents.filter((a) => {
      const meta = parseMetadata(a.metadata);
      return (
        meta.capabilities?.platform === 'babylon' &&
        meta.capabilities?.userType === 'player'
      );
    });
    ...
    const babylonAgents = allAgents.filter((a) => {
      ...
      return (
        meta.capabilities?.platform === 'babylon' &&
        meta.capabilities?.userType === 'agent'
      );
    });
    ...
    if (babylonGame) {
      ...
      console.log(
        `   Registered: ${new Date(Number(babylonGame.createdAt) * 1000).toISOString()}`
      );

      if (meta.capabilities) {
        console.log(
          `   Markets: ${meta.capabilities.markets?.join(', ') || 'N/A'}`
        );
        console.log(
          `   Protocols: ${meta.capabilities.protocols?.join(', ') || 'N/A'}`
        );
        console.log(
          `   Actions: ${meta.capabilities.actions?.length || 0} available`
        );
      }
      console.log('─'.repeat(80));
      console.log();
    ...
    if (babylonUsers.length > 0) {
      ...
      for (const user of babylonUsers.slice(0, 10)) {
        // Show first 10
        ...
        console.log(
          `      Registered: ${new Date(Number(user.createdAt) * 1000).toISOString()}`
        );
    ...
    if (babylonAgents.length > 0) {
      ...
      for (const agent of babylonAgents.slice(0, 10)) {
        // Show first 10
        ...
        console.log(`      Wallet: ${meta.agentWallet || 'N/A'}`);
        console.log(
          `      Registered: ${new Date(Number(agent.createdAt) * 1000).toISOString()}`
        );
    ...
    console.log('\n📊 Summary:');
    console.log(
      `   🎮 Babylon Game: ${babylonGame ? '✅ Registered' : '❌ Not Found'}`
    );
    console.log(`   👥 Babylon Users: ${babylonUsers.length}`);
